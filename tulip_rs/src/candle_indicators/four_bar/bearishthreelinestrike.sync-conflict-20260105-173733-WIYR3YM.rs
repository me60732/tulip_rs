use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_height, cdl_real_within_body, determine_volatility, cdl_real_in_body_position, FILL, LONG, RED};
...
use crate::candle_types::{CDLBasic, CDLDoji, CDLMarubozu, DojiOptions};
...
pub fn info() -> CandleInfo {
    CandleInfo {
        ...
        prior_trend: TrendType::Downtrend,
        bars: 4,
...
pub fn min_data(options: &[f64]) -> usize {
    rema_min_data(options) + 4
}
pub fn output_length(data_len: usize, options: &[f64]) -> usize {
    data_len - min_data(options) + 4
}
...
pub fn indicator_from_state(inputs: &[&[f64]; 4], options: &[f64], indicator_state: &IndicatorState, _optional_outputs: Option<&[bool]>) -> Result<Output, IndicatorError> {
    ...
    let mut open = Vec::<f64>::with_capacity(info.bars + inputs[0].len());
    open.extend(&state_prices[0]);
    open.extend(inputs[0]);
    let mut high = Vec::<f64>::with_capacity(info.bars + inputs[1].len());
    high.extend(&state_prices[1]);
    high.extend(inputs[1]);
    let mut low = Vec::<f64>::with_capacity(info.bars + inputs[2].len());
    low.extend(&state_prices[2]);
    low.extend(inputs[2]);
    let mut close = Vec::<f64>::with_capacity(info.bars + inputs[3].len());
    close.extend(&state_prices[3]);
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    let (preceding, first, second, third, fourth) = (i-4, i-3, i-2, i-1 ,i);

    //compare fills
    if cdl_body_fill(open[second], close[second]) != FILL { return 0 }
    if cdl_body_fill(open[third], close[third]) != FILL { return 0 }
    if cdl_body_fill(open[first], close[first]) != FILL { return 0 }

    if !cdl_real_within_body((open[first], close[first]), open[second]) { return 0 }
    if !cdl_real_within_body((open[second], close[second]), open[third]) { return 0 }
    
    if cdl_real_in_body_position((open[fourth], close[fourth]), close[third]) < 0.0 { return 0 }
    if cdl_real_in_body_position((open[fourth], close[fourth]), open[first]) > 100.0 { return 0 }

    
    if cdl_colour(close[first], close[second]) != RED { return 0 }
    if cdl_colour(close[second], close[third]) != RED { return 0 }
    if cdl_colour(close[preceding], close[first]) != RED { return 0 }

    if CDLDoji::is_candlestick(open[second], high[second], low[second], close[second], options) { return 0 }
    if CDLDoji::is_candlestick(open[third], high[third], low[third], close[third], options) { return 0 }
    if CDLDoji::is_candlestick(open[first], high[first], low[first], close[first], options) { return 0 }

    //fourth bar
    if let Some(bar) = CDLMarubozu::classify(open[fourth], high[fourth], low[fourth], close[fourth], options) {
        if !cdl_height((high[fourth], low[fourth]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) != LONG { return 0 }
        if !matches!(bar, CDLMarubozu::WhiteMarubozu | CDLMarubozu::OpeningWhiteMarubozu | CDLMarubozu::ClosingWhiteMarubozu) { return 0 }
    } else if let Some(bar) = CDLBasic::classify(open[fourth], high[fourth], low[fourth], close[fourth], options) {
        if !matches!(bar, CDLBasic::WhiteCandle | CDLBasic::LongWhiteCandle) { return 0 }
    } else {
        return 0
    }
