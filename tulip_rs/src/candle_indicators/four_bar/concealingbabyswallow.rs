use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_real_within_body, determine_volatility, FILL, RED};
...
use crate::candle_types::{DojiOptions, CDLMarubozu, CDLSpinngingTop};
...
fn cycle(open: &[f64], high: &[f64], low: &[f64], close: &[f64], avg_line: f64, avg_body: f64, start: usize, options: &[f64], output: &mut [f64]) -> (f64, f64) {
    ...
    for (i, _) in close.iter().enumerate().take(close.len()).skip(start) {
        ...
        pattern = calc(open, high, low, close, i, &options);//unsafe { calc_unchecked(open, high, low, close, i, &options) };
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_body_fill(open[fourth], close[fourth]) != FILL { return 0 }
    
    if !cdl_real_within_body((open[fourth], close[fourth]), low[third]) { return 0 }
    if !cdl_real_within_body((open[fourth], close[fourth]), high[third]) { return 0 }
    if !cdl_real_within_body((open[second], close[second]), high[third]) { return 0 }
    ...
    if low[third] != close[third] { return 0 }
    if open[third] >= close[second] { return 0 }

    if !matches!(CDLSpinngingTop::classify(open[third], high[third], low[third], close[third], options), Some(CDLSpinngingTop::HighWave)) { return 0 }
    if !matches!(CDLMarubozu::classify(open[second], high[second], low[second], close[second], options), Some(CDLMarubozu::BlackMarubozu)) { return 0 }
    if !matches!(CDLMarubozu::classify(open[first], high[first], low[first], close[first], options), Some(CDLMarubozu::BlackMarubozu)) { return 0 }
