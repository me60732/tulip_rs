use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_height, cdl_real_in_body_position, cdl_real_within_body, determine_volatility, FILL, LONG, RED};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_real_in_body_position((open[fourth], close[fourth]), close[third]) > 100.0 { return 0 }
    if cdl_real_in_body_position((open[fourth], close[fourth]), open[first]) < 0.0 { return 0 }
    ...
    if let Some(bar) = CDLMarubozu::classify(open[fourth], high[fourth], low[fourth], close[fourth], options) {
        ...
        if !matches!(bar, CDLMarubozu::BlackMarubozu | CDLMarubozu::OpeningBlackMarubozu | CDLMarubozu::ClosingBlackMarubozu) { return 0 }
    } else if let Some(bar) = CDLBasic::classify(open[fourth], high[fourth], low[fourth], close[fourth], options) {
        if !matches!(bar, CDLBasic::BlackCandle | CDLBasic::LongBlackCandle) { return 0 }
    } else {
        return 0
    }
