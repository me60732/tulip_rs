use crate::cdlcommon::{cdl_colour, cdl_height, cdl_wick_length, determine_volatility, LONG, RED, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_wick_length((open[i], close[i]), low[i], None) == LONG { return 0 }
    if cdl_wick_length((open[i], close[i]), high[i], Some(2.0)) == SHORT { return 0 }
    
    if cdl_colour(close[i-1], close[i]) == RED { return 0 }
