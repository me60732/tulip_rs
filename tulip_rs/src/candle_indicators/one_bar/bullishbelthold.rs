...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_height((open[i], close[i]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
    if cdl_colour(close[i-1], close[i]) == GREEN { return 0 }
    if cdl_wick_length((open[i], close[i]), low[i], Some(0.25000001)) == LONG { return 0 }

    if let Some(CDLMarubozu::OpeningWhiteMarubozu) = CDLMarubozu::classify(open[i], high[i], low[i], close[i], options) { return 100 }
    0
}
