...
use crate::cdlcommon::{
    cdl_colour, cdl_height, cdl_wick_length, determine_volatility, GREEN, LONG, SHORT,
};
...
pub fn calc(
    ...
    if cdl_wick_length((open[i], close[i]), low[i], Some(3.0)) == SHORT {
        return 0;
    }
    if cdl_wick_length((open[i], close[i]), high[i], None) == LONG {
        return 0;
    }
    ...
    if cdl_colour(close[i - 1], close[i]) == GREEN {
        return 0;
    }

    if CDLDoji::is_candlestick(open[i], high[i], low[i], close[i], options) {
        return 0;
    }
    100
}
