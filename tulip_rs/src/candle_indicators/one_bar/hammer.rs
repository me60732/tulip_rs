use crate::cdlcommon::{cdl_colour, cdl_height, cdl_wick_length, determine_volatility, LONG, GREEN, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    if cdl_height((open[i], close[i]), options.avg_body, options.min_cdl_height, options.min_cdl_height_tolerance) == LONG { return 0 }

    if cdl_colour(close[i-1], close[i]) == GREEN { return 0 }
    
    if cdl_wick_length((open[i], close[i]), low[i], Some(2.0)) == SHORT { return 0 }
    if cdl_wick_length((open[i], close[i]), low[i], Some(3.00000001)) == LONG { return 0 }
    if cdl_wick_length((open[i], close[i]), high[i], None) == LONG { return 0 }
    if cdl_height((high[i], low[i]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
