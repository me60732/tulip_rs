use crate::cdlcommon::determine_volatility;
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //a doji candle with at least one shadow, pattern's high price has to be lower than the low price of the previous candle (i.e. price gap is required), a candle prior the pattern cannot be a Four-Price Doji
    if high[i] >= low[i-1] { return 0 }
    ...
    if let Some(CDLDoji::FourPriceDoji) = CDLDoji::classify(open[i-1], high[i-1], low[i-1], close[i-1], options) { return 0 }
