...
use crate::cdlcommon::{cdl_body_fill, determine_volatility, HALLOW};
...
//TODO dependent on trend line, find a way to define a trend line
...
pub fn calc(
    ...
    let (first, second) = (i - 1, i);

    if cdl_body_fill(open[second], close[second]) == HALLOW {
        return 0;
    }
    ...
    if let Some(CDLDoji::FourPriceDoji) =
        CDLDoji::classify(open[first], high[first], low[first], close[first], options)
    {
        return 0;
    }
    if let Some(CDLDoji::FourPriceDoji) = CDLDoji::classify(
        open[second],
        high[second],
        low[second],
        close[second],
