use crate::cdlcommon::{cdl_colour, cdl_height, determine_volatility, SHORT, GREEN};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_colour(close[prev], close[first]) == GREEN { return 0 }
    ...
    if let Some(bar) = CDLMarubozu::classify(open[second], high[second], low[second], close[second], options) {
        if !matches!(bar, CDLMarubozu::ClosingBlackMarubozu | CDLMarubozu::BlackMarubozu) { return 0 }
    } else {
        return 0
    }

    if let Some(bar) = CDLMarubozu::classify(open[first], high[first], low[first], close[first], options) {
        if cdl_height((open[first], close[first]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
        if !matches!(bar, CDLMarubozu::ClosingBlackMarubozu | CDLMarubozu::BlackMarubozu) { return 0 }
    } else {
        return 0
    }
