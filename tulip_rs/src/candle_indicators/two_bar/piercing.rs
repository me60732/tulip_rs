use crate::cdlcommon::{cdl_colour, cdl_height, cdl_real_in_body_position, determine_volatility, GREEN, RED, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //2nd candle The opening price of the second line may be equal to the first candle’s closing price. The closing price of the second line may be equal to the opening price of the first candle. These two situations cannot happen at the same time, however.
    ...
    if pos <= 50.0 || pos >= 100.0 { return 0 }
