use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_down_bar, cdl_height, determine_volatility, FILL, GREEN, HALLOW, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    { return 0 }
    if cdl_down_bar((open[preceding], close[preceding]), (open[first], close[first])) { return 0 }
    ...
    || cdl_height((high[second], low[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT 
    { return 0 }

    //no candle can be a doji, only check 2nd and 3rd candle as 1st is checked against Doji in that if statement
