use crate::cdlcommon::{cdl_height, determine_volatility, SHORT};
...
use crate::candle_types::{CDLMarubozu, DojiOptions};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_height((open[first], close[first]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT
    || cdl_height((open[second], close[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT 
    { return 0 }
    
    if let Some(bar) = CDLMarubozu::classify(open[first], high[first], low[first], close[first], options) {
        if !matches!(bar, CDLMarubozu::ClosingBlackMarubozu | CDLMarubozu::OpeningBlackMarubozu | CDLMarubozu::BlackMarubozu) { return 0 }
    } else {
        return 0
    }
    if let Some(bar) = CDLMarubozu::classify(open[second], high[second], low[second], close[second], options) {
        if !matches!(bar, CDLMarubozu::ClosingWhiteMarubozu | CDLMarubozu::OpeningWhiteMarubozu | CDLMarubozu::WhiteMarubozu) { return 0 }
    } else {
        return 0
    }
