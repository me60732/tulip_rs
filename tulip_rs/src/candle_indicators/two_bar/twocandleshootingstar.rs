use crate::cdlcommon::{cdl_colour, cdl_height, cdl_wick_length, determine_volatility, RED, SHORT};
...
use crate::candle_types::{CDLBasic, CDLMarubozu, CDLSpinngingTop, DojiOptions};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_wick_length((open[second], close[second]), high[second], Some(2.5)) == SHORT { return 0 }
    if open[second] < close[first] { return 0 }

    //1st
    if cdl_colour(close[prev], close[first]) == RED { return 0 }
