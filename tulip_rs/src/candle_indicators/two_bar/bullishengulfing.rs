...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if let Some(basic_result) = CDLBasic::classify(open[second], high[second], low[second], close[second], options) {
        ...
        if cdl_height((high[second], low[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
        if !matches!(marubozu_result, CDLMarubozu::WhiteMarubozu | CDLMarubozu::ClosingWhiteMarubozu | CDLMarubozu::OpeningWhiteMarubozu) { return 0 }
    } else {
        return 0
    }   
