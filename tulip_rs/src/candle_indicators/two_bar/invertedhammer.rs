use crate::cdlcommon::{cdl_colour, cdl_height, cdl_wick_length, determine_volatility, GREEN, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if !CDLSpinngingTop::is_candlestick(open[second], high[second], low[second], close[second], options)
    || CDLSpinngingTop::is_candlestick(open[first], high[first], low[first], close[first], options) 
    { return 0 }
