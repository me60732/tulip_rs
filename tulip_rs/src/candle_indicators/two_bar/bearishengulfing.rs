use crate::cdlcommon::{cdl_colour, cdl_real_within_body, cdl_body_fill, cdl_height, determine_volatility, SHORT, RED, HALLOW, GREEN};
...
use crate::candle_types::{CDLBasic, CDLSpinngingTop, CDLDoji, CDLMarubozu, DojiOptions};
...
pub fn output_length(data_len: usize, options: &[f64]) -> usize {
    let info = info();
    data_len - min_data(options) + info.bars
}
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    let (first, second, prev) = (i-1, i, i-2);
    ...
    if cdl_colour(close[prev], close[first]) == RED
