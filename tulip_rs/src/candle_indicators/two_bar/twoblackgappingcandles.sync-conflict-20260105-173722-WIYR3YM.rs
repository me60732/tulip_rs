use crate::cdlcommon::{cdl_body_fill, cdl_wick_length, determine_volatility, HALLOW, LONG};
...
use crate::candle_types::{CDLDoji, DojiOptions, CDLSpinngingTop};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if open[second] >= open[first] { return 0 }
    if open[second] <= low[first] { return 0 }
    if close[second] >= close[first] { return 0 }
    if CDLSpinngingTop::is_candlestick(open[second], high[second], low[second], close[second], options) && (cdl_wick_length((open[second], close[second]), high[second], Some(2.0000001)) == LONG || cdl_wick_length((open[second], close[second]), low[second], Some(2.0000001)) == LONG) {
            return 0;
    }

    //1st bar
    if cdl_body_fill(open[first], close[first]) == HALLOW { return 0 }
    if open[first] >= low[preceding] { return 0 }
    if CDLSpinngingTop::is_candlestick(open[first], high[first], low[first], close[first], options) && (cdl_wick_length((open[first], close[first]), high[first], Some(2.0000001)) == LONG || cdl_wick_length((open[first], close[first]), low[first], Some(2.0000001)) == LONG) {
            return 0;
    }
