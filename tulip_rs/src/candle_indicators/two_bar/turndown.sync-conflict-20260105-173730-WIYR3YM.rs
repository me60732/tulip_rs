...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if let Some(CDLDoji::FourPriceDoji) = CDLDoji::classify(open[first], high[first], low[first], close[first], options) { return 0 }
    if let Some(CDLDoji::FourPriceDoji) = CDLDoji::classify(open[second], high[second], low[second], close[second], options) { return 0 }
