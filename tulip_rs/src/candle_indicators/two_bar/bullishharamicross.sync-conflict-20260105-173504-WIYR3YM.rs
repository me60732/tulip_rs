use crate::cdlcommon::{cdl_colour, cdl_real_within_body, cdl_height, GREEN, RED, SHORT, determine_volatility};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if !cdl_real_within_body((open[first], close[first]), low[second]) 
    && !cdl_real_within_body((open[first], close[first]), high[second]) 
    { return 0 }
