use crate::cdlcommon::{cdl_colour, cdl_body_fill, cdl_height, cdl_real_within_body, FILL, GREEN, RED, LONG, determine_volatility};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_colour(close[second], close[third]) != GREEN 
    ...
    //check last 2 bars open within previous bars body
    ...
    if CDLSpinngingTop::is_candlestick(open[first], high[first], low[first], close[first], options)
    || CDLSpinngingTop::is_candlestick(open[second], high[second], low[second], close[second], options)
    || CDLSpinngingTop::is_candlestick(open[third], high[third], low[third], close[third], options) 
    {return 0}
