...
use crate::cdlcommon::{
    cdl_colour, cdl_height, cdl_real_in_body_position, cdl_real_within_body, determine_volatility,
    RED, SHORT,
};
...
pub fn calc(
    ...
    if cdl_colour(close[first], close[second]) == RED
        || cdl_colour(close[preceding], close[first]) == RED
    {
        return 0;
    }

    //3rd bar opens within the boy of the 2nd; 2nd opens and closes within the body of the 1st
    ...
    if matches!(
        ...
        Some(CDLDoji::FourPriceDoji) | None
    ) {
        return 0;
    }
    ...
    if let Some(basic_result) =
        CDLBasic::classify(open[third], high[third], low[third], close[third], options)
    {
        if !matches!(
            basic_result,
            CDLBasic::LongBlackCandle | CDLBasic::BlackCandle
        ) {
            return 0;
        }
    } else if let Some(marubozu_result) =
        CDLMarubozu::classify(open[third], high[third], low[third], close[third], options)
    {
        if !matches!(
            marubozu_result,
            CDLMarubozu::BlackMarubozu
                | CDLMarubozu::OpeningBlackMarubozu
                | CDLMarubozu::ClosingBlackMarubozu
        ) {
            return 0;
        }
    } else {
    ...
    if let Some(basic_result) =
        CDLBasic::classify(open[first], high[first], low[first], close[first], options)
    {
        if !matches!(
            basic_result,
            CDLBasic::WhiteCandle | CDLBasic::LongWhiteCandle
        ) {
            return 0;
        }
    } else if let Some(marubozu_result) =
        CDLMarubozu::classify(open[first], high[first], low[first], close[first], options)
    {
        if !matches!(
            marubozu_result,
            CDLMarubozu::WhiteMarubozu
                | CDLMarubozu::OpeningWhiteMarubozu
                | CDLMarubozu::ClosingWhiteMarubozu
        ) {
            return 0;
        }
    } else {
