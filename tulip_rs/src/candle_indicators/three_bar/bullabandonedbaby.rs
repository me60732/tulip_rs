use crate::cdlcommon::{cdl_colour, cdl_height, cdl_real_in_body_position, cdl_real_within_body, determine_volatility, GREEN, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //first 2 bars must be green close > previous close
    if cdl_colour(close[first], close[second]) == GREEN { return 0 }
    if cdl_colour(close[preceding], close[first]) == GREEN { return 0 }

    //3rd bar must open bellow the 2nd bar body, long line, close > half way into the 1st bar body
    if cdl_real_within_body((open[first], close[first]), open[second]) { return 0 }
    if cdl_real_within_body((open[first], close[first]), close[second]) { return 0 }
    if cdl_real_within_body((open[second], close[second]), open[third]) { return 0 }

    if cdl_real_in_body_position((open[first], close[first]), close[third]) < 50.0 { return 0 }

    if high[second] >= low[third] { return 0 }
    if high[second] >= low[first] { return 0 }

    if matches!(CDLDoji::classify(open[second], high[second], low[second], close[second], options), Some(CDLDoji::FourPriceDoji) | None) { return 0 }
    
    if cdl_height((high[third], low[third]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
    if let Some(basic_result) = CDLBasic::classify(open[third], high[third], low[third], close[third], options) {
        ...
        if !matches!(marubozu_result, CDLMarubozu::WhiteMarubozu | CDLMarubozu::OpeningWhiteMarubozu | CDLMarubozu::ClosingWhiteMarubozu) { return 0 }
    } else {
        return 0
    }

    if cdl_height((high[first], low[first]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
    if let Some(basic_result) = CDLBasic::classify(open[first], high[first], low[first], close[first], options) {
        ...
        if !matches!(marubozu_result, CDLMarubozu::BlackMarubozu | CDLMarubozu::OpeningBlackMarubozu | CDLMarubozu::ClosingBlackMarubozu) { return 0 }
    } else {
        return 0
    }
