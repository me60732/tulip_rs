use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_height, determine_volatility, FILL, GREEN, RED, HALLOW, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //1st candle
    if cdl_body_fill(open[first], close[first]) == FILL
    || cdl_body_fill(open[second], close[second]) == FILL
    || cdl_body_fill(open[third], close[third]) == HALLOW 
    { return 0 }
    ...
    if open[third] > close[second] && open[third] < open[second] { return 0 }

    if low[second] <= high[first]
    || low[third] <= high[first] 
    { return 0 }
