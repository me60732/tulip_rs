...
use crate::cdlcommon::{cdl_colour, cdl_real_in_body_position, determine_volatility, GREEN, RED};
...
pub fn calc(
    ...
    //1st bar above previous close
    if cdl_colour(close[first], close[second]) == GREEN {
        return 0;
    }
    ...
    //All bars must be Doji except four-price doji
    if matches!(
        CDLDoji::classify(open[first], high[first], low[first], close[first], options),
    ...
    if matches!(
        CDLDoji::classify(open[third], high[third], low[third], close[third], options),
        Some(CDLDoji::FourPriceDoji) | None
    ) {
        return 0;
    }

    if cdl_real_in_body_position((open[preceding], close[preceding]), close[first]) >= 100.0 {
        return 0;
    }
    100
}
