use crate::cdlcommon::{cdl_body_fill, cdl_body_greater_body, cdl_colour, cdl_height, cdl_real_in_body_position, cdl_real_within_body, cdl_wick_length, determine_volatility, FILL, LONG, RED};
...
use crate::candle_types::{CDLSpinngingTop, CDLDoji, DojiOptions, CDLMarubozu};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //check the fill, all 3 bars must be filled close < open
    if cdl_body_fill(open[third], close[third]) != FILL
    || cdl_body_fill(open[second], close[second]) != FILL
    || cdl_body_fill(open[first], close[first]) != FILL 
    {return 0}
    
    if !cdl_real_within_body((high[second], low[second]), open[third])
    || !cdl_real_within_body((high[second], low[second]), close[third]) 
    { return 0 }

    //all 3 vars must be getting smaller
    if !cdl_body_greater_body((high[first], low[first]), (high[second], low[second]), 1.0)
    || !cdl_body_greater_body((high[second], low[second]), (high[third], low[third]), 1.0) 
    { return 0 }
    ...
    if CDLDoji::is_candlestick(open[first], high[first], low[first], close[first], options)
    || CDLDoji::is_candlestick(open[second], high[second], low[second], close[second], options)
    || CDLDoji::is_candlestick(open[third], high[third], low[third], close[third], options) 
    { return 0 }

    //1st bar
    if !CDLSpinngingTop::is_candlestick(open[first], high[first], low[first], close[first], options) { return 0 }
    if cdl_wick_length((open[first], close[first]), low[first], Some(1.000001)) != LONG { return 0 }

    //2nd bar
    if open[first] <= open[second] { return 0 }
    if low[second] <= low[first] { return 0 }

    //3rd bar
    let pos = cdl_real_in_body_position((high[first], low[first]), open[third]);
    if !(45.0..=55.0).contains(&pos) { return 0 }
    if cdl_height((high[third], low[third]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == LONG { return 0 }
    if !CDLMarubozu::is_candlestick(open[third], high[third], low[third], close[third], options) { return 0 }
