use crate::cdlcommon::{cdl_body_fill, cdl_body_greater_body, cdl_colour, cdl_height,  determine_volatility, FILL, RED, LONG, SHORT};
...
use crate::candle_types::{CDLBasic, CDLDoji, CDLSpinngingTop, DojiOptions};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //check colour all 3 bars must be green close > previous close
    ...
    //check the fill, all 3 bars must be Hollow (white) close > open
    ...
    if cdl_body_greater_body((open[third], close[third]), (open[second], close[second]), 1.0) { return 0 } 
    if cdl_body_greater_body((open[third], close[third]), (open[first], close[first]), 1.0) { return 0 }

    //3rd bar smaller then the previous 2 bars, closing is close to the previous close
    if cdl_height((low[first], high[first]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
    if cdl_height((low[second], high[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT { return 0 }
    if cdl_height((low[third], high[third]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == LONG { return 0 }
    
    if open[third] < open[second] || open[third] > close[second] { return 0 }
    if open[second] < open[first] || open[second] > close[first] { return 0 }

    if let Some(basic_result) = CDLBasic::classify(open[third], high[third], low[third], close[third], options) {
        if !matches!(basic_result, CDLBasic::ShortWhiteCandle) { return 0 }
    } else if let Some(spinning_top_result) = CDLSpinngingTop::classify(open[third], high[third], low[third], close[third], options) {
        if !matches!(spinning_top_result, CDLSpinngingTop::WhiteSpinningTop) { return 0 }
    } else {
        return 0;
    }
