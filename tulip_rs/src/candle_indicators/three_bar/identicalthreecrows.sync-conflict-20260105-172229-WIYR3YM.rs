use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_height, cdl_real_in_body_position, determine_volatility, FILL, LONG, RED};
...
use crate::candle_types::{CDLSpinngingTop, doji::{CDLDoji, DojiOptions}};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //check colour all 3 bars must be red close < previous close
    if cdl_colour(close[second], close[third]) != RED
    ...
    //check line size, all 3 bars must be long
    if cdl_height((high[third], low[third]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) != LONG
    || cdl_height((open[second], close[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) != LONG
    || cdl_height((open[first], close[first]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) != LONG 
    { return 0}

    // no caldle can be Doji or spinning tops
    ...
    //check last 2 bars open close to previous bars close
    ...
    if !(-10.0..=10.0).contains(&second_pos) {return 0}
    if !(-10.0..=10.0).contains(&third_pos) {return 0}

    if CDLSpinngingTop::is_candlestick(open[first], high[first], low[first], close[first], options) {return 0}
    if CDLSpinngingTop::is_candlestick(open[second], high[second], low[second], close[second], options) {return 0}
    if CDLSpinngingTop::is_candlestick(open[third], high[third], low[third], close[third], options) {return 0}
