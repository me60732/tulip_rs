use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_height, cdl_real_in_body_position, cdl_similar_height, cdl_total_range, determine_volatility, FILL, RED, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_real_in_body_position((close[first], low[first]), low[second]) <= 100.0 { return 0 }
    if cdl_real_in_body_position((close[first], low[first]), low[third]) <= 100.0 { return 0 }
