use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_height, cdl_real_within_body, cdl_wick_length, determine_volatility, FILL, GREEN, HALLOW, LONG, RED, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    || cdl_body_fill(open[second], close[second]) == HALLOW
    || cdl_body_fill(open[first], close[first]) == HALLOW 
    { return 0 }
    ...
    //first 2 bars are long, third bar is short
    if cdl_height((high[first], low[first]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) != LONG
    || cdl_height((high[second], low[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) != LONG
    || cdl_height((high[third], low[third]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) != SHORT 
    { return 0 }

    if low[first] <= low[second] { return 0 }
    if cdl_wick_length((open[second], close[second]), low[second], Some(2.0)) != LONG { return 0 }

    //3rd bar body bellow 2nd body low[3rd] > low[2nd]
    if close[third] >= close[second] { return 0 }
    if low[third] <= low[second] { return 0 }
