...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if low[second] <= high[third] { return 0 }
    if low[second] <= high[first] { return 0 }
    ...
    || cdl_height((high[first], low[first]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT 
    { return 0 }

    if let Some(basic_result) = CDLBasic::classify(open[third], high[third], low[third], close[third], options) {
        if !matches!(basic_result, CDLBasic::LongBlackCandle | CDLBasic::BlackCandle) { return 0 }
