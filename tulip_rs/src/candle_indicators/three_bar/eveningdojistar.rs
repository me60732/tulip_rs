use crate::cdlcommon::{cdl_colour, cdl_height, cdl_real_in_body_position, cdl_real_within_body, determine_volatility, RED, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    // 2ndbar must be a Doji but not a Four Price Doji, body must gap up, lower wick can not gap above the previous or the proceeding line
    ...
    //1st bar any white candle appearing as a long line 
