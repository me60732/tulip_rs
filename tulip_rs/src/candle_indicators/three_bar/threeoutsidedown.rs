...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    || cdl_colour(close[second], close[third]) == GREEN 
    { return 0 }
    ...
    if let Some(basic_result) = CDLBasic::classify(open[second], high[second], low[second], close[second], options) {
        ...
        || !matches!(marubozu_result, CDLMarubozu::BlackMarubozu | CDLMarubozu::ClosingBlackMarubozu | CDLMarubozu::OpeningBlackMarubozu) 
        { return 0 }
    } else {
        return 0
    }
