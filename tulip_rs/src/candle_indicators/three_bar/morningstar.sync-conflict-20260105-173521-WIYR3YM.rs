use crate::cdlcommon::{cdl_colour, cdl_height, cdl_real_in_body_position, cdl_real_within_body, determine_volatility, GREEN, LONG, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    || cdl_colour(close[preceding], close[first]) == GREEN 
    { return 0 }
    ...
    if cdl_height((high[second], low[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == LONG { return 0 }
