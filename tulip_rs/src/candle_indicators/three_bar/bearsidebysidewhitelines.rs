...
use crate::cdlcommon::{
    cdl_body_fill, cdl_colour, cdl_height, cdl_real_in_body_position, cdl_similar_height,
    cdl_total_range, determine_volatility, FILL, GREEN, HALLOW, SHORT,
};
use crate::cldcommontypes::{CandleInfo, CandleStick, ForcastType, TrendType};
use crate::common::{validate_indicator_state, validate_inputs, validate_options};
use crate::indicators::rema::{
    calc as calc_rema, min_data as rema_min_data, multiplier as rema_multiplier,
};
use crate::types::{
    DisplayType, IndicatorError, IndicatorState, IndicatorType, Info, InfoIndicatorState, Output,
};
...
pub fn indicator(
    ...
    let (mut avg_line, mut avg_body) =
        determine_volatility(open, high, low, close, line_period, body_period);
    let mut output: Vec<f64> = Vec::with_capacity(output_length(close.len(), options));

    (avg_line, avg_body) = cycle(
        open,
        high,
        low,
        close,
    ...
    Ok(Output {
        ...
        state: IndicatorState {
            ...
            array_values: Some(vec![
                open[open.len() - bars..].to_vec(),
                high[high.len() - bars..].to_vec(),
                low[low.len() - bars..].to_vec(),
                close[close.len() - bars..].to_vec(),
            ]),
        },
    })
}

pub fn indicator_from_state(
    inputs: &[&[f64]; 4],
    options: &[f64],
...
pub fn calc(
    ...
    //last 2 bars bodies not greater the first bar body
    ...
    if cdl_body_fill(open[first], close[first]) == HALLOW {
        return 0;
    }
    if cdl_body_fill(open[second], close[second]) == FILL {
        return 0;
    }
    if cdl_body_fill(open[third], close[third]) == FILL {
        return 0;
    }

    //last 2 bars high must be lower then the 1st bar low & total line height should be simular and the body height should be simular, but not exceeding the 1st bar body height
    if cdl_real_in_body_position((close[first], low[first]), high[second]) >= 0.0 {
        return 0;
    }
    if cdl_real_in_body_position((close[first], low[first]), high[third]) >= 0.0 {
        return 0;
    }
    ...
    if cdl_height(
        ...
        options.avg_line,
        options.min_cdl_height,
        options.min_cdl_height_tolerance,
    ) == SHORT
    {
        return 0;
    }
    if !cdl_similar_height((high[second], low[second]), (high[third], low[third]), 10.0) {
        return 0;
    }
    ...
    if CDLDoji::is_candlestick(open[third], high[third], low[third], close[third], options) {
        return 0;
    }

    // last 2 bars open and close prices are simular
    let mut pos = cdl_real_in_body_position((open[second], open[second]), open[third]);
    if !(-5.0..=105.0).contains(&pos) {
        return 0;
    }
    pos = cdl_real_in_body_position((close[second], close[second]), close[third]);
    if !(-5.0..=105.0).contains(&pos) {
        return 0;
    }
