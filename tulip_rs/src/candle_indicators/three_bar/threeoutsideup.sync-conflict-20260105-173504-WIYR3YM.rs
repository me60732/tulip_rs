use crate::cdlcommon::{cdl_colour, cdl_body_fill, cdl_real_within_body, cdl_height, FILL, GREEN, RED, SHORT, determine_volatility};
...
use crate::candle_types::{CDLDoji, DojiOptions, CDLBasic, CDLMarubozu, CDLSpinngingTop};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    // only the first bar can be a doji and not a 4 price doji
    if CDLDoji::is_candlestick(open[second], high[second], low[second], close[second], options)
    ...
    if let Some(CDLDoji::FourPriceDoji) = CDLDoji::classify(open[first], high[first], low[first], close[first], options) {
        return 0
    } else if !CDLBasic::is_candlestick(open[first], high[first], low[first], close[first], options) { 
        return 0 
    }
    
    if let Some(basic_result) = CDLBasic::classify(open[second], high[second], low[second], close[second], options) {
        if !matches!(basic_result, CDLBasic::WhiteCandle | CDLBasic::LongWhiteCandle) { return 0 }
    } else if let Some(marubozu_result) = CDLMarubozu::classify(open[second], high[second], low[second], close[second], options) {
        if cdl_height((high[second], low[second]), options.avg_line, options.min_cdl_height, options.min_cdl_height_tolerance) == SHORT
        || !matches!(marubozu_result, CDLMarubozu::WhiteMarubozu | CDLMarubozu::ClosingWhiteMarubozu | CDLMarubozu::OpeningWhiteMarubozu) 
        { return 0 }
    } else {
        return 0
    }
