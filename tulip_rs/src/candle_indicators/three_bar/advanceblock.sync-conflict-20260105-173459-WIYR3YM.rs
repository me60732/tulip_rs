use crate::cdlcommon::{cdl_body_fill, cdl_body_greater_body, cdl_colour, cdl_total_wick_length, cdl_real_within_body, determine_volatility, FILL, RED};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    //last 2 bars open within the previous bar body
    if !cdl_real_within_body((open[second], close[second]), open[third])
    || !cdl_real_within_body((open[first], close[first]), open[second]) 
    { return 0 }

    // shadows of the second and third line should be longer than the one of the first line
    ...
    //check body height in each bar must be smaller then the previous bar
    if cdl_body_greater_body((open[third], close[third]), (open[second], close[second]), 1.0)
    || cdl_body_greater_body((open[second], close[second]), (open[first], close[first]), 1.0) 
    { return 0 }
