use crate::cdlcommon::{cdl_colour, cdl_real_in_body_position, determine_volatility, RED, GREEN };
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if matches!(CDLDoji::classify(open[first], high[first], low[first], close[first], options), Some(CDLDoji::FourPriceDoji) | None) { return 0 }
    ...
    if matches!(CDLDoji::classify(open[third], high[third], low[third], close[third], options), Some(CDLDoji::FourPriceDoji) | None) { return 0 }

    if cdl_real_in_body_position((open[preceding], close[preceding]), close[first]) >= 100.0 { return 0 }
    100
}
