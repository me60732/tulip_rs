use crate::cdlcommon::{cdl_colour, cdl_body_fill, determine_volatility, cdl_height, RED, HALLOW, GREEN, SHORT};
...
fn cycle(open: &[f64], high: &[f64], low: &[f64], close: &[f64], mut line_avg: f64, mut body_avg: f64, start: usize, options: &[f64], output: &mut [f64]) -> (f64, f64) {
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64] , close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if open[second] > open[third] { return 0 }
    if close[third] <= close[first] { return 0 }
