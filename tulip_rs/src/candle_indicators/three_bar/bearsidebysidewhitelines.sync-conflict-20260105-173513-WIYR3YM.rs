use crate::cdlcommon::{cdl_body_fill, cdl_colour, cdl_height, cdl_real_in_body_position, cdl_similar_height, cdl_total_range, determine_volatility, FILL, GREEN, HALLOW, SHORT};
...
pub fn calc(open: &[f64], high: &[f64], low: &[f64], close: &[f64], i: usize, options: &DojiOptions) -> i8 {
    ...
    if cdl_total_range(open[second], close[second]) > first_line_body { return 0 }
    if cdl_total_range(open[third], close[third]) > first_line_body { return 0 }

    if cdl_body_fill(open[first], close[first]) == HALLOW { return 0 }
    ...
    if cdl_real_in_body_position((close[first], low[first]), high[second]) >= 0.0 { return 0 }
    if cdl_real_in_body_position((close[first], low[first]), high[third]) >= 0.0 { return 0 }
    ...
    if !cdl_similar_height((high[second], low[second]), (high[third], low[third]), 10.0) { return 0 }
    ...
    if !(-5.0..=105.0).contains(&pos) { return 0 }
