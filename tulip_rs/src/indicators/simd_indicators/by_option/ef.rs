...
use crate::indicators::ef::{Ef, Indicator, IndicatorState, State, INPUTS, OPTIONS};
...
impl Driver<State<Warm>, usize> for EfDriver {
...
pub(crate) fn indicator_by_options<const N: usize>(
    ...
    validate_inputs::<OPTIONS>(inputs, options, Ef::min_data)?;
    ...
    for i in 0..N {
        ...
        let capacity = Ef::output_length(len, options[i]);
        let mut ef_line = crate::uninit_vec!(f64, capacity);
        let period = options[i][0] as usize;
        let state = State::init_state(inputs[0], period, &mut ef_line);
