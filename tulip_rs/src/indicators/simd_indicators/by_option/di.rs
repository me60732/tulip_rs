...
use crate::indicators::di::{Di, Indicator, IndicatorState, State, INPUTS, OPTIONS};
use crate::indicators::simd_indicators::di_simd::{SimdState, TSimdState, TState};
use crate::indicators::simd_indicators::road_train::{Asset, Driver, PrimeMover};
use crate::indicators::tr::Tr;
use crate::types::{IndicatorError, Warm};
use std::simd::Simd;

/// SIMD driver for the Directional Indicator (DI) indicator, processing `N` option-set lanes per scheduling epoch.
struct DiDriver {
    want_optional_outputs: (bool, bool, bool),
}

impl Driver<State<Warm>> for DiDriver {
    ...
    fn next_run<const N: usize>(
        ...
        //collect outputs
        let (plus_di_line_ptr, minus_di_line_ptr, atr_line_ptr, tr_line_ptr) = crate::extract_output_ptrs!(
            outputs,
            N,
            plus_di_line_ptr,
            minus_di_line_ptr,
        ...
        for i in 0..len {
            ...
            let (plus_di, minus_di, atr, tr) = state.calc(inputs);

            // Store results using pre-computed pointers
            crate::write_simd_at_indices!(N, i,
                plus_di_line_ptr => plus_di,
                minus_di_line_ptr => minus_di
            );
            if has_optional {
                ...
                crate::store_simd_optional_outputs_corrected!(i, N,
                    want_atr, atr_line_ptr => corrected(atr, state.atr_state.wilders_state.inv_multiplier)
                );
            }
        }
...
/// Calculates the Directional Indicator (DI) on a single asset with `N` different option sets
...
pub(crate) fn indicator_by_options<const N: usize>(
    ...
    validate_inputs::<OPTIONS>(inputs, options, Di::min_data)?;
    ...
    for i in 0..N {
        ...
        let (plus_di_line, minus_di_line, atr_line, mut tr_line);
        {
            let len = inputs[0].len();
            let capacity = Di::output_length(len, options[i]);
        ...
        let mut starts = [0; 4];
        starts[3] = crate::slice_outputs_start!(plus_di_line.len(), tr_line);
        if i == 0 {
            want_optional_outputs = crate::calc_want_flags!(atr_line, tr_line);
        }

        let mut output_buffer = vec![plus_di_line, minus_di_line, atr_line, tr_line];
