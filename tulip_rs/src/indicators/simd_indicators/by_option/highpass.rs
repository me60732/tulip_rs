...
/// SIMD driver for the Ehlers High Pass indicator, processing `N` option-set lanes
...
/// Calculates the Ehlers High Pass filter on a single asset with `N` different option
...
///
/// # Returns
/// `Ok((outputs, states))` where `outputs[i][0]` contains the `highpass` line
