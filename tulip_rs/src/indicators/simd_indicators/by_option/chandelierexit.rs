...
impl Driver<State, (usize, usize, (f64, (f64, f64)))> for ChandelierExitDriver {
    ...
    fn next_run<const N: usize>(
        ...
        //collect outputs
        let (long_line_ptr, short_line_ptr, atr_line_ptr, tr_line_ptr) =
            crate::extract_output_ptrs!(outputs, N, long_line_ptrs, short_line_ptrs, atr_line_ptrs, tr_line_ptrs);
        ...
        for j in 0..len {
            ...
            let (long, short, atr, tr) = unsafe {
                state.calc_unchecked_simd(high_ptrs, low_ptrs, close, i_simd, look_back, multipliers)
            };
...
pub fn indicator_by_options<const N: usize>(
    ...
    for i in 0..N {
        ...
        let state = State::new(inputs[0], inputs[1], inputs[2], params[i].0, params[i].1, &mut tr_line);
        let mut starts = [0; 4];
    ...
    let mut driver = ChandelierExitDriver { want_optional_outputs };
    let states_vec = road_train.drive(&mut driver);
    let mut states = Vec::with_capacity(N);
    for (state, &param) in states_vec.into_iter().zip(params.iter()) {
        let periods = (param.0, param.1);
        let multipliers = param.2;
        states.push(IndicatorState::new(inputs[0], inputs[1], state, periods, multipliers));
    }
