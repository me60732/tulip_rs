...
use crate::indicators::adxr::{Adxr, Indicator, IndicatorState, State, INPUTS, OPTIONS};
use crate::indicators::simd_indicators::adxr_simd::options::{SimdState, TSimdState, TState};
use crate::indicators::{adx::Adx, dx::Dx, tr::Tr};
...
pub fn indicator_by_options<const N: usize>(
    ...
    validate_inputs::<OPTIONS>(inputs, options, Adxr::min_data)?;
    ...
    for i in 0..N {
        ...
        let (adxr_line, (mut adx_line, mut dx_line, mut atr_line, mut tr_line), start) = {
            ...
            let adxr_capacity = Adxr::output_length(len, options[i]);
            let adx_capacity = Adx::output_length(len, options[i]);
            let dx_capacity = Dx::output_length(len, options[i]);
