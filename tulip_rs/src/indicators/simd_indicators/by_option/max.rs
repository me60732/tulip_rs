...
use crate::indicators::max::{Indicator, IndicatorState, Max, State, INPUTS, OPTIONS};
use crate::indicators::simd_indicators::max_simd::{options::SimdState, TSimdState, TState};
...
impl Driver<State<Warm>, usize> for MaxDriver {
    ...
    fn next_run<const N: usize>(
        ...
        for j in 0..len {
            let (max, _) = state.calc((real_ptrs, i_simd, look_back));
...
pub fn indicator_by_options<const N: usize>(
    ...
    validate_inputs::<OPTIONS>(inputs, options, Max::min_data)?;
    ...
    for (i, &(_period, look_back)) in periods.iter().enumerate() {
        ...
        let max_line = {
            ...
            let capacity = Max::output_length(len, options[i]);
