...
use crate::indicators::simd_indicators::trvi_simd::options::{SimdState, TSimdState, TState};
use crate::indicators::{
    ema::{Ema, Indicator},
    tr::Tr,
    trvi::{IndicatorState, State, Trvi, INPUTS, OPTIONS},
};
...
pub(crate) fn indicator_by_options<const N: usize>(
    ...
    validate_inputs::<OPTIONS>(inputs, options, Trvi::min_data)?;
    ...
    for i in 0..N {
        ...
        let (trvi_line, (mut tr_line, mut ema_line)) = {
            let capacity = Trvi::output_length(high.len(), options[i]);
            ...
            (
                ...
                crate::init_optional_outputs_eff!(
                    ...
                    tr_line: tr_capacity,
                    ema_line: Ema::output_length(tr_capacity, options[i])
                ),
            )
        };
