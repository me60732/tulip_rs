...
use crate::indicators::simd_indicators::willr_simd::{options::SimdState, TSimdState, TState};
...
impl Driver<State<Warm>, usize> for WillrDriver {
    ...
    fn next_run<const N: usize>(
        ...
        for j in 0..len {
            ...
            let (willr, min, max) = state.calc((high_ptrs, low_ptrs, close, i_simd, look_back));
...
pub(crate) fn indicator_by_options<const N: usize>(
    ...
    validate_inputs::<OPTIONS>(inputs, options, Willr::min_data)?;
    ...
    for i in 0..N {
        ...
        let (willr_line, (mut min_line, mut max_line)) = {
            let len = high.len();
            let capacity = Willr::output_length(len, options[i]);
            let min_max_capacity = Max::output_length(len, options[i]);
            (
