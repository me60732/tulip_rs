...
use crate::indicators::macd::{
    Macd, Indicator, validate_options, IndicatorState, State, INPUTS,
    OPTIONS,
};
...
impl Driver<State<Warm>> for MacdDriver {
    ...
    fn next_run<const N: usize>(
        ...
        for i in 0..len {
            ...
            let (macd, signal, histogram, short_ema, long_ema) = state.calc(values);
...
pub(crate) fn indicator_by_assets<const N: usize>(
    ...
    validate_inputs::<INPUTS>(inputs, Macd::min_data(options))?;
    ...
    for i in 0..N {
        ...
        let caps = Macd::slot_lengths(len, options);
