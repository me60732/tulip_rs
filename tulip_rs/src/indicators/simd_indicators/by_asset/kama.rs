use crate::indicators::kama::{
    Kama, Indicator, IndicatorState, State, INPUTS, OPTIONS,
};
...
impl Driver<State<Warm>> for KamaDriver {
...
pub(crate) fn indicator_by_assets<const N: usize>(
    ...
    validate_inputs::<INPUTS>(inputs, Kama::min_data(options))?;
    ...
    for i in 0..N {
        let (mut kama_line, mut ef_line) = {
            let capacity = Kama::output_length(inputs[i][0].len(), options);
            (
