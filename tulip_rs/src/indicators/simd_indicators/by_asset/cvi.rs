...
use crate::indicators::cvi::{
    Cvi, Indicator, IndicatorState, State, INPUTS,
    OPTIONS,
};
use crate::indicators::simd_indicators::cvi_simd::{
    assets::SimdState,
    TSimdState, TState
};
...
pub fn indicator_by_assets<const N: usize>(
    ...
    validate_inputs::<INPUTS>(inputs, Cvi::min_data(options))?;
    ...
    for i in 0..N {
        ...
        let cvi_line = {
            let capacity = Cvi::output_length(inputs[i][0].len(), options);
            crate::uninit_vec!(f64, capacity)
        };

        let state = State::init_state(inputs[i], period);
