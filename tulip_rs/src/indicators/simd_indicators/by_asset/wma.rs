...
use crate::indicators::wma::{
    Wma, Indicator, IndicatorState, State, INPUTS, OPTIONS,
};
...
impl Driver<State<Warm>> for WmaDriver {
...
pub(crate) fn indicator_by_assets<const N: usize>(
    ...
    validate_inputs::<INPUTS>(inputs, Wma::min_data(options))?;
    ...
    for i in 0..N {
        ...
        let (wma_line, sma_line) = {
            let capacity = Wma::output_length(inputs[i][0].len(), options);
            (
