...
use crate::indicators::simd_indicators::chandelierexit_simd::{
    assets::SimdState, TSimdState, TState,
};
use crate::indicators::simd_indicators::road_train::{Asset, Driver, PrimeMover};
use crate::indicators::{
    chandelierexit::{
        validate_options, ChandelierExit, Indicator, IndicatorState, State, INPUTS, OPTIONS,
    },
    max::Max,
    tr::Tr,
};
...
impl Driver<State<Warm>> for ChandelierExitDriver {
    ...
    fn next_run<const N: usize>(
        ...
        self.cycle::<N>(inputs, outputs, data_len, &mut state);
    ...
    fn cycle<const N: usize>(
        ...
        for (j, i) in (period..data_len).enumerate() {
            ...
            let (long, short, atr, tr, min, max) =
                state.calc((high_ptrs, low_ptrs, close, i, look_back));
...
pub(crate) fn indicator_by_assets<const N: usize>(
    ...
    validate_inputs::<INPUTS>(inputs, ChandelierExit::min_data(options))?;
    ...
    for i in 0..N {
        ...
        let (long_line, short_line, (atr_line, mut tr_line, mut min_line, mut max_line)) = {
            let len = high.len();
            let capacity = ChandelierExit::output_length(len, options);
