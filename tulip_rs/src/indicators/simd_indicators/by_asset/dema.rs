...
use crate::indicators::dema::{
    Dema, Indicator, IndicatorState, State, INPUTS, OPTIONS,
};
...
pub fn indicator_by_assets<const N: usize>(
    ...
    validate_inputs::<INPUTS>(inputs, Dema::min_data(options))?;
    ...
    for i in 0..N {
        let dema_capacity = Dema::output_length(inputs[i][0].len(), options);
        let dema_line = crate::uninit_vec!(f64, dema_capacity);
        let ema_capacity = Ema::output_length(inputs[i][0].len(), options);
