...
use crate::indicators::highpass::{
    HighPass, Indicator, IndicatorState, State, INPUTS, OPTIONS,
};
...
pub(crate) fn indicator_by_assets<const N: usize>(
    ...
    validate_inputs::<INPUTS>(inputs, HighPass::min_data(options))?;
    ...
    for i in 0..N {
        ...
        let highpass_line = {
            let capacity = HighPass::output_length(inputs[i][0].len(), options);
