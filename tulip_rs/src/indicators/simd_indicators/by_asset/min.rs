...
/// Calculates the Minimum Value (min) for `N` assets simultaneously using SIMD parallelism.
///
...
///
/// # Returns
/// `Ok((outputs, states))` where `outputs[i][0]` is the rolling minimum for asset `i`
