...
pub struct State<S = Cold> {
    ...
    pub smooth_buf: FixedRingBuffer<f64, 7, S>,

    // ── Stage 2: Detrender → I1, Q1 ──────────────────────────────────────────
    pub detrender_buf: FixedRingBuffer<f64, 7, S>,
    ...
    pub iq1_buf: FixedRingBuffer<Simd<f64, 2>, 7, S>,
...
impl State<Cold> {
    ...
    pub fn init_state(real: &[f64]) -> State<Warm> {
        ...
        State {
            ...
            detrender_buf: state.detrender_buf.into_full(),
            iq1_buf: state.iq1_buf.into_full(),
            i2_prev: state.i2_prev,
            q2_prev: state.q2_prev,
            re_prev: state.re_prev,
            im_prev: state.im_prev,
            period: state.period,
    ...
    pub fn into_full(self) -> State<Warm> {
        State {
            ...
            detrender_buf: self.detrender_buf.into_full(),
            iq1_buf: self.iq1_buf.into_full(),
            i2_prev: self.i2_prev,
            q2_prev: self.q2_prev,
            re_prev: self.re_prev,
            im_prev: self.im_prev,
            period: self.period,
...
impl TIndicatorState<INPUTS> for IndicatorState {
    fn batch_indicator(
        ...
        cycle(inputs[0], self, &mut dc_period_line);
...
impl Indicator<INPUTS, OPTIONS> for HomodyneDiscriminator {
    ...
    /// Fixed at 23 bars — independent of options (the HD has no user parameters).
