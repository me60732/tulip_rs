...
fn main() {
    // Ehlers Homodyne Discriminator — measures the dominant cycle period bar-by-bar.
    // No user-supplied options needed; all constants are fixed by Ehlers.
    // min_data = 23 (ring-buffer warmup). The IIR converges over ~50 bars.
    // Options: none (empty array)
    let options: [f64; 0] = [];
    ...
    println!("Dominant Cycle Period — {} values", outputs[0].len());
    println!("First 10: {:?}", &outputs[0][..10]);
    println!("Last   5: {:?}", &outputs[0][outputs[0].len() - 5..]);
    println!("\nNote: early bars are transient while IIR smoothers converge (~50 bars).");
