...
fn main() {
    ...
    // Uses the Homodyne Discriminator to measure the instantaneous phase rate-of-change
    // and derives an adaptive smoothing coefficient α each bar.
    //
    // Options: [fast_limit, slow_limit]
    //   fast_limit — maximum alpha (Ehlers default: 0.5)
    //   slow_limit — minimum alpha (Ehlers default: 0.05)
    // min_data = 23 (HD warmup). First ~50 bars are transient.
    let options = [0.5, 0.05]; // Ehlers defaults
    ...
    println!("MAMA — {} output bars", outputs[0].len());
    println!(
        "MAMA  (last 10): {:?}",
    ...
    println!("Continuation — MAMA: {:?}", continuation[0]);
    println!("Continuation — FAMA: {:?}", continuation[1]);
    ...
    println!(
        "\nSIMD by-assets MAMA last 5 (asset 0): {:?}",
        &simd_asset_outputs[0][0][simd_asset_outputs[0][0].len() - 5..]
    );
    println!(
        "SIMD by-assets FAMA last 5 (asset 0): {:?}",
    ...
    for (i, opts) in options_4.iter().enumerate() {
        println!(
            "\nSIMD by-options MAMA last 5 (fast_limit={}, slow_limit={}): {:?}",
            opts[0],
            opts[1],
            &simd_option_outputs[i][0][simd_option_outputs[i][0].len() - 5..]
        );
    }
}
