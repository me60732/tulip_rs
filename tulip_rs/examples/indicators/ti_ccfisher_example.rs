...
fn main() {
    // Ehlers CyberCycle Fisher — applies a Fisher Transform to the normalised
    // CyberCycle oscillator, making turning-point extremes more visible.
    //
    // Options: [alpha]  where alpha ∈ (0.0, 1.0).
    //   alpha = 0.0  → adaptive: alpha derived from the dominant cycle via the embedded HD.
    //   alpha > 0.0  → fixed. Ehlers' default is 0.07.
    // min_data = 56 (HD + CyberCycle + Fisher warmup)
    let options = [0.07]; // Ehlers default fixed alpha

    let inputs = [CLOSE.as_slice()];

    // --- Full run with all optional outputs enabled ---
    // outputs[0] = fisher    (Fisher Transform value)
    // outputs[1] = signal    (trigger = Fisher[1])
    ...
    println!("CCFisher — {} output bars", outputs[0].len());
    println!("Fisher    (all): {:?}", outputs[0]);
    println!("Signal    (all): {:?}", outputs[1]);
    println!("TrendMode (all): {:?}", outputs[2]);
    println!("Cycle     (all): {:?}", outputs[3]);
    println!("Peak      (all): {:?}", outputs[4]);
    ...
    println!(
        "\nCCFisher (adaptive alpha) — Fisher: {:?}",
        outputs_adaptive[0]
    );
    ...
    println!("  Fisher last: {:?}", outputs_partial[0].last());
    println!("  Signal last: {:?}", outputs_partial[1].last());
    ...
    println!("Continuation — Fisher: {:?}", continuation[0]);
    println!("Continuation — Signal: {:?}", continuation[1]);
    ...
    for (i, outs) in simd_asset_outputs.iter().enumerate() {
        let len = outs[0].len();
        println!(
            "\nSIMD by-assets Fisher last 5 (asset {}): {:?}",
            i,
            &outs[0][len - 5..]
        );
    }
    ...
    for (i, opts) in options_4.iter().enumerate() {
        ...
        println!(
            "\nSIMD by-options Fisher last 5  ({}): {:?}",
            label,
            &simd_option_outputs[i][0][len_f - 5..]
        );
        println!(
            "SIMD by-options Signal last 5  ({}): {:?}",
            label,
            &simd_option_outputs[i][1][len_s - 5..]
        );
    }
}
