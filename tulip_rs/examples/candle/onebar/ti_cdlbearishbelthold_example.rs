...
fn main() {
    ...
    println!("Last bar before pattern:");
    println!(
        "  O: {}, H: {}, L: {}, C: {}",
        open.last().unwrap(),
        high.last().unwrap(),
    ...
    println!("\nPattern bar (to be added):");
    println!(
        "  O: {}, H: {}, L: {}, C: {}",
        pattern_open[0], pattern_high[0], pattern_low[0], pattern_close[0]
    );
    println!(
        "  Body: {} (open > close = bearish)",
        pattern_open[0] - pattern_close[0]
    );
    println!(
        "  Upper wick: {} (should be ~0 for Opening Black Marubozu)",
        pattern_high[0] - pattern_open[0]
    );
    println!(
        "  Lower wick: {} (can exist)",
        pattern_close[0] - pattern_low[0]
    );
    println!();
    ...
    let (result, _) = match indicator(&inputs, &options, Some(ForecastType::BearishReversal)) {
        Ok(r) => r,
        Err(e) => panic!("Error: {}", e),
    };
    println!("\n\nResult: {:?}", result);
