use tulip_rs::candle_indicators::threeoutsideup::{indicator, indicator_from_state, info};
...
fn main() {
    ...
    let pattern_open = vec![88.30, 86.20, 89.00];
    let pattern_high = vec![88.40, 89.60, 91.85];
    let pattern_low = vec![87.27, 86.10, 88.90];
    let pattern_close = vec![87.28, 89.50, 91.80];
