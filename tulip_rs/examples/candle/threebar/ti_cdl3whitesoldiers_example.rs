...
fn main() {
    ...
    let pattern_open = vec![84.28, 85.25, 86.45];
    let pattern_high = vec![85.55, 86.65, 87.55];
    let pattern_low = vec![84.10, 85.20, 86.20];
    let pattern_close = vec![85.50, 86.50, 87.50];
    ...
    let (result, _) = match CandleStick::indicator(&inputs, &options, Some(ForecastType::BullishReversal)) {
        Ok(r) => r,
        Err(e) => panic!("Error: {}", e),
    };
    println!("\n\nResult: {:?}", result);
