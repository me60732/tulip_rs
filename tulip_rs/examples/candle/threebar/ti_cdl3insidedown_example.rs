...
fn main() {
    ...
    let pattern_open = vec![87.30, 88.00, 87.60];
    let pattern_high = vec![88.50, 88.10, 87.60];
    let pattern_low = vec![87.20, 87.30, 85.90];
    let pattern_close = vec![88.30, 87.40, 86.0];
    ...
    if let Some(patterns) = result.last().and_then(|opt| opt.as_ref()) {
        println!("Forecast type BearishReversal - Patterns found:");
        for pattern in patterns {
            let pattern_info = pattern.get_info();
            println!("  - {} ({}), Bars: {}", 
                pattern_info.full_name, 
