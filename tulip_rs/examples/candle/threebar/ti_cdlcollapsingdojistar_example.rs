use tulip_rs::candle_indicators::collapsingdojistar::{indicator, indicator_from_state, info};
...
fn main() {
    ...
    let pattern_open = vec![87.30, 87.10, 86.90];
    let pattern_high = vec![88.35, 87.20, 87.0];
    let pattern_low = vec![87.25, 87.05, 86.40];
    let pattern_close = vec![88.30, 87.10, 86.50];
