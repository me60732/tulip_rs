...
fn main() {
    ...
    let pattern_open = vec![87.40, 88.50, 87.60];
    let pattern_high = vec![88.10, 88.60, 87.65];
    let pattern_low = vec![87.30, 87.10, 84.90];
    let pattern_close = vec![88.00, 87.20, 85.0];
    ...
    if let Some(patterns) = result.get(result.len() - 2).and_then(|opt| opt.as_ref()) {
