...
fn main() {
    ...
    let pattern_open = vec![86.30, 85.35];
    ...
    if let Some(patterns) = result.last().and_then(|opt| opt.as_ref()) {
        ...
        for pattern in patterns {
            ...
            if let Some(ext_pattern) = pattern_info.extended_pattern {
    ...
    let (result, _) = match CandleStick::indicator(&inputs, &options, Some(ForecastType::BearishContinuation)) {
        Ok(r) => r,
        Err(e) => panic!("Error: {}", e),
    };
    println!("\n\nResult: {:?}", result);
