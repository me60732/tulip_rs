use tulip_rs::candle_indicators::tweezerstop::{indicator, indicator_from_state, info};
...
fn main() {
    ...
    let pattern_open = vec![87.30, 87.80];
    let pattern_high = vec![88.50, 88.50];
    let pattern_low = vec![87.20, 87.20];
    let pattern_close = vec![88.30, 87.70];
