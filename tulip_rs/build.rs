...
fn main() {
    ...
    generate_msw_twiddles(&out_dir);
    println!("cargo:warning=Generated MSW twiddle tables for periods 6..=50");
}

fn generate_msw_twiddles(out_dir: &str) {
    use std::f64::consts::PI;
    const TPI: f64 = PI * 2.0;
    const MIN_PERIOD: usize = 6;
    const MAX_PERIOD: usize = 50;
    const NUM_PERIODS: usize = MAX_PERIOD - MIN_PERIOD + 1; // 45

    let mut code = String::from(
        "// Auto-generated by build.rs — do not edit by hand.\n\
         // DFT twiddle factors for periods 6..=50.\n\
         // MSW_COS_TWIDDLES[i][k] = cos(2π·(period−1−k)/period), period = i+6.\n\
         // MSW_SIN_TWIDDLES[i][k] = sin(2π·(period−1−k)/period), period = i+6.\n\
         // Only the first `period` entries of each row are meaningful; the rest are 0.0.\n\n",
    );

    let mut emit_table = |name: &str, use_cos: bool| {
        ...
        for period in MIN_PERIOD..=MAX_PERIOD {
            ...
            for k in 0..50usize {
                ...
                let v = if k < period {
                    let angle = TPI * (period - 1 - k) as f64 / period as f64;
                    if use_cos {
                        angle.cos()
                    } else {
                        angle.sin()
    ...
    emit_table("MSW_COS_TWIDDLES", true);
    emit_table("MSW_SIN_TWIDDLES", false);

    let dest = std::path::Path::new(out_dir).join("msw_twiddles.rs");
    std::fs::write(&dest, &code).expect("Failed to write msw_twiddles.rs");
}
