...
=== "Rust"
    ...
    ```rust
    use tulip_rs::indicators::trima::{Trima, TIndicatorState, Indicator};

    let close = vec![81.59, 81.06, 82.87, 83.00, 83.61,
                     83.15, 82.84, 83.99, 84.55, 84.36_f64];

    let (outputs, _state) = Trima::indicator(&[close.as_slice()], &[14.0], None).unwrap();
    println!("TRIMA(14): {:?}", outputs[0]);

    // State continuation
    let partial = close[..8].to_vec();
    let (outputs2, mut state) = Trima::indicator(&[partial.as_slice()], &[14.0], None).unwrap();
...
=== "Go"
    ...
    res, st, _ := indicators.Trima.Indicator(close, options, nil)
    fmt.Println(res.Rows[0]) // TRIMA(14) values
    res.Close()
    st.Close()

    // Partial computation + state continuation.
    res2, st2, _ := indicators.Trima.Indicator(close[:8], options, nil)
    res2.Close() // outputs consumed or closed; state stays live
    batch, _ := st2.Batch(close[8:], nil)
    fmt.Println(batch.Rows[0]) // continued TRIMA values
    batch.Close()
    st2.Close()
    ```
...
=== "Java"
    ...
    Outcome oc = Trima.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // TRIMA(14) values
    }

    // Partial computation + state continuation.
    Outcome p = Trima.indicator(
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued TRIMA values
        }
    }
    ```
...
=== "Rust"
    ...
    **By assets** — same period applied to 4 assets in parallel:
    ...
    ```rust
    use tulip_rs::indicators::trima::{Trima, Indicator};
    ...
    let results = Trima::indicator_by_assets::<4>(&inputs, &[14.0], None).unwrap();
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    ```rust
    use tulip_rs::indicators::trima::{Trima, IndicatorByOptions};
    ...
    let results = Trima::indicator_by_options::<4>(&[close.as_slice()], &opts, None).unwrap();
...
=== "Go"
    ...
    **By assets** — same period applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    assets := [][indicators.TrimaInputs][]float64{{a1}, {a2}, {a3}, {a4}}
    sim, _ := indicators.Trima.SimdByAssets(assets, []float64{14.0}, nil)
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    close := []float64{81.59, 81.06, 82.87, 83.00, 83.61,
                       83.15, 82.84, 83.99, 84.55, 84.36}
    sim2, _ := indicators.Trima.SimdByOptions(close, [][]float64{{5}, {10}, {14}, {20}}, nil)
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Trima.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Trima.simdByOptions(new double[][] {close},
