...
=== "Java"
    ...
    Outcome oc = Dema.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // DEMA(14) values
    }

    // Partial computation + state continuation.
    Outcome p = Dema.indicator(
        new double[][] {java.util.Arrays.copyOfRange(close, 0, 8)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(
            new double[][] {java.util.Arrays.copyOfRange(close, 8, 10)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued DEMA values
        }
    }
    ```
...
=== "Java"
    ...
    // Request optional output: ema
    boolean[] mask = {true}; // one per optional output (ema)
    Outcome oc = Dema.indicator(new double[][] {close}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // dema (primary)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // "ema" (optional — requested)
    }
    oc.state().close();
    ```
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Dema.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Dema.simdByOptions(new double[][] {close},
