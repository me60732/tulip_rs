...
=== "Java"
    ...
    Outcome oc = Wma.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // WMA(14) values
    }

    // Partial computation + state continuation.
    Outcome p = Wma.indicator(
        new double[][] {java.util.Arrays.copyOfRange(close, 0, 8)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(
            new double[][] {java.util.Arrays.copyOfRange(close, 8, 9)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued WMA values
        }
    }
    ```
...
=== "Java"
    ...
    // Request the SMA alongside the WMA
    boolean[] mask = {true}; // one per optional output (sma)
    Outcome oc = Wma.indicator(new double[][] {close}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // wma (primary)
...
=== "Rust"
    ...
    **By assets** — same period applied to 4 assets in parallel:
    ...
    let results = Wma::indicator_by_assets::<4>(&inputs, &[14.0], None).unwrap();
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    ```rust
    use tulip_rs::indicators::wma::{Wma, IndicatorByOptions};
    ...
    let results = Wma::indicator_by_options::<4>(&[close.as_slice()], &opts, None).unwrap();
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Wma.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Wma.simdByOptions(new double[][] {close},
