...
=== "Rust"
    ...
    let (outputs, mut state) = Nvi::indicator(&inputs, &[], None).unwrap();
...
=== "Java"
    ...
    Outcome oc = Nvi.indicator(new double[][] {close, volume}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // NVI values
    }

    // Partial computation + state continuation.
    int n = 10;
    Outcome p = Nvi.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued NVI
        }
    }
    ```
...
=== "Java"
    ...
    **By assets** — same options applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Nvi.simdByAssets(assets, new double[] {}, null)) {
