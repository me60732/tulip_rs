...
=== "Java"
    ...
    ```java
    import org.tuliprs.*;
    import org.tuliprs.indicators.Stddev;
    ...
    Outcome oc = Stddev.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // StdDev values
    }

    // Partial computation + state continuation.
    Outcome p = Stddev.indicator(
        new double[][] {java.util.Arrays.copyOfRange(close, 0, 8)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(
            new double[][] {java.util.Arrays.copyOfRange(close, 8, 10)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued StdDev values
        }
    }
    ```
...
=== "Java"
    ...
    boolean[] mask = {true}; // request sma

    Outcome oc = Stddev.indicator(new double[][] {close}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // stddev (primary)
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    // Lanes 2–4 reuse real1 in this example.
    double[] real1 = {81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36};
    double[][][] assets = {{real1}, {real1}, {real1}, {real1}};
    try (SimdResult sim = Stddev.simdByAssets(assets, new double[] {20.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Stddev.simdByOptions(new double[][] {close},
            new double[][] {{10}, {20}, {30}, {50}}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
