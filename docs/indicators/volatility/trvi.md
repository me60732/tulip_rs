...
=== "Java"
    ...
    Outcome oc = Trvi.indicator(new double[][] {high, low, close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // TRVI values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Trvi.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued TRVI
        }
    }
    ```
...
=== "Java"
    ...
    `trvi` exposes 2 optional outputs: `tr`, `ema`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    boolean[] mask = {true, true}; // tr, ema
    Outcome oc = Trvi.indicator(new double[][] {high, low, close}, new double[] {14.0}, mask);
    try (Result res = oc.result()) {
        // row 0 = trvi (primary), row 1 = tr, row 2 = ema
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // tr
    }
    oc.state().close();
    ```
...
=== "Go"
    ...
    **By assets** — same period applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    l1 := []float64{81.29, 80.64, 81.31, 82.65, 83.07,
                    83.11, 82.49, 82.30, 84.15, 84.11}
    c1 := []float64{81.59, 81.06, 82.87, 83.00, 83.61,
                    83.15, 82.84, 83.99, 84.55, 84.36}
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    #define EXPANDED_LEN (10 * 20)
    h_exp := make([]float64, EXPANDED_LEN)
    l_exp := make([]float64, EXPANDED_LEN)
    c_exp := make([]float64, EXPANDED_LEN)
    for i := 0; i < 20; i++ {
        for j := 0; j < 10; j++ {
            h_exp[i*10+j] = high[j]
            l_exp[i*10+j] = low[j]
            c_exp[i*10+j] = close[j]
        }
    }

    sim2, _ := indicators.Trvi.SimdByOptions(h_exp, l_exp, c_exp, [][]float64{{5.0}, {10.0}, {14.0}, {20.0}}, nil)
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Trvi.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    // (high/low/close series as in the Basic tab)
    try (SimdResult sim = Trvi.simdByOptions(new double[][] {high, low, close},
