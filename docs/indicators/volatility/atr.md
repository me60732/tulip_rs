...
=== "Java"
    ...
    Outcome oc = Atr.indicator(new double[][] {high, low, close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // ATR values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Atr.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued ATR
        }
    }
    ```
...
=== "Java"
    ...
    boolean[] mask = {true}; // request tr
    Outcome oc = Atr.indicator(new double[][] {high, low, close}, new double[] {14.0}, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // atr (primary)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // tr  (optional — requested)
    }
    oc.state().close();
    ```
...
=== "Java"
    ...
    **By assets** — same options applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Atr.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    // (high/low/close series as in the Basic tab)
    try (SimdResult sim = Atr.simdByOptions(new double[][] {high, low, close},
