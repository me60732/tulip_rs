...
=== "Java"
    ...
    // instantaneoustrendline takes no options — pass an empty array
    Outcome oc = Instantaneoustrendline.indicator(new double[][] {close}, new double[] {});
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // trendline values
    }

    // State continuation
    int n = 35;
    Outcome p = Instantaneoustrendline.indicator(
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued trendline
        }
    }
    ```
...
=== "Java"
    ...
    `instantaneoustrendline` exposes 3 optional outputs: `trigger`, `dc_period`, `alpha`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    boolean[] mask = {true, true, true}; // trigger, dc_period, alpha
    Outcome oc = Instantaneoustrendline.indicator(new double[][] {close}, new double[] {}, mask);
    try (Result res = oc.result()) {
        // row 0 = trendline (primary), row 1 = trigger, row 2 = dc_period; row 3 = alpha
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // trigger
    }
    oc.state().close();
    ```
...
=== "Go"
    ...
    assets := [][indicators.InstantaneoustrendlineInputs][]float64{{close}, {a2}, {a3}, {a4}}
...
=== "Java"
    ...
    **By assets** — applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    try (SimdResult sim = Instantaneoustrendline.simdByAssets(assets, new double[] {}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
            System.out.printf("Asset %d: %s%n", i + 1,
                java.util.Arrays.toString(sim.toDoubleArray(i, 0))); // trendline
        }
