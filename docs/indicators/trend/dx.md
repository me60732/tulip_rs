...
=== "Rust"
    ...
    let (outputs, mut state) = Dx::indicator(&inputs, &[14.0], None).unwrap();
    ...
    let (outputs2, mut state) = Dx::indicator(&[partial_high.as_slice(), partial_low.as_slice(), partial_close.as_slice()], &[14.0], None).unwrap();
    println!("{:?}", outputs2[0]);
...
=== "Go"
    ...
    res, st, _ := indicators.Dx.Indicator(high, low, close, options, nil)
    fmt.Println(res.Rows[0]) // DX values
    res.Close()
    st.Close()

    // Partial computation + state continuation.
    res2, st2, _ := indicators.Dx.Indicator(high[:8], low[:8], close[:8], options, nil)
    res2.Close() // outputs consumed or closed; state stays live
    batch, _ := st2.Batch(high[8:], low[8:], close[8:], nil)
    fmt.Println(batch.Rows[0]) // continued DX values
    batch.Close()
    st2.Close()
    ```
...
=== "Java"
    ...
    ```java
    import org.tuliprs.*;
    import org.tuliprs.indicators.Dx;
    ...
    Outcome oc = Dx.indicator(new double[][] {high, low, close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // DX values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Dx.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued DX values
        }
    }
    ```
...
=== "Rust"
    ...
    let mask = [true, false];
    let (outputs, _state) = Dx::indicator(
...
=== "Go"
    ...
    ```go
    // dx exposes 2 optional outputs, in fixed order: atr, tr.
    res, st, _ := indicators.Dx.Indicator(high, low, close, []float64{14.0}, []bool{true, false})
    defer res.Close()
    defer st.Close()

    dx  := res.Rows[0] // dx (primary)
    atr := res.Rows[1] // atr (optional — requested; tr skipped, not appended)
    fmt.Println(dx, atr)
    ```

=== "Java"

    `dx` exposes 2 optional outputs: `atr`, `tr`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    Outcome oc = Dx.indicator(new double[][] {high, low, close}, new double[] {14.0}, mask);
    try (Result res = oc.result()) {
        // row 0 = dx (primary), row 1 = atr (optional — requested); tr was not requested
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // dx
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // atr
    }
    oc.state().close();
    ```
...
=== "Rust"
    ...
    **By assets** — same options, N assets in parallel:
    ...
    ```rust
    use tulip_rs::indicators::dx::{Dx, Indicator};
    ...
    let results = Dx::indicator_by_assets::<4>(&inputs, &[14.0], None).unwrap();
    ...
    **By options** — same asset, N option sets in parallel:
    ...
    ```rust
    use tulip_rs::indicators::dx::{Dx, IndicatorByOptions};

    let opts: [&[f64; 1]; 4] = [&[7.0], &[14.0], &[21.0], &[28.0]];
    let results = Dx::indicator_by_options::<4>(&inputs, &opts, None).unwrap();
...
=== "Go"
    ...
    **By assets** — same period applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    assets := [][indicators.DxInputs][]float64{{h1, l1, c1}, {h2, l2, c2}, {h3, l3, c3}, {h4, l4, c4}}
    sim, _ := indicators.Dx.SimdByAssets(assets, []float64{14.0}, nil)
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    sim2, _ := indicators.Dx.SimdByOptions(high, low, close, [][]float64{{7}, {14}, {21}, {28}}, nil)
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Dx.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Dx.simdByOptions(new double[][] {high, low, close},
