...
=== "Rust"
    ...
    let (outputs, mut state) = Adx::indicator(&inputs, &[14.0], None).unwrap();
    ...
    let (outputs2, mut state) = Adx::indicator(&[partial_high.as_slice(), partial_low.as_slice(), partial_close.as_slice()], &[14.0], None).unwrap();
    println!("{:?}", outputs2[0]);
...
=== "Go"
    ...
    res, st, _ := indicators.Adx.Indicator(high, low, close, options, nil)
    fmt.Println(res.Rows[0]) // ADX(14) values
    res.Close()
    st.Close()

    // Partial computation + state continuation.
    res2, st2, _ := indicators.Adx.Indicator(high[:8], low[:8], close[:8], options, nil)
    res2.Close() // outputs consumed or closed; state stays live
    batch, _ := st2.Batch(high[8:], low[8:], close[8:], nil)
    fmt.Println(batch.Rows[0]) // continued ADX values
    batch.Close()
    st2.Close()
    ```
...
=== "Java"
    ...
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // ADX(14) values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Adx.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued ADX
        }
    }
    ```
...
=== "Rust"
    ...
    let (outputs, _state) = Adx::indicator(
...
=== "Go"
    ...
    mask := []bool{true, true, false} // dx, atr, tr
    res, st, _ := indicators.Adx.Indicator(high, low, close, options, mask)
    fmt.Println(res.Rows[0])  // adx (primary)
    fmt.Println(res.Rows[1])  // dx (optional — requested)
    fmt.Println(res.Rows[2])  // atr (optional — requested)
    res.Close()
    st.Close()
    ```
...
=== "Java"
    ...
    `adx` exposes 3 optional outputs: `dx`, `atr`, `tr`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    boolean[] mask = {true, true, false}; // dx, atr, tr
    Outcome oc = Adx.indicator(new double[][] {high, low, close}, new double[] {14.0}, mask);
    try (Result res = oc.result()) {
        // row 0 = adx (primary), row 1 = dx, row 2 = atr; tr was not requested
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // dx
    }
    oc.state().close();
    ```
...
=== "Rust"
    ...
    **By assets** — same options, N assets in parallel:
    ...
    let results = Adx::indicator_by_assets::<4>(&inputs, &[14.0], None).unwrap();
    ...
    **By options** — same asset, N option sets in parallel:
    ...
    ```rust
    use tulip_rs::indicators::adx::{Adx, IndicatorByOptions};

    let opts: [&[f64; 1]; 4] = [&[7.0], &[14.0], &[21.0], &[28.0]];
    let results = Adx::indicator_by_options::<4>(&inputs, &opts, None).unwrap();
...
=== "Go"
    ...
    **By assets** — same options applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    assets := [][indicators.AdxInputs][]float64{{h1, l1, c1}, {h2, l2, c2}, {h3, l3, c3}, {h4, l4, c4}}
    sim, _ := indicators.Adx.SimdByAssets(assets, []float64{14.0}, nil)
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    sim2, _ := indicators.Adx.SimdByOptions(high, low, close, [][]float64{{7}, {14}, {21}, {28}}, nil)
...
=== "Java"
    ...
    **By assets** — same options applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Adx.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    // (high/low/close series as in the Basic tab)
    try (SimdResult sim = Adx.simdByOptions(new double[][] {high, low, close},
