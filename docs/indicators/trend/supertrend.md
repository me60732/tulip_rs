...
=== "Java"
    ...
    double[] close = {81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36,
                      ...
                      88.80, 89.10, 88.70, 89.30, 89.70, 90.10, 89.50, 90.20, 90.80, 91.10,
                      90.50, 91.20, 91.80, 92.10, 91.50, 92.20, 92.80, 93.10, 92.50, 93.20};
    double[] options = {10.0, 3.0}; // period, step

    // Full computation — output rows are zero-copy views, valid until close().
    Outcome oc = Supertrend.indicator(new double[][] {high, low, close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // supertrend values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Supertrend.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued supertrend values
        }
    }
    ```
...
=== "Java"
    ...
    // ... (same high, low, close data as above)
    double[] options = {10.0, 3.0};
    boolean[] mask = {true, true, false}; // atr, tr

    Outcome oc = Supertrend.indicator(new double[][] {high, low, close}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // supertrend (primary)
...
=== "Go"
    ...
    **By assets** — same options applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    sim, _ := indicators.Supertrend.SimdByAssets(assets, options, nil)
    ...
    **By options** — same asset, 4 different option sets in parallel:
    ...
    optionSets := [][]float64{{7.0, 2.0}, {10.0, 3.0}, {14.0, 2.5}, {20.0, 3.0}}
    sim2, _ := indicators.Supertrend.SimdByOptions(highTiled, lowTiled, closeTiled, optionSets, nil)
    for i, lanes := range sim2.Results {
        fmt.Printf("Period/Step %v/%v: %s%n", optionSets[i][0], optionSets[i][1],
...
=== "Java"
    ...
    **By options** — same asset, 4 different option sets in parallel:
    ...
    double[][] optionSets = {{7.0, 2.0}, {10.0, 3.0}, {14.0, 2.5}, {20.0, 3.0}};
    try (SimdResult sim = Supertrend.simdByOptions(new double[][] {highTiled, lowTiled, closeTiled},
            optionSets, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
            System.out.printf("Period/Step %v/%v: %s%n", optionSets[i][0], optionSets[i][1],
                java.util.Arrays.toString(sim.toDoubleArray(i, 0)));
        }
    }
    ```
