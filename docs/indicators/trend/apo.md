...
=== "Java"
    ...
    Outcome oc = Apo.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // APO values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Apo.indicator(new double[][] {java.util.Arrays.copyOfRange(close, 0, n)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(new double[][] {java.util.Arrays.copyOfRange(close, n, 10)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued APO
        }
    }
    ```
...
=== "Java"
    ...
    `apo` exposes 2 optional outputs: `short_ema`, `long_ema`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    Outcome oc = Apo.indicator(new double[][] {close}, new double[] {5.0, 20.0}, mask);
    try (Result res = oc.result()) {
        // row 0 = apo (primary), row 1 = short_ema, row 2 = long_ema
...
=== "Java"
    ...
    **By assets** — same options applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Apo.simdByAssets(assets, new double[] {12.0, 26.0}, null)) {
    ...
    **By options** — same asset, 4 different option sets in parallel:
    ...
    // Tile the series so longer-period option sets have enough data (as in C tab)
    double[] close = {81.59, 81.06, 82.87, 83.00, 83.61,
                      83.15, 82.84, 83.99, 84.55, 84.36};
    double[] expanded = new double[close.length * 20];
    for (int i = 0; i < 20; i++) {
        System.arraycopy(close, 0, expanded, i * close.length, close.length);
    }

    try (SimdResult sim = Apo.simdByOptions(new double[][] {expanded},
