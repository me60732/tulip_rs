...
=== "Go"
    ...
    res, st, _ := indicators.Mom.Indicator(close, options, nil)
    fmt.Println(res.Rows[0]) // MOM(5) values
    res.Close()
    st.Close()

    // Partial computation + state continuation.
    res2, st2, _ := indicators.Mom.Indicator(close[:8], options, nil)
    res2.Close() // outputs consumed or closed; state stays live
    batch, _ := st2.Batch(close[8:], nil)
    fmt.Println(batch.Rows[0]) // continued MOM values
    batch.Close()
    st2.Close()
    ```
...
=== "Java"
    ...
    Outcome oc = Mom.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // MOM(5) values
    }

    // Partial computation + state continuation.
    Outcome p = Mom.indicator(
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued MOM values
        }
    }
    ```
...
=== "Go"
    ...
    **By assets** — same period applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    assets := [][indicators.MomInputs][]float64{{a1}, {a2}, {a3}, {a4}}
    sim, _ := indicators.Mom.SimdByAssets(assets, []float64{5.0}, nil)
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    close := []float64{81.59, 81.06, 82.87, 83.00, 83.61,
                       83.15, 82.84, 83.99, 84.55, 84.36}
    sim2, _ := indicators.Mom.SimdByOptions(close, [][]float64{{5}, {10}, {20}, {50}}, nil)
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    try (SimdResult sim = Mom.simdByAssets(assets, new double[] {5.0}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
            System.out.printf("Asset %d: %s%n", i + 1,
                java.util.Arrays.toString(sim.toDoubleArray(i, 0))); // MOM(5) values
        }
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Mom.simdByOptions(new double[][] {close},
            new double[][] {{5}, {10}, {20}, {50}}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
            System.out.printf("Period set %d: %s%n", i + 1,
                java.util.Arrays.toString(sim.toDoubleArray(i, 0))); // MOM values
        }
    }
    ```
