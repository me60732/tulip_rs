...
=== "Java"
    ...
    Outcome oc = Linreg.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // LinReg(14) values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Linreg.indicator(new double[][] {java.util.Arrays.copyOfRange(close, 0, n)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(new double[][] {java.util.Arrays.copyOfRange(close, n, 10)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued LinReg values
        }
    }
    ```
...
=== "Java"
    ...
    boolean[] mask = {true, true}; // one per optional output (linregslope, linregintercept)

    // Full computation with optional outputs.
    Outcome oc = Linreg.indicator(new double[][] {close}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // linreg (primary)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // linregslope (optional — requested)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(2))); // linregintercept (optional — requested)
    }
    oc.state().close();
    ```
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Linreg.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Linreg.simdByOptions(new double[][] {close},
