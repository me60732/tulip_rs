...
=== "Rust"
    ...
    let (outputs, mut state) = Roc::indicator(&[close.as_slice()], &[10.0], None).unwrap();
    println!("{:?}", outputs[0]);

    // State continuation — feed new bars without reprocessing history
    let partial = close[..8].to_vec();
    let (outputs2, mut state) = Roc::indicator(&[partial.as_slice()], &[10.0], None).unwrap();
    println!("{:?}", outputs2[0]);
...
=== "Java"
    ...
    Outcome oc = Roc.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // ROC(10) values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Roc.indicator(
        new double[][] {java.util.Arrays.copyOfRange(close, 0, n)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(
            new double[][] {java.util.Arrays.copyOfRange(close, n, 10)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued ROC values
        }
    }
    ```
...
=== "Rust"
    ...
    let (outputs, _state) = Roc::indicator(&[close.as_slice()], &[10.0], Some(&mask)).unwrap();
...
=== "C"
    ...
    bool optional_outputs[1] = {true}; /* request mom */

    CIndicatorResult r = roc_indicator(inputs, 10, options, optional_outputs, 1);
...
=== "Java"
    ...
    `roc` exposes 1 optional output: `mom`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    boolean[] mask = {true}; // mom
    Outcome oc = Roc.indicator(new double[][] {close}, new double[] {10.0}, mask);
    try (Result res = oc.result()) {
        // row 0 = roc (primary), row 1 = mom (optional — requested)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // mom
    }
    oc.state().close();
    ```
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    try (SimdResult sim = Roc.simdByAssets(assets, new double[] {10.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Roc.simdByOptions(new double[][] {close},
            new double[][] {{5}, {10}, {20}, {50}}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
