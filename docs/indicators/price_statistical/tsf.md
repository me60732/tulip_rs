...
=== "Go"
    ...
    res, st, _ := indicators.Tsf.Indicator(close, options, nil)
    fmt.Println(res.Rows[0]) // TSF(14) values
    res.Close()
    st.Close()

    // Partial computation + state continuation.
    res2, st2, _ := indicators.Tsf.Indicator(close[:8], options, nil)
    res2.Close() // outputs consumed or closed; state stays live
    batch, _ := st2.Batch(close[8:], nil)
    fmt.Println(batch.Rows[0]) // continued TSF values
    batch.Close()
    st2.Close()
    ```
...
=== "Java"
    ...
    Outcome oc = Tsf.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // TSF(14) values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Tsf.indicator(new double[][] {java.util.Arrays.copyOfRange(close, 0, n)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(new double[][] {java.util.Arrays.copyOfRange(close, n, 10)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued TSF values
        }
    }
    ```
...
=== "Go"
    ...
    mask := []bool{true, true, false} // linreg, linregslope, linregintercept
    res, st, _ := indicators.Tsf.Indicator(close, options, mask)
    fmt.Println(res.Rows[0])  // tsf (primary)
    fmt.Println(res.Rows[1])  // linreg (optional — requested)
    fmt.Println(res.Rows[2])  // linregslope (optional — requested)
    res.Close()
    st.Close()
    ```
...
=== "Java"
    ...
    `tsf` exposes 3 optional outputs: `linreg`, `linregslope`, `linregintercept`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    boolean[] mask = {true, true, false}; // linreg, linregslope, linregintercept
    Outcome oc = Tsf.indicator(new double[][] {close}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0)));  // tsf (primary)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1)));  // linreg (optional — requested)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(2)));  // linregslope (optional — requested)
    }
    oc.state().close();
    ```
...
=== "Go"
    ...
    **By assets** — same options applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    assets := [][indicators.TsfInputs][]float64{{a1}, {a2}, {a3}, {a4}}
    sim, _ := indicators.Tsf.SimdByAssets(assets, []float64{14.0}, nil)
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    sim2, _ := indicators.Tsf.SimdByOptions(close, [][]float64{{7.0}, {14.0}, {21.0}, {28.0}}, nil)
...
=== "Java"
    ...
    **By assets** — same options applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    try (SimdResult sim = Tsf.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Tsf.simdByOptions(new double[][] {close},
