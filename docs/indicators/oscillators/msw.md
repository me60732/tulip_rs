...
=== "C"
    ...
    double options[MSW_OPTIONS] = {10.0};

    /* Full computation (check r.error == C_INDICATOR_ERROR_OK in real code) */
    CIndicatorResult r = msw_indicator(inputs, 10, options, NULL, 0);
    /* r.outputs[0] -> MSW Sine, length r.output_lens[0] */
    /* r.outputs[1] -> MSW Lead, length r.output_lens[1] */
    tulip_ffi_result_free(r);
    msw_state_free(r.state);

    /* Partial computation + state continuation */
    CIndicatorResult p = msw_indicator(inputs, 8, options, NULL, 0);
    double new_close[] = {84.55, 84.36};
    const double *new_inputs[MSW_INPUTS] = {new_close};
    CBatchResult b = msw_batch(p.state, new_inputs, 2, NULL, 0);
    /* b.outputs[0] -> MSW Sine for just the two new bars */
    /* b.outputs[1] -> MSW Lead for just the two new bars */
...
=== "Java"
    ...
    Outcome oc = Msw.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // MSW Sine values
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // MSW Lead values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Msw.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued MSW Sine values
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(1))); // continued MSW Lead values
        }
    }
    ```
...
=== "C"
    ...
    **By assets** — same period applied to 4 assets in one call (N must be 2/4/8/16):
    ...
    const double *const asset1[MSW_INPUTS] = {a1};
    const double *const asset2[MSW_INPUTS] = {a2};
    const double *const asset3[MSW_INPUTS] = {a3};
    const double *const asset4[MSW_INPUTS] = {a4};
    const double *const *const simd_inputs[4] = {asset1, asset2, asset3, asset4};

    CSimdResult r = msw_simd_by_assets(simd_inputs, 4, 10, options, NULL, 0);
    for (uintptr_t i = 0; i < r.num_results; i++) {
        /* r.outputs[i][0] -> Sine */
        /* r.outputs[i][1] -> Lead */
        msw_state_free(r.states[i]);
    }
    ...
    **By options** — same asset, 4 different periods in one call:
    ...
    CSimdResult r = msw_simd_by_options(inputs, 10, simd_opts, 4, NULL, 0);
    /* r.outputs[i][0] -> Sine */
    /* r.outputs[i][1] -> Lead */
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Msw.simdByAssets(assets, new double[] {10.0}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
            System.out.printf("Asset %d Sine: %s%n", i + 1,
                java.util.Arrays.toString(sim.toDoubleArray(i, 0)));
            System.out.printf("Asset %d Lead: %s%n", i + 1,
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Msw.simdByOptions(new double[][] {close},
            new double[][] {{5}, {10}, {14}, {20}}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
            System.out.printf("Option set %d Sine: %s%n", i + 1,
                java.util.Arrays.toString(sim.toDoubleArray(i, 0)));
            System.out.printf("Option set %d Lead: %s%n", i + 1,
                java.util.Arrays.toString(sim.toDoubleArray(i, 1)));
        }
    }
    ```
