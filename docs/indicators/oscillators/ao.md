...
=== "Java"
    ...
    Outcome oc = Ao.indicator(new double[][] {high, low}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // AO values
    }

    // Partial computation + state continuation.
    Outcome p = Ao.indicator(new double[][] {
        java.util.Arrays.copyOfRange(high, 0, 30),
        java.util.Arrays.copyOfRange(low, 0, 30)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(new double[][] {
            java.util.Arrays.copyOfRange(high, 30, 35),
            java.util.Arrays.copyOfRange(low, 30, 35)});
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued AO values
        }
    }
    ```
...
=== "Java"
    ...
    double[] options = {};
    boolean[] mask = {true, false, false}; // short_sma, long_sma, medprice

    // Full computation with optional outputs — output rows are zero-copy views.
    Outcome oc = Ao.indicator(new double[][] {high, low}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // ao (primary)
...
=== "Java"
    ...
    **By assets** — applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    double[][][] assets = {{h1, l1}, {h1, l1}, {h1, l1}, {h1, l1}};
    try (SimdResult sim = Ao.simdByAssets(assets, options, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
            System.out.printf("Asset %d: %s%n", i + 1,
                java.util.Arrays.toString(sim.toDoubleArray(i, 0))); // AO values
        }
