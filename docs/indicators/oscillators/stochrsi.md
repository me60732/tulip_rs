...
=== "Java"
    ...
    Outcome oc = Stochrsi.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // StochRSI(14) values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Stochrsi.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued StochRSI values
        }
    }
    ```
...
=== "Java"
    ...
    `stochrsi` exposes 1 optional output: `rsi`. Pass a boolean mask as the third argument — one `boolean` per optional output.
    ...
    boolean[] mask = {true}; // request rsi
    Outcome oc = Stochrsi.indicator(new double[][] {close}, new double[] {14.0}, mask);
    try (Result res = oc.result()) {
        // row 0 = stochrsi (primary), row 1 = rsi (optional — requested)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // stochrsi
    }
    oc.state().close();
    ```
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Stochrsi.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Stochrsi.simdByOptions(new double[][] {close},
