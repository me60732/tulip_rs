...
=== "Go"
    **By options** — same asset, 4 different periods in parallel:
    ...
    const double *inputs[RSI_INPUTS] = {close_expanded};

    static const double o1[RSI_OPTIONS] = {7.0};
    static const double o2[RSI_OPTIONS] = {14.0};
    static const double o3[RSI_OPTIONS] = {21.0};
    static const double o4[RSI_OPTIONS] = {28.0};
    const double *const simd_opts[4] = {o1, o2, o3, o4};

    CSimdResult r = rsi_simd_by_options(inputs, EXPANDED_LEN, simd_opts, 4, NULL, 0);
    for (uintptr_t i = 0; i < r.num_results; i++) {
        rsi_state_free(r.states[i]);
    }
    tulip_ffi_simd_result_free(r);
...
=== "Go"
    ...
    **By assets** — same period applied to 4 assets in parallel (lane counts 2/4/8/16):
    ...
    assets := [][indicators.RsiInputs][]float64{{a1}, {a2}, {a3}, {a4}}
