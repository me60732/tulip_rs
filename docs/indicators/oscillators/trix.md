...
=== "Rust"
    ...
    ```rust
    use tulip_rs::indicators::trix::{Trix, TIndicatorState, Indicator};

    let close = vec![81.59, 81.06, 82.87, 83.00, 83.61,
                     83.15, 82.84, 83.99, 84.55, 84.36_f64];

    let (outputs, _state) = Trix::indicator(&[close.as_slice()], &[14.0], None).unwrap();
    println!("{:?}", outputs[0]);

    // State continuation
    let partial = close[..8].to_vec();
    let (outputs2, mut state) = Trix::indicator(&[partial.as_slice()], &[14.0], None).unwrap();
    println!("Partial TRIX: {:?}", outputs2[0]);
...
=== "Java"
    ...
    Outcome oc = Trix.indicator(new double[][] {close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        for (int i = 0; i < res.numOutputs(); i++) {
            System.out.println(java.util.Arrays.toString(res.toDoubleArray(i))); // TRIX values
        }
    }

    // Partial computation + state continuation.
    Outcome p = Trix.indicator(
        new double[][] {java.util.Arrays.copyOfRange(close, 0, 8)}, options);
    try (Result pr = p.result(); State st = p.state()) {
        Result br = st.batch(
            new double[][] {java.util.Arrays.copyOfRange(close, 8, 10)});
        try (br) {
            for (int i = 0; i < br.numOutputs(); i++) {
                System.out.println(java.util.Arrays.toString(br.toDoubleArray(i))); // continued TRIX values
            }
        }
    }
    ```
...
=== "Rust"
    ...
    let (outputs, _state) = Trix::indicator(&[close.as_slice()], &[5.0], Some(&mask)).unwrap();
...
=== "Java"
    ...
    boolean[] mask = {false, false, true}; // tema, dema, ema
    Outcome oc = Trix.indicator(new double[][] {close}, options, mask);
    try (Result res = oc.result()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // trix (primary)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // tema (optional — not requested here)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(2))); // dema (optional — not requested here)
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(3))); // ema (optional — requested)
    }
    oc.state().close();
    ```
...
=== "C"
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    const double *inputs[TRIX_INPUTS] = {close_expanded};

    static const double o1[TRIX_OPTIONS] = {9.0};
    static const double o2[TRIX_OPTIONS] = {14.0};
    static const double o3[TRIX_OPTIONS] = {21.0};
    static const double o4[TRIX_OPTIONS] = {30.0};
    const double *const simd_opts[4] = {o1, o2, o3, o4};

    CSimdResult r = trix_simd_by_options(inputs, EXPANDED_LEN, simd_opts, 4, NULL, 0);
    for (uintptr_t i = 0; i < r.num_results; i++) {
        trix_state_free(r.states[i]);
    }
    tulip_ffi_simd_result_free(r);
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Trix.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Trix.simdByOptions(new double[][] {close},
            new double[][] {{9}, {14}, {21}, {30}}, null)) {
        for (int i = 0; i < sim.numResults(); i++) {
