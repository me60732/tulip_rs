...
=== "Java"
    ...
    Outcome oc = Willr.indicator(new double[][] {high, low, close}, options);
    try (Result res = oc.result(); State st = oc.state()) {
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(0))); // Williams %R(14) values
    }

    // Partial computation + state continuation.
    int n = 8;
    Outcome p = Willr.indicator(new double[][] {
    ...
    try (Result pr = p.result(); State st = p.state()) {
        ...
        try (br) {
            System.out.println(java.util.Arrays.toString(br.toDoubleArray(0))); // continued Williams %R
        }
    }
    ```
...
=== "Java"
    ...
    `willr` exposes 2 optional outputs: `min`, `max`. Pass a boolean mask as the third argument — one `boolean` per optional output, in order.
    ...
    boolean[] mask = {true, true}; // min, max
    Outcome oc = Willr.indicator(new double[][] {high, low, close}, new double[] {14.0}, mask);
    try (Result res = oc.result()) {
        // row 0 = willr (primary), row 1 = min, row 2 = max
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(1))); // min
        System.out.println(java.util.Arrays.toString(res.toDoubleArray(2))); // max
    }
    oc.state().close();
    ```
...
=== "Java"
    ...
    **By assets** — same period applied to 4 assets in parallel (N must be 2, 4, 8, or 16):
    ...
    try (SimdResult sim = Willr.simdByAssets(assets, new double[] {14.0}, null)) {
    ...
    **By options** — same asset, 4 different periods in parallel:
    ...
    try (SimdResult sim = Willr.simdByOptions(new double[][] {h, l, c},
