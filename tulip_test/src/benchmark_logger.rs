...
impl BenchmarkLogger {
    pub async fn new() -> Result<Self, sqlx::Error> {
        let database_url = std::env::var("BENCHMARK_DATABASE_URL")
            .unwrap_or_else(|_| "postgresql://postgres:79Pveoz7#Beesh@192.168.50.10:5432/indicator_benchmark".to_string());
    ...
    pub async fn log_benchmark_result(&self, result: BenchmarkResult) -> Result<(), sqlx::Error> {
        ...
        let indicator_id = self.indicator_cache.get(&result.indicator_name)
            .copied()
            .unwrap_or_else(|| {
                eprintln!("Warning: Indicator '{}' not found in cache", result.indicator_name);
                -1
            });
        ...
        let options_json = serde_json::to_string(&result.options)
            .map_err(|e| sqlx::Error::TypeNotFound { type_name: format!("JSON serialization error: {}", e) })?;
    ...
    pub async fn calculate_performance_ratios(&self) -> Result<(), sqlx::Error> {
        ...
        println!("Calculated performance ratios for {} Rust and {} Rust_FromState results", 
                rust_rows.rows_affected(), rust_state_rows.rows_affected());
...
pub async fn get_global_logger() -> &'static mut BenchmarkLogger {
    unsafe {
        ...
        GLOBAL_LOGGER.as_mut().expect("Global logger not initialized")
    }
}
...
pub fn init_logging(benchmark_name: &str) {
    if should_log_to_db() {
        ...
        if LOGGER.get().is_none() {
            ...
            let logger = runtime.block_on(async {
                match BenchmarkLogger::new().await {
                    Ok(mut logger) => {
                        if let Err(e) = logger.start_benchmark_run(Some(&format!("Criterion {} Benchmarks", benchmark_name))).await {
        ...
        // Only set empty logger if not already set
        if LOGGER.get().is_none() {
            if let Err(_) = LOGGER.set(None) {
                // Already set by another thread, this is fine
            }
        }
    }
