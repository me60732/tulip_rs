...
mod tests {
    ...
    use tulip_rs::indicators::wilders::{Indicator, TIndicatorState, Wilders, IndicatorState, INPUTS, OPTIONS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Wilders::indicator(&first, &options, None).unwrap();
        let cfg = standard();
