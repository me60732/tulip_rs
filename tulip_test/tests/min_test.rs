...
mod tests {
    ...
    use tulip_rs::indicators::min::{Indicator, Min, TIndicatorState, IndicatorState, INPUTS, OPTIONS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Min::indicator(&first, &options, None).unwrap();
        let cfg = standard();
    ...
    fn test_min_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            let mut min_output_vec_c = vec![0.0_f64; output_len_c];
            let min_ptr: *mut f64 = min_output_vec_c.as_mut_ptr();
            let mut outputs_c: Vec<*mut f64> = vec![min_ptr];
            let ret = unsafe {
                ti_min(
                    close.len() as i32,
                    inputs_c.as_ptr(),
            ...
            for (i, (&c_val, &rust_val)) in min_output_vec_c
                .iter()
                .rev()
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust MIN has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust MIN has infinity at index {}: Rust = {}",
                        index, rust_val
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-10) {
                    ...
                    println!(
                        ...
                        index, min_output_vec_c, outputs[0], options
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust MIN has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-10) {
                        ...
                        /*println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, min_output_vec_c, outputs[0], options, stock_symbol
                        );*/
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
    ...
    fn test_min_simd_by_assets_vs_regular_database() {
        ...
        for options in OPTIONS_LIST {
            ...
            for (stock_idx, (stock_symbol, stock_close)) in stock_data.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD by assets MIN has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD by assets MIN has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
        ...
        println!("✓ All SIMD by assets vs Regular MIN database tests passed!");
    }

    #[test]
    fn test_min_database_state() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let close = get_close_array(stock_data);
            ...
            for options in OPTIONS_LIST {
                ...
                for close_chunk in close_chunks.by_ref() {
                    ...
                    let chunk_outputs = state
                        ...
                        .expect("MIN batch indicator failed");
                    ...
                    .enumerate()
                {
                    assert_eq!(
                        full_val, batch_val,
                        "Mismatch in MIN output at index {}: full = {}, batch = {}, Stock: {}, Options: {:?}",
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD MIN has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD MIN has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
