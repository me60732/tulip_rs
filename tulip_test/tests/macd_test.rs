...
mod tests {
    ...
    use tulip_rs::indicators::macd::{Indicator, IndicatorByOptions, Macd, TIndicatorState, INPUTS, OPTIONS, IndicatorState};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let len = close.len();
        let mid = len - 50;
        let options: [f64; OPTIONS] = [2.0, 5.0, 9.0]; // EMV has no options
        let first: [&[f64]; INPUTS] = [&close[..mid]];
        let second: [&[f64]; INPUTS] = [&close[mid..]];

        let (_rows, mut original) = Macd::indicator(&first, &options, None).unwrap();
        let cfg = standard();
