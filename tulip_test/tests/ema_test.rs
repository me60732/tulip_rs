...
mod tests {
    ...
    fn test_ema_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            // Determine the offset required by the C EMA function
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust EMA has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust EMA has infinity at index {}: Rust = {}, Options = {:?}",
                        index, rust_val, options
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = EPSILON) {
                    println!(
                        ...
                        index, ema_output_vec_c, outputs[0], options
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust EMA has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    if rust_val.is_infinite() {
                        panic!(
                            "Rust EMA has infinity at index {}: Rust = {}, Options = {:?}, Stock: {}",
                            index, rust_val, options, stock_symbol
                        );
                    }
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = EPSILON) {
                        println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, ema_output_vec_c, outputs[0], options, stock_symbol
                        );
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
                ...
                if close.len() <= min_data_val {
                    ...
                    for close_chunk in close_chunks.by_ref() {
                        ...
                        let chunk_outputs = state
                            ...
                            .expect("EMA batch indicator failed");
    ...
    fn test_ema_simd_by_options_vs_regular_database() {
        ...
        println!("✓ All SIMD vs Regular EMA database tests passed!");
    }
        ...
        for options in OPTIONS_LIST {
            ...
            for (stock_idx, (stock_symbol, stock_close)) in stock_data.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD by assets EMA has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD by assets EMA has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
    ...
    #[test]
    fn test_ema_state_bincode_roundtrip() {
        use bincode::config::standard;
        use bincode::serde::{decode_from_slice, encode_to_vec};

        // Generate enough data: EMA min_data = period + 1, so for period=12 we need at least 13
        let close: Vec<f64> = (0..400)
            .map(|i| 100.0 + 10.0 * ((i as f64) * 0.3).sin())
            .collect();
        let len = close.len();
        let mid = len - 50;
        let options: [f64; OPTIONS] = [12.0]; // one of the option sets used in existing tests
        let first: [&[f64]; INPUTS] = [&close[..mid]];
        let second: [&[f64]; INPUTS] = [&close[mid..]];

        let (_rows, mut original) = Ema::indicator(&first, &options, None).unwrap();
        let cfg = standard();
