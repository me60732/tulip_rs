...
mod tests {
    ...
    fn test_cmo_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            // Determine the offset required by the C CMO function
            ...
            let mut cmo_output_vec_c = vec![0.0_f64; output_len_c];
            let cmo_ptr: *mut f64 = cmo_output_vec_c.as_mut_ptr();
            let mut outputs_c: Vec<*mut f64> = vec![cmo_ptr];
            let ret = unsafe {
                ti_cmo(
                    close.len() as i32,
                    inputs_c.as_ptr(),
            ...
            for (i, (&c_val, &rust_val)) in cmo_output_vec_c
                .iter()
                .rev()
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust CMO has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust CMO has infinity at index {}: Rust = {}",
                        index, rust_val
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                    println!(
                        ...
                        index, cmo_output_vec_c, outputs[0], options
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust CMO has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-8) {
                        println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, cmo_output_vec_c, outputs[0], options, stock_symbol
                        );
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                if close.len() <= min_data_val {
                    ...
                    for close_chunk in close_chunks.by_ref() {
                        ...
                        let chunk_outputs = state
                            ...
                            .expect("CMO batch indicator failed");
                    ...
                    .enumerate()
                {
                    assert_eq!(
                        full_val, batch_val,
                        "CMO mismatch at index {}: full = {}, batch = {}, options = {:?}, stock = {}",
        ...
        for options in OPTIONS_LIST {
            ...
            for asset_idx in 0..4 {
                    ...
                    simd_output.iter().zip(regular_output.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD CMO has NaN at index {} for asset {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD CMO has infinity at index {} for asset {}: SIMD = {}, Options = {:?}",
                            i, asset_idx, simd_val, options
                        );
                    }
    ...
    #[test]
    fn test_cmo_simd_by_assets_database() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
            ...
            for options in OPTIONS_LIST {
                ...
                for (asset_idx, close_data) in padded_close.iter().enumerate().take(chunk.len()) {
                        ...
                        simd_output.iter().zip(regular_output.iter()).enumerate()
                    {
                        if simd_val.is_nan() {
                            panic!(
                                "SIMD CMO has NaN at index {} for asset {}: SIMD = {}",
                        ...
                        if simd_val.is_infinite() {
                            panic!(
                                "SIMD CMO has infinity at index {} for asset {}: SIMD = {}",
                                i, asset_idx, simd_val
                            );
                        }
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD CMO has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD CMO has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
