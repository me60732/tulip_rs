...
mod tests {
    ...
    use tulip_rs::indicators::stddev::{Indicator, StdDev, TIndicatorState, IndicatorState, OPTIONS, INPUTS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = StdDev::indicator(&first, &options, None).unwrap();
        let cfg = standard();
    ...
    fn test_stddev_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            // Determine the offset required by the C STDDEV function
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust STDDEV has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust STDDEV has infinity at index {}: Rust = {}, Options = {:?}",
                        index, rust_val, options
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = EPSILON) {
                    ...
                    /*println!(
                        ...
                        index, stddev_output_vec_c, outputs[0], options
                    );*/
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust STDDEV has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    // Skip if only C has NaN (C bug)
                    if c_val.is_nan() && !rust_val.is_nan() || c_val < 0.0 {
                        continue;
                    }
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-4) {
                        println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, stddev_output_vec_c, outputs[0], options, stock_symbol
                        );
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
            ...
            for options in OPTIONS_LIST {
                ...
                for close_chunk in close_chunks.by_ref() {
                    ...
                    let chunk_outputs = state
                        ...
                        .expect("STDDEV batch indicator failed");
    ...
    fn test_stddev_simd_vs_regular_database() {
            ...
            // Test without optional outputs
            {
                ...
                for (stock_idx, (stock_symbol, stock_close)) in stock_data.iter().enumerate() {
                        ...
                        simd_result.iter().zip(regular_result.iter()).enumerate()
                    {
                        ...
                        if simd_val.is_nan() {
                            panic!(
                                "SIMD by assets STDDEV has NaN at index {} for stock {} with options {:?}: SIMD = {}",
                        ...
                        if simd_val.is_infinite() {
                            panic!(
                                "SIMD by assets STDDEV has infinity at index {} for stock {} with options {:?}: SIMD = {}",
                                i, stock_symbol, options, simd_val
                            );
                        }

                        // Compare values with appropriate epsilon for STDDEV
        ...
        println!("✓ All SIMD by assets vs Regular STDDEV database tests passed!");
    }
    ...
    fn test_stddev_simd_vs_regular_database_optional_outputs() {
        ...
        println!("✓ All SIMD by assets vs Regular STDDEV optional outputs database tests passed!");
    }
    #[test]
    fn test_stddev_sma_optional_output_vs_c_tulip() {
        ...
        if rust_sma.is_empty() {
            panic!("Rust STDDEV sma optional output is empty - this indicates an indicator bug");
        }
        ...
        println!("Comparing STDDEV sma optional output vs C Tulip SMA:");
        println!(
        ...
        for (i, (rust_val, c_val)) in rust_sma.iter().rev().zip(c_sma.iter().rev()).enumerate() {
            ...
            if diff > EPSILON {
                panic!(
                    "STDDEV sma mismatch at reverse position {}: Rust = {:.12}, C = {:.12}, diff = {:.2e}",
                    i, rust_val, c_val, diff
                );
            }
        }

        println!("✓ STDDEV sma optional output matches C Tulip SMA output");
    }

    #[test]
    fn test_stddev_database_optional_sma() {
        const EPSILON: f64 = 1e-10;
            ...
            for &options in &OPTIONS_LIST {
                ...
                for i in 0..compare_len {
                    ...
                    assert!(
                        ...
                        "STDDEV SMA optional output mismatch at index {} (options {:?}): rust={}, c={}, diff={}",
                        rust_idx,
                        options,
                        rust_val,
                        c_val,
    ...
    fn test_stddev_simd_by_options_vs_regular_database() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD STDDEV has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD STDDEV has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
        ...
        println!("✓ All SIMD by options vs Regular STDDEV database tests passed!");
    }
    ...
    fn test_stddev_simd_by_options_vs_regular_database_optional_outputs() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                ...
                let simd_stddev_result = &all_simd_results[idx][0];
                let regular_stddev_result = &regular_results[0];

                let simd_sma_result = &all_simd_results[idx][1];
                let regular_sma_result = &regular_results[1];

                // Compare STDDEV output lengths
                assert_eq!(
                    simd_stddev_result.len(),
                    regular_stddev_result.len(),
                    "STDDEV output length mismatch for stock {} options {:?}: SIMD={}, Regular={}",
                    stock_symbol,
                    options,
                    simd_stddev_result.len(),
                    regular_stddev_result.len()
                ...
                for (i, (&simd_val, &regular_val)) in simd_stddev_result
                    .iter()
                    .zip(regular_stddev_result.iter())
                    .enumerate()
                {
                    // Check for NaN/infinity in SIMD result
                    if simd_val.is_nan() {
                    ...
                    if !approx_eq!(f64, simd_val, regular_val, epsilon = EPSILON) {
                        panic!(
                            "STDDEV mismatch at index {} for stock {} options {:?}: SIMD = {}, Regular = {}",
                            i, stock_symbol, options, simd_val, regular_val
                        );
                    }
                }
