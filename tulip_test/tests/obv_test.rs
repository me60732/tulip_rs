...
mod tests {
    ...
    fn test_obv_indicator() {
        ...
        // Determine the offset required by the C OBV function
        let start_index = unsafe { ti_obv_start(std::ptr::null()) };
        assert!(start_index >= 0, "ti_obv_start returned a negative index");
        let output_len_c = close.len() - (start_index as usize);

        // Run the C implementation
        let mut obv_output_vec_c = vec![0.0_f64; output_len_c];
        let obv_ptr: *mut f64 = obv_output_vec_c.as_mut_ptr();
        let mut outputs_c: Vec<*mut f64> = vec![obv_ptr];
        let ret = unsafe {
            ti_obv(
                close.len() as i32,
                inputs_c.as_ptr(),
        ...
        for (i, (&c_val, &rust_val)) in obv_output_vec_c
            .iter()
            .rev()
            ...
            .enumerate()
        {
            ...
            if rust_val.is_nan() {
                panic!("Rust OBV has NaN at index {}: Rust = {}", index, rust_val);
            }

            // Fail test if Rust has infinity
            if rust_val.is_infinite() {
                panic!(
                    "Rust OBV has infinity at index {}: Rust = {}",
                    index, rust_val
                );
            }
            ...
            if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                ...
                println!(
                    ...
                    index, obv_output_vec_c, outputs[0]
                );
                panic!(
    ...
    fn test_obv_database() {
        ...
        for (stock_symbol, stock_data) in data {
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust OBV has NaN at index {}: Rust = {}, Stock: {}",
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                    println!(
                        "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Stock: {}",
                        index, obv_output_vec_c, outputs[0], stock_symbol
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            while processed < close.len() {
                ...
                let chunk_outputs = state
                    ...
                    .expect("OBV batch indicator failed");
    ...
    fn test_obv_simd_by_assets() {
        ...
        for i in 0..4 {
                ...
                .enumerate()
            {
                ...
                if simd_val.is_nan() {
                    panic!(
                        "SIMD OBV has NaN at index {}: SIMD = {}, Stock: {}",
                ...
                if simd_val.is_infinite() {
                    panic!(
                        "SIMD OBV has infinity at index {}: SIMD = {}, Stock: {}",
                        j, simd_val, stock_data[i].0
                    );
                }
    ...
    #[test]
    fn test_obv_state_bincode_roundtrip() {
        ...
        let (mut restored_state, consumed): (ObvState, usize) =
