...
mod tests {
    ...
    use tulip_rs::indicators::mom::{Indicator, IndicatorByOptions, Mom, TIndicatorState, IndicatorState, INPUTS, OPTIONS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Mom::indicator(&first, &options, None).unwrap();
        let cfg = standard();
    ...
    fn test_mom_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            // Determine the offset required by the C MOM function
            let start_index = unsafe { ti_mom_start(options.as_ptr()) };
            assert!(start_index >= 0, "ti_mom_start returned a negative index");
            let output_len_c = close.len() - (start_index as usize);

            // Run the C implementation
            let mut mom_output_vec_c = vec![0.0_f64; output_len_c];
            let mom_ptr: *mut f64 = mom_output_vec_c.as_mut_ptr();
            let mut outputs_c: Vec<*mut f64> = vec![mom_ptr];
            let ret = unsafe {
                ti_mom(
                    close.len() as i32,
                    inputs_c.as_ptr(),
            ...
            for (i, (&c_val, &rust_val)) in mom_output_vec_c
                .iter()
                .rev()
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust MOM has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust MOM has infinity at index {}: Rust = {}",
                        index, rust_val
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                    ...
                    println!(
                        ...
                        index, mom_output_vec_c, outputs[0], options
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust MOM has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                        println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, mom_output_vec_c, outputs[0], options, stock_symbol
                        );
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
            ...
            for options in OPTIONS_LIST {
                ...
                for close_chunk in close_chunks.by_ref() {
                    ...
                    let chunk_outputs = state
                        ...
                        .expect("MOM batch indicator failed");
                    ...
                    .enumerate()
                {
                    assert_eq!(
                        full_val, batch_val,
                        "Mismatch in MOM output at index {}: full = {}, batch = {}, Stock: {}, Options: {:?}",
        ...
        for options in OPTIONS_LIST {
            ...
            for i in 0..4 {
                    ...
                    .enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD MOM has NaN at index {}: SIMD = {}, Options = {:?}, Stock: {}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD MOM has infinity at index {}: SIMD = {}, Options = {:?}, Stock: {}",
                            j, simd_val, options, stock_data[i].0
                        );
                    }
    ...
    #[test]
    fn test_mom_simd_by_options_vs_regular_database() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                ...
                assert_eq!(
                    simd_result.len(),
                    regular_result.len(),
                    "MOM output length mismatch for stock {} options {:?}: SIMD={}, Regular={}",
                    stock_symbol,
                    options,
                    simd_result.len(),
                    regular_result.len()
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD MOM has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD MOM has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
                    ...
                    if !approx_eq!(f64, simd_val, regular_val, epsilon = 1e-12) {
                        panic!(
                            "MOM mismatch at index {} for stock {} options {:?}: SIMD = {}, Regular = {}",
