...
mod tests {
    ...
    use tulip_rs::indicators::volatility::{Indicator, TIndicatorState, Volatility, INPUTS, OPTIONS, IndicatorState};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Volatility::indicator(&first, &options, None).unwrap();
        let cfg = standard();
