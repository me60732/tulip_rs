...
mod tests {
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Trima::indicator(&first, &options, None).unwrap();
        let cfg = standard();
    ...
    fn test_trima_indicator() {
        ...
        for options in OPTIONS_LIST {
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust TRIMA has NaN at index {}: Rust = {}, Options = {:?}",
            ...
            for options in OPTIONS_LIST {
                ...
                for close_chunk in close_chunks.by_ref() {
                    ...
                    let chunk_outputs = state
                        ...
                        .expect("TRIMA batch indicator failed");
    ...
    #[test]
    fn test_trima_simd_by_assets_vs_regular_database() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        ...
        for options in OPTIONS_LIST {
            ...
            for (stock_idx, (stock_symbol, stock_close)) in stock_data.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD by assets TRIMA has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD by assets TRIMA has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
    ...
    fn test_trima_simd_by_options_vs_regular_database() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            // Prepare option groups: 4 + 2 lanes (supported SIMD lane counts)
            let options_4 = [
                &OPTIONS_LIST[0],
                &OPTIONS_LIST[1],
                &OPTIONS_LIST[2],
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD TRIMA has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD TRIMA has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }

                    // Compare values with tolerance appropriate for TRIMA
