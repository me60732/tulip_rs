...
mod tests {
    ...
    use tulip_rs::indicators::ao::{
        Ao, Indicator, IndicatorState as AoState, TIndicatorState, INPUTS,
    };
    ...
    #[test]
    fn test_ao_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Ao::indicator(&first, &options, None).unwrap();
        let cfg = standard();
        let bytes = encode_to_vec(&original, cfg).expect("serialize");
        let (mut restored, consumed): (AoState, usize) =
