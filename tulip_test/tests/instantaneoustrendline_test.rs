...
mod tests {
    ...
    use tulip_rs::indicators::instantaneoustrendline::{
        IndicatorState, InstantaneousTrendline, INPUTS,
    };
    ...
    fn test_it_simd_by_assets_vs_scalar() {
        ...
        let (simd_results, _) = InstantaneousTrendline::indicator_by_assets::<4>(
            &inputs_4,
            &[],
            Some(&[true, true, true]),
        )
    ...
    fn test_it_simd_by_assets_state_continuity() {
        ...
        let (simd_first, mut states) = InstantaneousTrendline::indicator_by_assets::<4>(
            &inputs_first,
            &[],
            Some(&[true, true, true]),
        )
    ...
    #[test]
    fn test_instantaneoustrendline_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) =
            InstantaneousTrendline::indicator(&first, &options, None).unwrap();
        let cfg = standard();
