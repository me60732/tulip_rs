...
mod tests {
    ...
    fn test_avgprice_indicator() {
        ...
        // Determine the offset required by the C AVGPRICE function
        let start_index = unsafe { ti_avgprice_start(std::ptr::null()) };
        assert!(
            start_index >= 0,
        ...
        let mut avgprice_output_vec_c = vec![0.0_f64; output_len_c];
        let avgprice_ptr: *mut f64 = avgprice_output_vec_c.as_mut_ptr();
        let mut outputs_c: Vec<*mut f64> = vec![avgprice_ptr];
        let ret = unsafe {
            ti_avgprice(
                open.len() as i32,
                inputs_c.as_ptr(),
        ...
        for (i, (&c_val, &rust_val)) in avgprice_output_vec_c
            .iter()
            .rev()
            ...
            .enumerate()
        {
            ...
            if rust_val.is_nan() {
                panic!(
                    "Rust AVGPRICE has NaN at index {}: Rust = {}, Options = {:?}",
            ...
            if rust_val.is_infinite() {
                panic!(
                    "Rust AVGPRICE has infinity at index {}: Rust = {}",
                    index, rust_val
                );
            }
            ...
            if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                println!(
                    ...
                    index, avgprice_output_vec_c, outputs[0]
                );
                panic!(
    ...
    fn test_avgprice_database() {
        ...
        for (stock_symbol, stock_data) in data {
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust AVGPRICE has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                    println!(
                        "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Stock: {}",
                        index, avgprice_output_vec_c, outputs[0], stock_symbol
                    );
                    panic!(
    ...
    #[test]
    fn test_avgprice_database_state() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let (open, high, low, close) = get_ohlc_arrays(&stock_data);
            ...
            if open.len() <= min_data_val {
                    ...
                    .zip(close_chunks.by_ref())
                {
                    ...
                    let chunk_outputs = state
                        ...
                        .expect("AVGPRICE batch indicator failed");
        ...
        // Test without optional outputs (AVGPRICE doesn't have optional outputs)
        {
                ...
                stock_data.iter().enumerate()
            {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD by assets AVGPRICE has NaN at index {} for stock {} with options {:?}: SIMD = {}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD by assets AVGPRICE has infinity at index {} for stock {} with options {:?}: SIMD = {}",
                            i, stock_symbol, options, simd_val
                        );
                    }

                    // Compare values with appropriate epsilon for AVGPRICE
