...
mod tests {
    ...
    use tulip_rs::indicators::rocr::{Indicator, Rocr, IndicatorByOptions, IndicatorState, INPUTS, OPTIONS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Rocr::indicator(&first, &options, None).unwrap();
        let cfg = standard();
