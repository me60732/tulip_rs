...
mod tests {
    ...
    use tulip_rs::indicators::atr::{Atr, Indicator, IndicatorState, TIndicatorState, INPUTS};
    ...
    fn test_atr_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            // Determine the offset required by the C ATR function
            ...
            for (i, (&c_val, &rust_val)) in atr_output_vec_c
                .iter()
                .rev()
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust ATR has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust ATR has infinity at index {}: Rust = {}, Options = {:?}",
                        index, rust_val, options
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = EPSILON) {
                    println!(
                        ...
                        index, atr_output_vec_c, outputs[0], options
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust ATR has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    if rust_val.is_infinite() {
                        panic!(
                            "Rust ATR has infinity at index {}: Rust = {}, Options = {:?}, Stock: {}",
                            index, rust_val, options, stock_symbol
                        );
                    }
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = EPSILON) {
                        println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, atr_output_vec_c, outputs[0], options, stock_symbol
                        );
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
                ...
                if high.len() <= min_data_val {
                        ...
                        .zip(close_chunks.by_ref())
                    {
                        ...
                        let chunk_outputs = state
                            ...
                            .expect("ATR batch indicator failed");
    ...
    #[test]
    fn test_atr_tr_optional_output_vs_c_tulip() {
        // Test ATR's TR optional output against C Tulip's standalone TR implementation
        // This validates that the TR values calculated as part of ATR match the reference
        let (high, low, close) = expand_inputs();

        for options in OPTIONS_LIST {
            println!("Testing ATR TR optional output with options: {:?}", options);
            ...
            let rust_tr = &rust_outputs[1];

            // Run the C implementation for TR on the full dataset
            let inputs_c: Vec<*const f64> = vec![high.as_ptr(), low.as_ptr(), close.as_ptr()];

            // No options needed for TR - it takes no parameters
            ...
            // Use reverse processing pattern as specified in testing guide
            // This handles alignment differences gracefully by comparing from the end
            let compare_len = rust_tr.len().min(tr_output_vec_c.len());
            for i in 0..compare_len {
                let rust_idx = rust_tr.len() - 1 - i;
                let c_idx = tr_output_vec_c.len() - 1 - i;
                let rust_val = rust_tr[rust_idx];
                let c_val = tr_output_vec_c[c_idx];

                // Fail test if Rust has NaN
                if rust_val.is_nan() {
                    panic!(
                        "Rust TR has NaN at index {} (from end): Rust = {}, Options = {:?}",
                        i, rust_val, options
                    );
                }
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust TR has infinity at index {} (from end): Rust = {}, Options = {:?}",
                        i, rust_val, options
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = EPSILON) {
                    println!(
                        "TR test failed at index {} (from end): \nC = {:?}, \nRust = {:?}, Options = {:?}",
                        i, tr_output_vec_c, rust_tr, options
                    );
                    panic!(
                        "TR mismatch at index {} (from end): C = {}, Rust = {}, Options = {:?}",
                        i, c_val, rust_val, options
                    );
                }
            }
        }

        println!("ATR TR optional output test passed");
    }
    ...
    #[test]
    fn test_atr_database_optional_tr() {
        init_database_data();
        let data = get_all_stock_data().unwrap();

        for (_, stock_data) in data {
            ...
            for &options in &OPTIONS_LIST {
                ...
                for i in 0..compare_len {
                    ...
                    assert!(
                        approx_eq!(f64, rust_val, c_val, epsilon = TR_EPSILON),
                        "ATR TR optional output mismatch at index {} (options {:?}): rust={}, c={}, diff={}",
                        rust_idx,
                        options,
                        rust_val,
                        c_val,
    ...
    #[test]
    fn test_atr_simd_vs_regular_database() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
    ...
    #[test]
    fn test_atr_simd_vs_regular_database_optional_outputs() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        ...
        for options in &OPTIONS_LIST {
            let optional_outputs = Some([true, true].as_slice()); // Enable both ATR and TR outputs
    ...
    #[test]
    fn test_atr_simd_by_options_vs_regular_database() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        ...
        println!("✓ All SIMD by options vs Regular ATR database tests passed!");
    }

    #[test]
    fn test_atr_simd_by_options_vs_regular_database_optional_outputs() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                ...
                let regular_atr_result = &regular_results[0];

                let simd_tr_result = &all_simd_results[idx][1];
                let regular_tr_result = &regular_results[1];
    ...
    #[test]
    fn test_atr_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Atr::indicator(&first, &options, None).unwrap();
        let cfg = standard();
