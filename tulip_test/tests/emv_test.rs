...
mod tests {
    ...
    use tulip_rs::indicators::emv::{
        Emv, Indicator, IndicatorState, TIndicatorState, INPUTS, OPTIONS as EMV_OPTIONS,
    };
    ...
    #[test]
    fn test_emv_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Emv::indicator(&first, &options, None).unwrap();
        let cfg = standard();
