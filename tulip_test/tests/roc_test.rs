...
mod tests {
    ...
    use tulip_rs::indicators::roc::{Indicator, IndicatorByOptions, Roc, TIndicatorState, IndicatorState, OPTIONS, INPUTS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Roc::indicator(&first, &options, None).unwrap();
        let cfg = standard();
