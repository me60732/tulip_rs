...
mod tests {
    ...
    use tulip_rs::indicators::zlema::{indicator as rust_zlema, min_data, TIndicatorState};
    ...
    fn test_zlema_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            // Determine the offset required by the C ZLEMA function
            let start_index = unsafe { ti_zlema_start(options.as_ptr()) };
            assert!(start_index >= 0, "ti_zlema_start returned a negative index");
            let output_len_c = close.len() - (start_index as usize);

            // Run the C implementation
            let mut zlema_output_vec_c = vec![0.0_f64; output_len_c];
            let zlema_ptr: *mut f64 = zlema_output_vec_c.as_mut_ptr();
            let mut outputs_c: Vec<*mut f64> = vec![zlema_ptr];
            let ret = unsafe {
                ti_zlema(
                    close.len() as i32,
                    inputs_c.as_ptr(),
            ...
            let (outputs, _) =
                rust_zlema(&inputs_rust, &options, None).expect("Rust ZLEMA indicator failed");

            let output_len_rust = outputs[0].len();

            // Compare the outputs in reverse for the length of the Rust outputs
            for (i, (&c_val, &rust_val)) in zlema_output_vec_c
                .iter()
                .rev()
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust ZLEMA has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust ZLEMA has infinity at index {}: Rust = {}, Options = {:?}",
                        index, rust_val, options
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                    ...
                    println!(
                        ...
                        index, zlema_output_vec_c, outputs[0], options
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust ZLEMA has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                        println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, zlema_output_vec_c, outputs[0], options, stock_symbol
                        );
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                let (first_outputs, mut state) =
                    rust_zlema(&chunk_inputs, &options, None).expect("Rust ZLEMA indicator failed");
                ...
                for close_chunk in close_chunks.by_ref() {
                    ...
                    let chunk_outputs = state
                        ...
                        .expect("ZLEMA batch indicator failed");
    ...
    fn test_zlema_simd_vs_regular_database() {
        ...
        for options in OPTIONS_LIST {
            ...
            for (stock_idx, (stock_symbol, stock_close)) in stock_data.iter().enumerate() {
                ...
                let (regular_results, _) = rust_zlema(&stock_inputs, &options, None)
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD by assets ZLEMA has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD by assets ZLEMA has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
        ...
        println!("✓ All SIMD by assets vs Regular ZLEMA database tests passed!");
    }
    ...
    fn test_zlema_simd_by_options_vs_regular_database() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            all_simd_results.extend(simd_results_4);
            all_simd_results.extend(simd_results_2);

            // Compare each SIMD result with regular
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                let (regular_results, _) =
                    rust_zlema(&inputs, options, None).expect("Regular ZLEMA indicator failed");
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD ZLEMA has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD ZLEMA has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
                    if !approx_eq!(f64, simd_val, regular_val, epsilon = 1e-12) {
