...
mod tests {
    ...
    use tulip_rs::indicators::zlema::{Indicator, TIndicatorState, Zlema, IndicatorState, INPUTS, OPTIONS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Zlema::indicator(&first, &options, None).unwrap();
        let cfg = standard();
    ...
    fn test_zlema_database_state() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                let min_data_val = Zlema::min_data(&options).max(CHUNK_SIZE);
    ...
    fn test_zlema_simd_vs_regular_database() {
        ...
        for options in OPTIONS_LIST {
            ...
            for (stock_idx, (stock_symbol, stock_close)) in stock_data.iter().enumerate() {
                ...
                let (regular_results, _) = Zlema::indicator(&stock_inputs, &options, None)
