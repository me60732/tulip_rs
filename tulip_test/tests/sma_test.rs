...
mod tests {
    ...
    use tulip_rs::indicators::sma::{Indicator, IndicatorByOptions, Sma, TIndicatorState, IndicatorState, OPTIONS, INPUTS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Sma::indicator(&first, &options, None).unwrap();
        let cfg = standard();
    ...
    fn test_sma_indicator() {
        ...
        for options in OPTIONS_LIST {
            ...
            // Determine the offset required by the C SMA function
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust SMA has NaN at index {}: Rust = {}, Options = {:?}",
                ...
                if rust_val.is_infinite() {
                    panic!(
                        "Rust SMA has infinity at index {}: Rust = {}, Options = {:?}",
                        index, rust_val, options
                    );
                }
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-10) {
                    ...
                    println!(
                        ...
                        index, sma_output_vec_c, outputs[0], options
                    );
                    panic!(
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                    ...
                    .enumerate()
                {
                    ...
                    if rust_val.is_nan() {
                        panic!(
                            "Rust SMA has NaN at index {}: Rust = {}, Options = {:?}, Stock: {}",
                    ...
                    if rust_val.is_infinite() {
                        panic!(
                            "Rust SMA has infinity at index {}: Rust = {}, Options = {:?}, Stock: {}",
                            index, rust_val, options, stock_symbol
                        );
                    }
                    ...
                    if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-10) {
                        println!(
                            "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Options = {:?}, Stock: {}",
                            index, sma_output_vec_c, outputs[0], options, stock_symbol
                        );
                        panic!(
                            "Mismatch at index {}: C = {}, Rust = {}, Options = {:?}",
                            index, c_val, rust_val, options
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                if close.len() <= min_data_val {
                    ...
                    for close_chunk in close_chunks.by_ref() {
                        ...
                        let chunk_outputs = state
                            ...
                            .expect("SMA batch indicator failed");
                    ...
                    .enumerate()
                {
                    assert_eq!(
                        full_val, batch_val,
                        "SMA mismatch at index {}: full = {}, batch = {}, options = {:?}, stock = {}",
    ...
    #[test]
    fn test_sma_simd_by_assets_vs_regular_database() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        ...
        for options in OPTIONS_LIST {
            ...
            for (stock_idx, (stock_symbol, stock_close)) in stock_data.iter().enumerate() {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD by assets SMA has NaN at index {} for stock {}: SIMD = {}, Options = {:?}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD by assets SMA has infinity at index {} for stock {}: SIMD = {}, Options = {:?}",
                            i, stock_symbol, simd_val, options
                        );
                    }
