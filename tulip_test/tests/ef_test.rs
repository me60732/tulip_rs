...
mod tests {
    ...
    #[test]
    fn test_ef_state_bincode_roundtrip() {
        use bincode::config::standard;
        use bincode::serde::{decode_from_slice, encode_to_vec};

        // Generate enough data: EF min_data = period + 1, so for period=10 we need at least 11
        let close: Vec<f64> = (0..400)
            .map(|i| 100.0 + 10.0 * ((i as f64) * 0.3).sin())
            .collect();
        let len = close.len();
        let mid = len - 50;
        let options: [f64; OPTIONS] = [10.0]; // one of the option sets used in existing tests
        let first: [&[f64]; INPUTS] = [&close[..mid]];
        let second: [&[f64]; INPUTS] = [&close[mid..]];

        let (_rows, mut original) = Ef::indicator(&first, &options, None).unwrap();
        let cfg = standard();
