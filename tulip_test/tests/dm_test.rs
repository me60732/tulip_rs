...
mod tests {
    ...
    #[test]
    fn test_dm_state_bincode_roundtrip() {
        use bincode::config::standard;
        use bincode::serde::{decode_from_slice, encode_to_vec};

        let (high, low) = expand_inputs();
        // Use 50 elements for first segment to ensure enough data for min_data (period + 1 = 15)
        let mid = 50;
        ...
        let (_rows, mut original) = Dm::indicator(&first, &options, None).unwrap();
        let cfg = standard();
