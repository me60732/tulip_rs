...
mod tests {
    ...
    use tulip_rs::indicators::mass::{
        Indicator, IndicatorByOptions, IndicatorState, Mass, TIndicatorState, INPUTS,
    };
    ...
    fn test_mass_simd_state_handover_by_options() {
        ...
        println!("✓ All SIMD by-options state handover by MASS tests passed!");
    }

    #[test]
    fn test_mass_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Mass::indicator(&first, &options, None).unwrap();
