...
mod tests {
    ...
    fn test_typprice_indicator() {
        ...
        // Determine the offset required by the C TYPPRICE function
        let start_index = unsafe { ti_typprice_start(std::ptr::null()) };
        assert!(
            start_index >= 0,
        ...
        for (i, (&c_val, &rust_val)) in typprice_output_vec_c
            .iter()
            .rev()
            ...
            .enumerate()
        {
            ...
            if rust_val.is_nan() {
                panic!(
                    "Rust TYPPRICE has NaN at index {}: Rust = {}",
                    index, rust_val
                );
            }
            ...
            if rust_val.is_infinite() {
                panic!(
                    "Rust TYPPRICE has infinity at index {}: Rust = {}",
                    index, rust_val
                );
            }
            ...
            if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                ...
                println!(
                    ...
                    index, typprice_output_vec_c, outputs[0]
                );
                panic!(
    ...
    fn test_typprice_database() {
        ...
        for (stock_symbol, stock_data) in data {
                ...
                .enumerate()
            {
                ...
                if rust_val.is_nan() {
                    panic!(
                        "Rust TYPPRICE has NaN at index {}: Rust = {}, Stock: {}",
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                    println!(
                        "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Stock: {}",
                        index, typprice_output_vec_c, outputs[0], stock_symbol
                    );
                    panic!(
    ...
    #[test]
    fn test_typprice_database_state() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let (high, low, close) = get_hlc_arrays(stock_data);
            ...
            while chunks_processed * CHUNK_SIZE < remaining_len {
                ...
                let chunk_outputs = state
                    ...
                    .expect("TYPPRICE batch indicator failed");
        ...
        // Test without optional outputs (TYPPRICE doesn't have optional outputs)
        {
                ...
                stock_data.iter().enumerate()
            {
                    ...
                    simd_result.iter().zip(regular_result.iter()).enumerate()
                {
                    ...
                    if simd_val.is_nan() {
                        panic!(
                            "SIMD by assets TYPPRICE has NaN at index {} for stock {} with options {:?}: SIMD = {}",
                    ...
                    if simd_val.is_infinite() {
                        panic!(
                            "SIMD by assets TYPPRICE has infinity at index {} for stock {} with options {:?}: SIMD = {}",
                            i, stock_symbol, options, simd_val
                        );
                    }

                    // Compare values with appropriate epsilon for TYPPRICE
    ...
    #[test]
    fn test_typprice_state_bincode_roundtrip() {
