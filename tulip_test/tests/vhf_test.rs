...
mod tests {
    ...
    use tulip_rs::indicators::vhf::{Indicator, TIndicatorState, Vhf, INPUTS, OPTIONS, IndicatorState};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Vhf::indicator(&first, &options, None).unwrap();
        let cfg = standard();
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for (idx, options) in OPTIONS_LIST.iter().enumerate() {
                ...
                let (regular_results, _) =
                    Vhf::indicator(&inputs, options, None).expect("Regular VHF indicator failed");
