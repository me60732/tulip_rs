...
mod tests {
    ...
    use tulip_rs::indicators::adxr::{Adxr, INPUTS, OPTIONS};
    ...
    #[test]
    fn test_adxr_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Adxr::indicator(&first, &options, None).unwrap();
        let cfg = bincode::config::standard();
        let bytes = bincode::serde::encode_to_vec(&original, cfg).expect("serialize");
        let (mut restored, consumed): (
            <Adxr as Indicator<INPUTS, OPTIONS>>::IndicatorState,
            usize,
    ...
    fn test_adxr_adx_optional_output_vs_c_tulip_database() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                let adx_inputs_c = [high.as_ptr(), low.as_ptr(), close.as_ptr()];
