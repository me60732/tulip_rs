...
mod tests {
    ...
    use tulip_rs::indicators::ccfisher::{CcFisher, IndicatorState, INPUTS};
    ...
    #[test]
    fn test_ccfisher_state_bincode_roundtrip() {
        use bincode::config::standard;
        use bincode::serde::{decode_from_slice, encode_to_vec};

        // Oscillating + mild drift series (DB-free): exercises the
        // cybercycle/HL2 filter math without degenerate constant input.
        ...
        let (_rows, mut original) = CcFisher::indicator(&first, &options, None).unwrap();
