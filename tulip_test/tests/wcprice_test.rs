...
mod tests {
    ...
    use tulip_rs::indicators::wcprice::{indicator as rust_wcprice, min_data, TIndicatorState};
    ...
    fn test_wcprice_indicator() {
        ...
        // Determine the offset required by the C WCPRICE function
        let start_index = unsafe { ti_wcprice_start(std::ptr::null()) };
        assert!(
            start_index >= 0,
            "ti_wcprice_start returned a negative index"
        );
        let output_len_c = high.len() - (start_index as usize);

        // Run the C implementation
        let mut wcprice_output_vec_c = vec![0.0_f64; output_len_c];
        let wcprice_ptr: *mut f64 = wcprice_output_vec_c.as_mut_ptr();
        let mut outputs_c: Vec<*mut f64> = vec![wcprice_ptr];
        let ret = unsafe {
            ti_wcprice(
                high.len() as i32,
                inputs_c.as_ptr(),
        ...
        assert_eq!(ret, 0, "ti_wcprice returned error code {}", ret);

        // Run the Rust implementation
        let inputs_rust = [high.as_slice(), low.as_slice(), close.as_slice()];
        let (outputs, _) =
            rust_wcprice(&inputs_rust, &[], None).expect("Rust WCPRICE indicator failed");

        let output_len_rust = outputs[0].len();

        // Compare the outputs in reverse for the length of the Rust outputs
        for (i, (&c_val, &rust_val)) in wcprice_output_vec_c
            .iter()
            .rev()
            ...
            .enumerate()
        {
            ...
            if rust_val.is_nan() {
                panic!(
                    "Rust WCPRICE has NaN at index {}: Rust = {}",
                    index, rust_val
                );
            }
            ...
            if rust_val.is_infinite() {
                panic!(
                    "Rust WCPRICE has infinity at index {}: Rust = {}",
                    index, rust_val
                );
            }
            ...
            if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                ...
                println!(
                    ...
                    index, wcprice_output_vec_c, outputs[0]
                );
                panic!(
        ...
        for (stock_symbol, stock_data) in data {
                ...
                .enumerate()
            {
                ...
                if !approx_eq!(f64, c_val, rust_val, epsilon = 1e-12) {
                    println!(
                        "Test failed at index {}: \nC = {:?}, \n\nRust = {:?}, Stock: {}",
                        index, wcprice_output_vec_c, outputs[0], stock_symbol
                    );
                    panic!(
    ...
    #[test]
    fn test_wcprice_database_state() {
        init_database_data();
        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let (high, low, close) = get_hlc_arrays(stock_data);
            ...
            let (full_outputs, _) =
                rust_wcprice(&inputs_rust, &options, None).expect("Rust WCPRICE indicator failed");
            ...
            let (first_outputs, mut state) =
                rust_wcprice(&chunk_inputs, &options, None).expect("Rust WCPRICE indicator failed");
            ...
            while chunks_processed * CHUNK_SIZE < remaining_len {
                ...
                let chunk_outputs = state
                    ...
                    .expect("WCPRICE batch indicator failed");
    ...
    fn test_wcprice_simd_vs_regular_database() {
            ...
            stock_data.iter().enumerate()
        {
            ...
            let (regular_results, _) = rust_wcprice(&stock_inputs, &options, None)
                .expect("Regular WCPRICE indicator failed");
                ...
                simd_result.iter().zip(regular_result.iter()).enumerate()
            {
                ...
                if simd_val.is_nan() {
                    panic!(
                        "SIMD by assets WCPRICE has NaN at index {} for stock {} with options {:?}: SIMD = {}",
                ...
                if simd_val.is_infinite() {
                    panic!(
                        "SIMD by assets WCPRICE has infinity at index {} for stock {} with options {:?}: SIMD = {}",
                        i, stock_symbol, options, simd_val
                    );
                }

                // Compare values with appropriate epsilon for WCPRICE
