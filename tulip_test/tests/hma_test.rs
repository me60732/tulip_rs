...
mod tests {
    ...
    use tulip_rs::indicators::hma::{Hma, Indicator, IndicatorByOptions, TIndicatorState, INPUTS, OPTIONS, IndicatorState};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Hma::indicator(&first, &options, None).unwrap();
        let cfg = standard();
