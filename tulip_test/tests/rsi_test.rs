...
mod tests {
    ...
    use tulip_rs::indicators::rsi::{Indicator, IndicatorByOptions, Rsi, TIndicatorState, IndicatorState, INPUTS, OPTIONS};
    ...
    fn test_state_bincode_roundtrip() {
        ...
        let (_rows, mut original) = Rsi::indicator(&first, &options, None).unwrap();
        let cfg = standard();
