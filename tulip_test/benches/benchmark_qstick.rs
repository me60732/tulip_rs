use criterion::{black_box, criterion_group, criterion_main, Criterion};
use tulip_rs::indicators::qstick::{
    indicator, indicator_by_assets, indicator_by_options, min_data, IndicatorState, TIndicatorState,
};
use tulip_test::benchmark_logger::{init_logging, log_timing_result, should_log_to_db};
use tulip_test::benchmark_utils::SAMPLE_SIZE;
use tulip_test::c_bindings::{ti_qstick, ti_qstick_start};
use tulip_test::criterion_logger::TimingMeasurements;
use tulip_test::database::{get_all_stock_data, init_database_data};

// Test input data (open and close prices) - copied from test file
const OPEN: [f64; 15] = [
    81.85, 81.20, 81.55, 82.91, 83.10, 83.41, 82.71, 82.70, 84.20, 84.25, 84.03, 85.45, 86.18,
    88.00, 87.60,
];
const CLOSE: [f64; 15] = [
    81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36, 85.53, 86.54, 86.89,
    87.77, 87.29,
];

// Options for QSTICK - copied from test file
const OPTIONS_LIST: [[f64; 1]; 4] = [[5.0], [2.0], [8.0], [14.0]];

/// Chunk size for from-state benchmarks
const CHUNK_SIZE: usize = 100;

/// Expand the sample input data by repeating it for profiling
fn expand_inputs() -> (Vec<f64>, Vec<f64>) {
    let mut open_vec = OPEN.to_vec();
    let mut close_vec = CLOSE.to_vec();
    for _ in 0..500 {
        open_vec.extend_from_slice(&OPEN);
        close_vec.extend_from_slice(&CLOSE);
    }
    (open_vec, close_vec)
}

// Helper function to get OC arrays from stock data
fn get_oc_arrays(stock_data: &[tulip_test::database::EodData]) -> (Vec<f64>, Vec<f64>) {
    let open: Vec<f64> = stock_data.iter().map(|d| d.open).collect();
    let close: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
    (open, close)
}

fn bench_c_qstick(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("qstick");

        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let (open, close) = get_oc_arrays(stock_data);
            let n = open.len();
            let inputs: Vec<*const f64> = vec![open.as_ptr(), close.as_ptr()];

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let start_index = unsafe { ti_qstick_start(options.as_ptr()) };
                        let output_len = open.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];

                        let ret = unsafe {
                            ti_qstick(
                                open.len() as i32,
                                inputs.as_ptr(),
                                options.as_ptr(),
                                outputs.as_mut_ptr(),
                            )
                        };
                        assert_eq!(ret, 0, "ti_qstick returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result(
                    "qstick",
                    "C_tulip",
                    &options,
                    n,
                    &timing,
                    Some(stock_symbol),
                );
            }
        }
    } else {
        let (open_vec, close_vec) = expand_inputs();
        let inputs: Vec<*const f64> = vec![open_vec.as_ptr(), close_vec.as_ptr()];

        for options in OPTIONS_LIST {
            let start_index = unsafe { ti_qstick_start(options.as_ptr()) };
            let output_len = open_vec.len() - (start_index as usize);

            let mut group = c.benchmark_group(format!("C QSTICK {{ {:.1} }}", options[0]));
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("C QSTICK {{ {:.1} }}", options[0]), |b| {
                b.iter(|| {
                    let mut output_vec = vec![0.0_f64; output_len];
                    let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];

                    let ret = unsafe {
                        ti_qstick(
                            open_vec.len() as i32,
                            inputs.as_ptr(),
                            options.as_ptr(),
                            outputs.as_mut_ptr(),
                        )
                    };
                    assert_eq!(ret, 0, "ti_qstick returned error code {}", ret);
                    black_box(&output_vec);
                });
            });
            group.finish();
        }
    }
}

fn bench_rust_qstick(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("qstick");

        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let (open, close) = get_oc_arrays(stock_data);
            let n = open.len();
            let inputs = [open.as_slice(), close.as_slice()];

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let result =
                            indicator(&inputs, &options, None).expect("QSTICK indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result("qstick", "Rust", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        let (open_vec, close_vec) = expand_inputs();
        let inputs = [open_vec.as_slice(), close_vec.as_slice()];

        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group(format!("Rust QSTICK {{ {:.1} }}", options[0]));
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Rust QSTICK {{ {:.1} }}", options[0]), |b| {
                b.iter(|| {
                    let result =
                        indicator(&inputs, &options, None).expect("QSTICK indicator failed");
                    black_box(&result);
                });
            });
            group.finish();
        }
    }
}

fn bench_rust_qstick_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("qstick");

        let data = get_all_stock_data().unwrap();

        // Get first 4 stocks' data
        let stock_data: Vec<(String, Vec<f64>, Vec<f64>)> = data
            .iter()
            .take(4)
            .map(|(symbol, data)| {
                let (open, close) = get_oc_arrays(data);
                (symbol.clone(), open, close)
            })
            .collect();

        // Prepare inputs in the format expected by indicator_by_assets
        let inputs: [&[&[f64]; 2]; 4] = [
            &[&stock_data[0].1, &stock_data[0].2], // open, close
            &[&stock_data[1].1, &stock_data[1].2], // open, close
            &[&stock_data[2].1, &stock_data[2].2], // open, close
            &[&stock_data[3].1, &stock_data[3].2], // open, close
        ];

        for options in OPTIONS_LIST {
            let mut timing = TimingMeasurements::new();
            timing.measure(
                || {
                    let result = indicator_by_assets::<4>(&inputs, &options, None)
                        .expect("Rust SIMD by assets QSTICK indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );

            log_timing_result(
                "qstick",
                "Rust_SIMD_by_assets",
                &options,
                stock_data[0].1.len(),
                &timing,
                Some("4_Assets"),
            );
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (open_vec, close_vec) = expand_inputs();

        // Create 4 identical datasets for SIMD processing
        let inputs: [&[&[f64]; 2]; 4] = [
            &[&open_vec, &close_vec],
            &[&open_vec, &close_vec],
            &[&open_vec, &close_vec],
            &[&open_vec, &close_vec],
        ];

        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("qstick_rust_simd_by_assets");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(
                format!("Rust SIMD by assets QSTICK {{ {:.1} }}", options[0]),
                |b| {
                    b.iter(|| {
                        let result = indicator_by_assets::<4>(&inputs, &options, None)
                            .expect("Rust SIMD by assets QSTICK indicator failed");
                        black_box(&result);
                    });
                },
            );
            group.finish();
        }
    }
}

fn bench_rust_qstick_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("qstick");

        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let (open, close) = get_oc_arrays(stock_data);
            let n = open.len();

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let min_data = min_data(&options).max(CHUNK_SIZE);
                        // First chunk
                        let chunk_inputs = [&open[..min_data], &close[..min_data]];

                        let (_, mut state) = indicator(&chunk_inputs, &options, None)
                            .expect("QSTICK indicator failed");

                        // Chunks
                        let mut open_chunks = open[min_data..].chunks_exact(CHUNK_SIZE);
                        let mut close_chunks = close[min_data..].chunks_exact(CHUNK_SIZE);

                        for (open_chunk, close_chunk) in
                            open_chunks.by_ref().zip(close_chunks.by_ref())
                        {
                            let result = state.batch_indicator(&[open_chunk, close_chunk], None);
                            black_box(&result);
                        }

                        // Remainder
                        let open_rem = open_chunks.remainder();
                        let close_rem = close_chunks.remainder();

                        if !open_rem.is_empty() && !close_rem.is_empty() {
                            let open_rem_vec = open_rem.to_vec();
                            let close_rem_vec = close_rem.to_vec();
                            let result =
                                state.batch_indicator(&[&open_rem_vec, &close_rem_vec], None);
                            black_box(&result);
                        }
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result(
                    "qstick",
                    "Rust_FromState",
                    &options,
                    n,
                    &timing,
                    Some(stock_symbol),
                );

                // --- Rust_FromState_1_Bar benchmark ---
                if open.len() > 1 {
                    let new_open_vec = open[..open.len() - 1].to_vec();
                    let new_close_vec = close[..close.len() - 1].to_vec();
                    let new_inputs = [new_open_vec.as_slice(), new_close_vec.as_slice()];

                    let final_open_vec = open[open.len() - 1..].to_vec();
                    let final_close_vec = close[close.len() - 1..].to_vec();
                    let (_, mut state) = indicator(&new_inputs, &options, None)
                        .expect("Rust QSTICK indicator failed");

                    let mut timing = TimingMeasurements::new();
                    timing.measure(
                        || {
                            let result = state
                                .batch_indicator(
                                    &[final_open_vec.as_slice(), final_close_vec.as_slice()],
                                    None,
                                )
                                .expect("Rust QSTICK from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );

                    log_timing_result(
                        "qstick",
                        "Rust_FromState_1_Bar",
                        &options,
                        n,
                        &timing,
                        Some(stock_symbol),
                    );

                    // --- Rust_FromState_1_Bar_json benchmark ---
                    let (_, state) = indicator(&new_inputs, &options, None)
                        .expect("Rust QSTICK indicator failed");
                    let json = serde_json::to_string(&state).expect("json failed");

                    let mut timing = TimingMeasurements::new();
                    timing.measure(
                        || {
                            let mut state: IndicatorState =
                                serde_json::from_str(&json).expect("JSON failed");
                            let result = state
                                .batch_indicator(
                                    &[final_open_vec.as_slice(), final_close_vec.as_slice()],
                                    None,
                                )
                                .expect("Rust QSTICK from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );

                    log_timing_result(
                        "qstick",
                        "Rust_FromState_1_Bar_json",
                        &options,
                        n,
                        &timing,
                        Some(stock_symbol),
                    );
                }
            }
        }
    } else {
        // Criterion profiling mode - benchmark synthetic data
        let (open_vec, close_vec) = expand_inputs();
        let _inputs = [&open_vec, &close_vec];

        for options in OPTIONS_LIST {
            let mut group =
                c.benchmark_group(format!("Rust QSTICK from state {{ {:.1} }}", options[0]));
            group.sample_size(SAMPLE_SIZE);

            group.bench_function("benchmark", |b| {
                b.iter(|| {
                    let min_data = min_data(&options).max(CHUNK_SIZE);
                    // First chunk
                    let chunk_inputs = [&open_vec[..min_data], &close_vec[..min_data]];

                    let (_, mut state) =
                        indicator(&chunk_inputs, &options, None).expect("QSTICK indicator failed");

                    // Chunks
                    let mut open_chunks = open_vec[min_data..].chunks_exact(CHUNK_SIZE);
                    let mut close_chunks = close_vec[min_data..].chunks_exact(CHUNK_SIZE);

                    for (open_chunk, close_chunk) in open_chunks.by_ref().zip(close_chunks.by_ref())
                    {
                        let result = state.batch_indicator(&[open_chunk, close_chunk], None);
                        black_box(&result);
                    }

                    // Remainder
                    let open_rem = open_chunks.remainder();
                    let close_rem = close_chunks.remainder();

                    if !open_rem.is_empty() && !close_rem.is_empty() {
                        let result = state.batch_indicator(&[open_rem, close_rem], None);
                        black_box(&result);
                    }
                });
            });
            group.finish();
        }

        let new_open_vec = open_vec[..open_vec.len() - 1].to_vec();
        let new_close_vec = close_vec[..close_vec.len() - 1].to_vec();
        let new_inputs = [new_open_vec.as_slice(), new_close_vec.as_slice()];

        let final_open_vec = open_vec[open_vec.len() - 1..].to_vec();
        let final_close_vec = close_vec[close_vec.len() - 1..].to_vec();

        for options in OPTIONS_LIST {
            // Benchmark with 1 bar from state
            let mut group = c.benchmark_group(format!(
                "Rust QSTICK from state 1 bar {{ {:.1} }}",
                options[0]
            ));
            group.sample_size(SAMPLE_SIZE);
            group.bench_function("benchmark", |b| {
                let (_, mut state) =
                    indicator(&new_inputs, &options, None).expect("Rust QSTICK indicator failed");
                b.iter(|| {
                    let result = state
                        .batch_indicator(
                            &[final_open_vec.as_slice(), final_close_vec.as_slice()],
                            None,
                        )
                        .expect("Rust QSTICK from state indicator failed");
                    black_box(&result);
                });
            });
            group.finish();

            // Benchmark with 1 bar from state using JSON serialization
            let mut group = c.benchmark_group(format!(
                "Rust QSTICK from state 1 bar JSON {{ {:.1} }}",
                options[0]
            ));
            group.sample_size(SAMPLE_SIZE);
            group.bench_function("benchmark", |b| {
                let (_, state) =
                    indicator(&new_inputs, &options, None).expect("Rust QSTICK indicator failed");
                let json = serde_json::to_string(&state).expect("json failed");
                b.iter(|| {
                    let mut state: IndicatorState =
                        serde_json::from_str(&json).expect("JSON failed");
                    let result = state
                        .batch_indicator(
                            &[final_open_vec.as_slice(), final_close_vec.as_slice()],
                            None,
                        )
                        .expect("Rust QSTICK from state indicator failed");
                    black_box(&result);
                });
            });
            group.finish();
        }
    }
}

fn bench_rust_qstick_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("qstick");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let open_vec: Vec<f64> = stock_data.iter().map(|d| d.open).collect();
            let close_vec: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let n = open_vec.len();
            let inputs = [open_vec.as_slice(), close_vec.as_slice()];

            let mut timing = TimingMeasurements::new();
            timing.measure(
                || {
                    // Process all 4 options with 4-wide SIMD
                    let options_4 = [
                        &OPTIONS_LIST[0],
                        &OPTIONS_LIST[1],
                        &OPTIONS_LIST[2],
                        &OPTIONS_LIST[3],
                    ];
                    let result = indicator_by_options::<4>(&inputs, &options_4, None)
                        .expect("Rust SIMD QSTICK indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );

            log_timing_result(
                "qstick",
                "Rust_SIMD",
                &[0.0],
                n,
                &timing,
                Some(stock_symbol),
            );
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (open_vec, close_vec) = expand_inputs();
        let inputs = [open_vec.as_slice(), close_vec.as_slice()];

        let mut group = c.benchmark_group("qstick_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options QSTICK (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
                    &OPTIONS_LIST[1],
                    &OPTIONS_LIST[2],
                    &OPTIONS_LIST[3],
                ];
                let result = indicator_by_options::<4>(&inputs, &options_4, None)
                    .expect("Rust SIMD QSTICK indicator failed");
                black_box(&result);
            });
        });
        group.finish();
    }
}

criterion_group!(
    qstick_benchmarks,
    bench_rust_qstick_simd_by_options,
    bench_rust_qstick_simd_by_assets,
    bench_rust_qstick,
    bench_c_qstick,
    bench_rust_qstick_from_state,
);
criterion_main!(qstick_benchmarks);
