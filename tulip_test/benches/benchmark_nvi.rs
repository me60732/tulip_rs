...
use tulip_rs::indicators::nvi::{Indicator, IndicatorState, Nvi, TIndicatorState};
...
// Sample input data (close, volume)
const CLOSE: [f64; 15] = [
    81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36, 85.53, 86.54, 86.89,
    87.32, 87.29,
];
const VOLUME: [f64; 15] = [
    1000.0, 1100.0, 1200.0, 1300.0, 1400.0, 1500.0, 1600.0, 1700.0, 1800.0, 1900.0, 2000.0, 2100.0,
    2200.0, 2300.0, 2400.0,
];

// Options for NVI (no options)
const OPTIONS: [f64; 0] = [];
...
/// Benchmark the C implementation of NVI.
fn bench_c_nvi(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("nvi");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let start_index = unsafe { ti_nvi_start(OPTIONS.as_ptr()) };
        ...
        let mut group = c.benchmark_group("nvi_c");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("C NVI", |b| {
            b.iter(|| {
                let mut output_vec = vec![0.0_f64; output_len];
                let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust implementation of NVI.
fn bench_rust_nvi(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("nvi");
        ...
        let mut group = c.benchmark_group("nvi_rust");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust from_state implementation of NVI.
fn bench_rust_nvi_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("nvi");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    let (_, mut state) = Nvi::indicator(&chunk_inputs, &OPTIONS, None)
                        .expect("NVI indicator failed");
                ...
                timing.measure(
                    || {
                        let result = state
                            ...
                            .expect("Rust NVI from state indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        let mut group = c.benchmark_group("Rust NVI from state");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust SIMD by assets implementation of NVI.
fn bench_rust_nvi_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("nvi");
        ...
        timing.measure(
            || {
                let result = Nvi::indicator_by_assets::<4>(&inputs, &OPTIONS, None)
                    .expect("Rust SIMD by assets NVI indicator failed");
                black_box(&result);
            },
            SAMPLE_SIZE,
        );
        ...
        let mut group = c.benchmark_group("nvi_rust_simd_by_assets");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by assets NVI", |b| {
            b.iter(|| {
                let result = Nvi::indicator_by_assets::<4>(&inputs, &OPTIONS, None)
                    .expect("Rust SIMD by assets NVI indicator failed");
                black_box(&result);
