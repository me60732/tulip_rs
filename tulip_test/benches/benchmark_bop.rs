...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_bop, ta_bop_start};

// Sample input data from bop_test.rs
const OPEN: [f64; 15] = [
...
// Options for BOP (no options needed)
const OPTIONS: [f64; 0] = [];
...
// Helper function to get OHLC arrays from stock data
fn get_ohlc_arrays(
...
/// Benchmark the C implementation of BOP.
fn bench_c_bop(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("bop");
        ...
        let mut group = c.benchmark_group("bop_c");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("C BOP", |b| {
            b.iter(|| {
                let mut output_vec = vec![0.0_f64; output_len];
                let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust implementation of BOP.
fn bench_rust_bop(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("bop");
        ...
        let mut group = c.benchmark_group("bop_rust");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the TA-Lib implementation of BOP.
#[cfg(feature = "talib")]
fn bench_talib_bop(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("bop");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let start_index = ta_bop_start();
                    assert!(start_index >= 0, "ta_bop_start returned a negative index");
                    let output_len = open.len() - (start_index as usize);
                    let mut output_vec = vec![0.0_f64; output_len];
                    let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                    let ret = ta_bop(
                    ...
                    assert_eq!(ret, 0, "ta_bop returned error code {}", ret);
                    black_box(&output_vec);
                },
                SAMPLE_SIZE,
            );
        ...
        let mut group = c.benchmark_group("bop_talib");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("TA-Lib BOP", |b| {
            b.iter(|| {
                let mut output_vec = vec![0.0_f64; output_len];
                let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust from_state implementation of BOP.
fn bench_rust_bop_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("bop");
                ...
                timing.measure(
                    || {
                        let result = state
                            ...
                            .expect("Rust BOP from state indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        let mut group = c.benchmark_group("Rust BOP from state");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust SIMD by assets implementation of BOP.
fn bench_rust_bop_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("bop");
        ...
        let mut group = c.benchmark_group("bop_rust_simd_by_assets");
        group.sample_size(SAMPLE_SIZE);
