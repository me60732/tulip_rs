...
use tulip_rs::indicators::aroon::{
    Aroon, Indicator, IndicatorByOptions, IndicatorState, TIndicatorState,
};
...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_aroon, ta_aroon_start};

// Sample input data from aroon_test.rs
const HIGH: [f64; 15] = [
...
/// Benchmark the C implementation of AROON.
fn bench_c_aroon(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("aroon");
                ...
                timing.measure(
                    || {
                        ...
                        let mut outputs: Vec<*mut f64> =
                            vec![aroon_down_vec.as_mut_ptr(), aroon_up_vec.as_mut_ptr()];
                        ...
                        black_box(&aroon_down_vec);
                        black_box(&aroon_up_vec);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust implementation of AROON.
fn bench_rust_aroon(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("aroon");
...
/// Benchmark the Rust from_state implementation of AROON.
const CHUNK_SIZE: usize = 100;
fn bench_rust_aroon_from_state(c: &mut Criterion) {
    if should_log_to_db() {
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust AROON from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the TA-Lib implementation of AROON.
#[cfg(feature = "talib")]
fn bench_talib_aroon(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("aroon");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_aroon_start(options[0]);
                        assert!(start_index >= 0, "ta_aroon_start returned a negative index");
                        let output_len = high.len() - (start_index as usize);
                        let mut aroon_down_vec = vec![0.0_f64; output_len];
                        let mut aroon_up_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> =
                        ...
                        assert_eq!(ret, 0, "ta_aroon returned error code {}", ret);
                        black_box(&aroon_down_vec);
                        black_box(&aroon_up_vec);
                    },
                    SAMPLE_SIZE,
...
fn bench_rust_aroon_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("aroon");
        ...
        for options in OPTIONS_LIST {
            ...
            timing.measure(
                || {
                    let result = Aroon::indicator_by_assets::<4>(&inputs, &options, None);
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );
        ...
        for options in OPTIONS_LIST {
            ...
            group.bench_function(
                ...
                |b| {
                    b.iter(|| {
                        let result = Aroon::indicator_by_assets::<4>(&inputs, &options, None)
                            .expect("Rust SIMD by assets Aroon indicator failed");
                        black_box(&result);
                    });
                },
            );
...
fn bench_rust_aroon_simd_by_options(c: &mut Criterion) {
    let options_4 = [
        &OPTIONS_LIST[0],
        &OPTIONS_LIST[1],
        &OPTIONS_LIST[2],
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let result = Aroon::indicator_by_options::<4>(&inputs, &options_4, None)
                        .expect("Rust SIMD AROON indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );
        ...
        let mut group = c.benchmark_group("aroon_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);

        group.bench_function("Rust SIMD by options AROON (4 lanes)", |b| {
            b.iter(|| {
                let result = Aroon::indicator_by_options::<4>(&inputs, &options_4, None)
                    .expect("Rust SIMD AROON indicator failed");
                black_box(&result);
...
criterion_group!(
    benches,
    bench_rust_aroon_simd_by_assets,
    bench_rust_aroon_simd_by_options,
    bench_rust_aroon,
    bench_c_aroon,
