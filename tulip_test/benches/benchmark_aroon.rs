...
fn bench_rust_aroon_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    let result = tulip_rs::indicators::aroon::indicator_by_options::<4>(
                        &inputs,
                        &options_4_first,
                        None,
                    )
        ...
        group.bench_function("Rust SIMD by options AROON (4 lanes first)", |b| {
            b.iter(|| {
                let result = tulip_rs::indicators::aroon::indicator_by_options::<4>(
                    &inputs,
                    &options_4_first,
        ...
        group.bench_function("Rust SIMD by options AROON (4 lanes second)", |b| {
            b.iter(|| {
                let result = tulip_rs::indicators::aroon::indicator_by_options::<4>(
                    &inputs,
                    &options_4_second,
