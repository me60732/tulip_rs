...
/// Benchmark the C implementation of WAD.
fn bench_c_wad(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wad");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let start_index = unsafe { ti_wad_start(OPTIONS.as_ptr()) };
        ...
        let mut group = c.benchmark_group("wad_c");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("C WAD", |b| {
            b.iter(|| {
                let mut output_vec = vec![0.0_f64; output_len];
                let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust implementation of WAD.
fn bench_rust_wad(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wad");
        ...
        let mut group = c.benchmark_group("wad_rust");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust from_state implementation of WAD.
fn bench_rust_wad_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wad");
                ...
                timing.measure(
                    || {
                        let result = state
                            ...
                            .expect("Rust WAD from state indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        let mut group = c.benchmark_group("Rust WAD from state");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust SIMD by assets implementation of WAD.
fn bench_rust_wad_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wad");
