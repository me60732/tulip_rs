...
use tulip_rs::indicators::zlema::{indicator, min_data, IndicatorState, TIndicatorState};
...
/// Benchmark the C implementation of ZLEMA.
fn bench_c_zlema(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("zlema");
...
/// Benchmark the Rust implementation of ZLEMA.
fn bench_rust_zlema(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("zlema");
...
/// Benchmark the Rust from_state implementation of ZLEMA.
fn bench_rust_zlema_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("zlema");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust ZLEMA from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
        ...
        for options in OPTIONS_LIST {
            ...
            group.bench_function("benchmark", |b| {
                b.iter(|| {
                    ...
                    let (_, mut state) =
                        indicator(&chunk_inputs, &options, None).expect("ZLEMA indicator failed");
...
fn bench_rust_zlema_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("zlema");
...
fn bench_rust_zlema_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        let mut group = c.benchmark_group("zlema_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
...
criterion_group!(
    ...
    bench_rust_zlema_simd_by_assets,
    bench_rust_zlema,
    bench_c_zlema,
