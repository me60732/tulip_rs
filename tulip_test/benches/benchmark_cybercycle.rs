...
const OPTIONS_LIST: [[f64; 1]; 4] = [[0.07], [0.10], [0.15], [0.20]];
...
fn bench_cybercycle(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cybercycle");
        ...
        for options in OPTIONS_LIST {
            for (stock_symbol, stock_data) in data {
                ...
                timing.measure(
                    || {
                        let result = indicator(&inputs, &options, None).expect("CyberCycle failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        let inputs = [close.as_slice()];
        let mut group = c.benchmark_group("cybercycle_rust");
...
fn bench_cybercycle_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cybercycle");
        let data = get_all_stock_data().unwrap();
            ...
            for (stock_symbol, stock_data) in data {
                ...
                if n > 1 {
                    let (_, mut state) = indicator(&[&close[..n - 1]], &options, None)
                        .expect("CyberCycle seed (1-bar) failed");
                        ...
                        || {
                            black_box(
                                state
                                    ...
                                    .expect("CyberCycle 1-bar update failed"),
                            );
                        },
                        SAMPLE_SIZE,
                    );
        ...
        for options in OPTIONS_LIST {
            let seed = min_data(&options).max(CHUNK_SIZE);
            let (_, mut state) =
                indicator(&[&close_vec[..seed]], &options, None).expect("CyberCycle seed failed");

            let mut group = c.benchmark_group("cybercycle_rust_from_state");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(
                format!("Rust CyberCycle from state (alpha={})", options[0]),
                |b| {
                    b.iter(|| {
        ...
        for options in OPTIONS_LIST {
            if close_vec.len() > 1 {
                ...
                let mut group = c.benchmark_group("cybercycle_rust_from_state_1_bar");
                group.sample_size(SAMPLE_SIZE);
                group.bench_function(
                    format!("Rust CyberCycle from state 1 bar (alpha={})", options[0]),
                    |b| {
                        b.iter(|| {
                            black_box(
                                state
...
fn bench_cybercycle_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cybercycle");
        let data = get_all_stock_data().unwrap();
        ...
        let mut group = c.benchmark_group("cybercycle_rust_simd_by_assets");
        group.sample_size(SAMPLE_SIZE);
        for options in OPTIONS_LIST {
            group.bench_function(
...
fn bench_cybercycle_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cybercycle");
        let data = get_all_stock_data().unwrap();
        ...
        let mut group = c.benchmark_group("cybercycle_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
...
criterion_group!(
    benches,
    bench_cybercycle_simd_by_assets,
    bench_cybercycle_simd_by_options,
    bench_cybercycle,
    bench_cybercycle_from_state,
);
