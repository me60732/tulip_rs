            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        let (_, mut state) =
                            indicator(&chunk_inputs, &options, None).expect("RSI indicator failed");
...
fn bench_rust_rsi_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let result_4 = indicator_by_options::<4>(
                        &inputs, &options_4, None,
                    )
                    .expect("Rust SIMD RSI indicator failed");
                    black_box(&result_4);

                    let result_2 = indicator_by_options::<2>(
        ...
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options RSI (4+2 lanes)", |b| {
            b.iter(|| {
                let result_4 =
                ...
                let result_2 =
                    tulip_rs::indicators::rsi::indicator_by_options::<2>(&inputs, &options_2, None)
                        .expect("Rust SIMD RSI indicator failed");
                black_box(&result_2);
            });
        });
