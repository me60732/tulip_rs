...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_rsi, ta_rsi_start};
...
/// Benchmark the C implementation of RSI.
fn bench_c_rsi(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rsi");
...
/// Benchmark the Rust implementation of RSI.
fn bench_rust_rsi(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rsi");
...
/// Benchmark the Rust from_state implementation of RSI.
fn bench_rust_rsi_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rsi");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let min_data = Rsi::min_data(&options).max(CHUNK_SIZE);
                        // First chunk
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust RSI from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the Rust SIMD by assets implementation of RSI.
fn bench_rust_rsi_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rsi");
...
/// Benchmark the TA-Lib implementation of RSI.
#[cfg(feature = "talib")]
fn bench_talib_rsi(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rsi");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_rsi_start(options[0]);
                        assert!(start_index >= 0, "ta_rsi_start returned a negative index");
                        let output_len = close.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_rsi(
                        ...
                        assert_eq!(ret, 0, "ta_rsi returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
fn bench_rust_rsi_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        let mut group = c.benchmark_group("rsi_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
...
fn bench_vector_ta_rsi(c: &mut Criterion) {
    ...
    if should_log_to_db() {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        let input = RsiInput::from_slice(&close_vec, params);
                        let output = rsi(&input).unwrap();
                        black_box(output.values);
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result("rsi", "VectorTa", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        let close_vec = expand_inputs();
        for options in OPTIONS_LIST {
            let period = options[0] as usize;
            let mut group = c.benchmark_group("rsi_vector_ta");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("VectorTa RSI {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let params = RsiParams {
                        period: Some(period),
                    };
