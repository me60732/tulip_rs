...
/// Benchmark the C implementation of HMA.
fn bench_c_hma(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("hma");
...
/// Benchmark the Rust implementation of HMA.
fn bench_rust_hma(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("hma");
...
/// Benchmark the Rust from_state implementation of HMA.
fn bench_rust_hma_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("hma");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let min_data = Hma::min_data(&options);
                        // First chunk
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust HMA from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
// Rustalib HMA benchmark removed due to Polars null weights issue:
// "weights not yet supported on array with null values"
// The rustalib HMA implementation creates intermediate null values during calculation
// which causes Polars to panic when using weighted rolling means.

/// Benchmark the Rust SIMD by assets implementation of HMA.
fn bench_rust_hma_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("hma");
...
/// Benchmark the Rust SIMD by options implementation of HMA.
fn bench_rust_hma_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("hma");
        ...
        for (symbol, close) in stock_data.iter() {
            let inputs = [close.as_slice()];
