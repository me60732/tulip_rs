...
// Sample input data from kvo_test.rs
const HIGH: [f64; 15] = [
...
// Options for KVO (short_period, long_period)
const OPTIONS_LIST: [[f64; 2]; 4] = [[2.0, 5.0], [9.0, 26.0], [14.0, 30.0], [20.0, 50.0]];
...
/// Benchmark the C implementation of KVO.
fn bench_c_kvo(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("kvo");
...
/// Benchmark the Rust implementation of KVO.
fn bench_rust_kvo(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("kvo");
...
/// Benchmark the Rust implementation of KVO with optional outputs.
fn bench_rust_kvo_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("kvo");
...
/// Benchmark the Rust from_state implementation of KVO.
fn bench_rust_kvo_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("kvo");
...
/// Benchmark the Rust SIMD by assets implementation of KVO.
fn bench_rust_kvo_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("kvo");
...
/// Benchmark the Rust SIMD by options implementation of KVO.
fn bench_rust_kvo_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("kvo");
        ...
        let mut group = c.benchmark_group("kvo_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("SIMD by options KVO", |b| {
            b.iter(|| {
                let options_4 = [
                    &OPTIONS_LIST[0],
                    &OPTIONS_LIST[1],
