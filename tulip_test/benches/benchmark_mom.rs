...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_mom, ta_mom_start};
...
const OPTIONS_LIST: [[f64; 1]; 4] = [[25.0], [30.0], [50.0], [100.0]];
...
/// Benchmark the C implementation of MOM.
fn bench_c_mom(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("mom");
...
/// Benchmark the Rust implementation of MOM.
fn bench_rust_mom(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("mom");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust MOM from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the TA-Lib implementation of MOM.
#[cfg(feature = "talib")]
fn bench_talib_mom(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("mom");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_mom_start(options[0]);
                        assert!(start_index >= 0, "ta_mom_start returned a negative index");
                        let output_len = close.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_mom(
                        ...
                        assert_eq!(ret, 0, "ta_mom returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust SIMD by assets implementation of MOM.
fn bench_rust_mom_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("mom");
...
fn bench_rust_mom_simd_by_options(c: &mut Criterion) {
    let options_4 = [
        &OPTIONS_LIST[0],
        &OPTIONS_LIST[1],
        &OPTIONS_LIST[2],
    ...
    if should_log_to_db() {
        ...
        let mut group = c.benchmark_group("mom_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
...
fn bench_kand_mom(c: &mut Criterion) {
    use kand::ohlcv::mom;
    ...
    if should_log_to_db() {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        mom::mom(&close, period, &mut output).unwrap();
                        black_box(&output);
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result("mom", "RustKanda", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        let close_vec = expand_inputs();
        ...
        for options in OPTIONS_LIST {
            let period = options[0] as usize;
            let mut group = c.benchmark_group("mom_kand");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Kand MOM {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut output = vec![0.0_f64; n];
                    mom::mom(&close_vec, period, &mut output).unwrap();
                    black_box(output);
                });
            });
            group.finish();
