...
fn bench_rust_mom_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    // Process remaining options with 4-wide SIMD
                    
                    let result_4_second =
                        indicator_by_options::<4>(&inputs, &options_4_2, None)
                            .expect("Rust SIMD MOM indicator failed");
        ...
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options MOM (4+4 lanes)", |b| {
            b.iter(|| {
                // Process first 4 options with 4-wide SIMD
                let result_4 = indicator_by_options::<4>(&inputs, &options_4_1, None)
                    .expect("Rust SIMD MOM indicator failed");
                ...
                // Process remaining options with 4-wide SIMD
                let result_4_second = indicator_by_options::<4>(&inputs, &options_4_2, None)
                    .expect("Rust SIMD MOM indicator failed");
                black_box(&result_4_second);
            });
        });
