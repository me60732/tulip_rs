...
use tulip_test::talib_bindings::{ta_rocr, ta_rocr_start};
...
/// Benchmark the C implementation of ROCR.
fn bench_c_rocr(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rocr");
...
/// Benchmark the Rust implementation of ROCR.
fn bench_rust_rocr(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rocr");
...
/// Benchmark the Rust from_state implementation of ROCR.
fn bench_rust_rocr_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rocr");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust ROCR from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the TA-Lib implementation of ROCR.
fn bench_talib_rocr(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rocr");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_rocr_start(options[0]);
                        assert!(start_index >= 0, "ta_rocr_start returned a negative index");
                        let output_len = close.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_rocr(
                        ...
                        assert_eq!(ret, 0, "ta_rocr returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust SIMD by assets implementation of ROCR.
fn bench_rust_rocr_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("rocr");
...
fn bench_rust_rocr_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    let result_1 = indicator_by_options::<4>(&inputs, &options_4_2, None)
                        .expect("Rust SIMD ROCR indicator failed");
                    black_box(&result_1);
                },
                SAMPLE_SIZE,
        ...
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options ROCR (4+1 lanes)", |b| {
            b.iter(|| {
                // Process first 4 options with 4-wide SIMD
                let result_4 = indicator_by_options::<4>(&inputs, &options_4_1, None)
                    .expect("Rust SIMD ROCR indicator failed");
