...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_adx, ta_adx_start};
// Sample input data from adx_test.rs
const CLOSE: [f64; 15] = [
...
/// Benchmark the C implementation of ADX.
fn bench_c_adx(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adx");
...
/// Benchmark the Rust implementation of ADX.
fn bench_rust_adx(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adx");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result = Adx::indicator(&inputs, &options, None)
        ...
        for options in OPTIONS_LIST {
            ...
            group.bench_function(format!("Rust ADX {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let result =
                        Adx::indicator(&inputs, &options, None).expect("Rust ADX indicator failed");
                    black_box(&result);
                });
            });
            group.finish();
...
/// Benchmark the TA-Lib implementation of ADX.'
#[cfg(feature = "talib")]
fn bench_talib_adx(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adx");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_adx_start(options[0]);
                        assert!(start_index >= 0, "ta_adx_start returned a negative index");
                        let output_len = high.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_adx(
                        ...
                        assert_eq!(ret, 0, "ta_adx returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust implementation of ADX with optional outputs.
fn bench_rust_adx_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adx");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result = Adx::indicator(&inputs, &options, Some(&[true, true, true]))
                            .expect("Rust ADX indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
...
fn bench_rust_adx_from_state(c: &mut Criterion) {
    if should_log_to_db() {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        let (_, mut state) = Adx::indicator(&chunk_inputs, &options, None)
                ...
                if inputs[0].len() > 1 {
                    ...
                    let (_, mut state) = Adx::indicator(&new_inputs, &options, None)
                    ...
                    let (_, state) = Adx::indicator(&new_inputs, &options, None)
                        .expect("Rust DX indicator failed");
...
fn bench_rust_adx_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adx");
...
fn bench_rust_adx_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adx");
        ...
        let mut group = c.benchmark_group("adx_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options ADX (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
...
criterion_group!(
    benches,
    bench_rust_adx_simd_by_options,
    bench_rust_adx_simd_by_assets,
    bench_rust_adx,
    bench_c_adx,
    bench_talib_adx,
