...
use tulip_rs::indicators::stochrsi::{
    indicator as rust_stochrsi, min_data, IndicatorState, TIndicatorState, indicator_by_options
};
...
fn bench_rust_stochrsi_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    /*let result =
                        tulip_rs::indicators::nightly::by_option::stochrsi::indicator_by_options::<2>(
        ...
        group.bench_function("Rust SIMD by options STOCHRSI (4 lanes first)", |b| {
            b.iter(|| {
                let result =
                    indicator_by_options::<4>(
                        &inputs,
        ...
        /*group.bench_function("Rust SIMD by options STOCHRSI (4 lanes second)", |b| {
            b.iter(|| {
                let result =
                    tulip_rs::indicators::nightly::by_option::stochrsi::indicator_by_options::<4>(
                        &inputs,
                        &options_4_second,
                        None,
                    )
        ...
        group.bench_function("Rust SIMD by options STOCHRSI (2 lanes)", |b| {
            b.iter(|| {
                let result = tulip_rs::indicators::stochrsi::indicator_by_options::<4>(
                    &inputs, &options_4, None,
                )
                .expect("Rust SIMD by options STOCHRSI indicator failed");
                black_box(&result);
            });
        });*/
