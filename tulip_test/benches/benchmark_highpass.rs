...
/// Benchmark the full HighPass indicator.
fn bench_rust_highpass(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                log_timing_result("highpass", "Rust", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("highpass_rust");
...
/// Benchmark HighPass using `batch_indicator` for streaming updates.
///
...
fn bench_rust_highpass_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("highpass");
                        ...
                        for chunk in chunks.by_ref() {
                            black_box(
                                state
                                    ...
                                    .expect("HighPass batch_indicator failed"),
                            );
                        }
                        ...
                        || {
                            black_box(
                                state
                                    ...
                                    .expect("HighPass 1-bar update failed"),
                            );
                        },
                        SAMPLE_SIZE,
                    );
        ...
        for options in OPTIONS_LIST {
            ...
            group.bench_function(
                format!("Rust HighPass from state {{ period: {} }}", options[0]),
                |b| {
                    b.iter(|| {
            ...
            if close_vec.len() > 1 {
                ...
                let mut group = c.benchmark_group("highpass_rust_from_state_1_bar");
                group.sample_size(SAMPLE_SIZE);
                group.bench_function(
                    format!(
                        "Rust HighPass from state 1 bar {{ period: {} }}",
                        options[0]
                    ),
                    |b| {
                        b.iter(|| {
...
/// Benchmark SIMD by-assets HighPass (4 assets processed simultaneously).
fn bench_rust_highpass_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("highpass");
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("highpass_rust_simd_by_assets");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(
...
/// Benchmark SIMD by-options HighPass (4 option sets on a single asset simultaneously).
fn bench_rust_highpass_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("highpass");
        ...
        let mut group = c.benchmark_group("highpass_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
