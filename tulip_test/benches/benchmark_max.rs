...
use tulip_test::talib_bindings::{ta_max, ta_max_start};
...
/// Benchmark the C implementation of MAX.
...
/// Benchmark the Rust implementation of MAX.
fn bench_rust_max(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("max");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result =
                            indicator(&inputs, &options, None).expect("MAX indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust from_state implementation of MAX.
const CHUNK_SIZE: usize = 100;
fn bench_rust_max_from_state(c: &mut Criterion) {
    if should_log_to_db() {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        let (_, mut state) =
                            indicator(&chunk_inputs, &options, None).expect("MAX indicator failed");
                ...
                if inputs[0].len() > 1 {
                    ...
                    let (_, mut state) =
                        indicator(&new_inputs, &options, None).expect("Rust MAX indicator failed");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust MAX from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the TA-Lib implementation of MAX.
fn bench_talib_max(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("max");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_max_start(options[0]);
                        assert!(start_index >= 0, "ta_max_start returned a negative index");
                        let output_len = close.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_max(
                        ...
                        assert_eq!(ret, 0, "ta_max returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust SIMD by assets implementation of max.
fn bench_rust_max_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("max");
...
// Benchmark the Rust SIMD by options implementation of max.
fn bench_rust_max_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("max");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    /*let result_8 =
                        tulip_rs::indicators::nightly::by_option::max::indicator_by_option::<4>(
                            &inputs,
                            &options_8,
                            None,
                        )
        ...
        let mut group = c.benchmark_group("max_rust_simd");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD MAX (4+4+1 lanes)", |b| {
            b.iter(|| {
                // Process first 4 options with 4-wide SIMD
                let options_4_1 = [
                    &OPTIONS_LIST[0],
                ...
                /*let options_1 = [&OPTIONS_LIST[8]];
                let result_1 =
                    tulip_rs::indicators::nightly::by_option::max::indicator_by_option::<1>(
