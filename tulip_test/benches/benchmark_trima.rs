...
use tulip_test::talib_bindings::{ta_trima, ta_trima_start};
...
/// Benchmark the C implementation of TRIMA.
fn bench_c_trima(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("trima");
...
/// Benchmark the Rust implementation of TRIMA.
fn bench_rust_trima(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("trima");
...
/// Benchmark the Rust from_state implementation of TRIMA.
fn bench_rust_trima_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("trima");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust TRIMA from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the Rust SIMD by assets implementation of TRIMA.
fn bench_rust_trima_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("trima");
...
/// Benchmark the TA-Lib implementation of TRIMA.
fn bench_talib_trima(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("trima");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_trima_start(options[0]);
                        assert!(start_index >= 0, "ta_trima_start returned a negative index");
                        let output_len = close.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_trima(
                        ...
                        assert_eq!(ret, 0, "ta_trima returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
fn bench_rust_trima_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
            ...
            timing.measure(
                || {
                    ...
                    let _ = indicator_by_options::<2>(&inputs, &options_2, None)
                        .expect("Rust SIMD TRIMA 2-wide failed");
                },
                SAMPLE_SIZE,
            );

            // Note: database expects the dstring "Rust_SIMD" for SIMD benchmarks
            log_timing_result(
                "trima",
                "Rust_SIMD",
                &[0.0],
        ...
        let mut group = c.benchmark_group("trima_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options TRIMA (4 lanes + 2 lanes)", |b| {
            b.iter(|| {
                // Process first 4 options with 4-wide SIMD
                let _ = indicator_by_options::<4>(&inputs, &options_4, None)
                    .expect("Rust SIMD TRIMA 4-wide failed");
