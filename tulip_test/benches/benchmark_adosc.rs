...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_adosc, ta_adosc_start};

// Sample input data from adosc_test.rs
const CLOSE: [f64; 15] = [
...
// Options for ADOSC (fast_period, slow_period)
...
/// Benchmark the C implementation of ADOSC.
fn bench_c_adosc(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adosc");
...
/// Benchmark the Rust implementation of ADOSC.
fn bench_rust_adosc(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adosc");
...
/// Benchmark the Rust implementation of ADOSC with optional outputs.
fn bench_rust_adosc_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adosc");
...
/// Benchmark the Rust from_state implementation of ADOSC.
fn bench_rust_adosc_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adosc");
                ...
                timing.measure(
                    || {
                        let result = state
                            ...
                            .expect("Rust ADOSC from state indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec, volume_vec) = expand_inputs();
        //let inputs = [high_vec.as_slice(), low_vec.as_slice(), close_vec.as_slice(), volume_vec.as_slice()];
...
/// Benchmark the Rust SIMD by assets implementation of ADOSC.
fn bench_rust_adosc_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adosc");
...
/// Benchmark the TA-Lib implementation of ADOSC.
#[cfg(feature = "talib")]
fn bench_talib_adosc(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adosc");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_adosc_start(options[0], options[1]);
                        assert!(start_index >= 0, "ta_adosc_start returned a negative index");
                        ...
                        assert_eq!(ret, 0, "ta_adosc returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust SIMD by options implementation of ADOSC.
fn bench_rust_adosc_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("adosc");
        ...
        let mut group = c.benchmark_group("adosc_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options ADOSC (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
...
criterion_group!(
    benches,
    bench_rust_adosc_simd_by_assets,
    bench_rust_adosc_simd_by_options,
    bench_rust_adosc,
    bench_c_adosc,
    bench_talib_adosc,
