...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_apo, ta_apo_start};

// Sample input data from apo_test.rs
const CLOSE: [f64; 15] = [
    81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36, 85.53, 86.54, 86.89,
    87.77, 87.29,
];

// Options for APO (fast period, slow period)
//const OPTIONS_LIST: [[f64; 2]; 2] = [[2.0, 5.0], [11.0, 21.0]];
...
/// Benchmark the C implementation of APO.
fn bench_c_apo(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("apo");
...
/// Benchmark the Rust implementation of APO.
fn bench_rust_apo(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("apo");
...
/// Benchmark the Rust from_state implementation of APO.
const CHUNK_SIZE: usize = 100;
fn bench_rust_apo_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("apo");
                ...
                timing.measure(
                    || {
                        let result = state
                            ...
                            .expect("Rust APO from state indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust SIMD by assets implementation of APO.
fn bench_rust_apo_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("apo");
...
/// Benchmark the TA-Lib implementation of APO.
#[cfg(feature = "talib")]
fn bench_talib_apo(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("apo");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_apo_start(options[0], options[1]);
                        assert!(start_index >= 0, "ta_apo_start returned a negative index");
                        let output_len = close.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_apo(
                        ...
                        assert_eq!(ret, 0, "ta_apo returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );
...
/// Benchmark the Rust SIMD by options implementation of APO.
fn bench_rust_apo_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("apo");
        ...
        let mut group = c.benchmark_group("apo_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options APO (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
...
criterion_group!(
    benches,
    bench_rust_apo_simd_by_assets,
    bench_rust_apo_simd_by_options,
    bench_rust_apo,
    bench_c_apo,
    bench_talib_apo,
