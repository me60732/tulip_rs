...
fn bench_rust_volatility_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    // Process remaining 1 option with 1-wide SIMD
                    let result_1 = indicator_by_options::<2>(&inputs, &options_2, None)
                        .expect("Rust SIMD VOLATILITY indicator failed");
                    black_box(&result_1);
                },
                SAMPLE_SIZE,
        ...
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD VOLATILITY (4+1 lanes)", |b| {
            b.iter(|| {
                // Process first 4 options with 4-wide SIMD
                let result_4 = indicator_by_options::<4>(&inputs, &options_4, None)
                    .expect("Rust SIMD VOLATILITY indicator failed");
