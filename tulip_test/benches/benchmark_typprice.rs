...
// Sample input data from typprice_test.rs
const HIGH: [f64; 15] = [
...
// Options for TYPPRICE (no options)
const OPTIONS: [f64; 0] = [];
...
/// Benchmark the C implementation of TYPPRICE.
fn bench_c_typprice(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let start_index = unsafe { ti_typprice_start(OPTIONS.as_ptr()) };
                    assert!(
                        start_index >= 0,
                        "ti_typprice_start returned a negative index"
                    );
                    let output_len = high_vec.len() - (start_index as usize);
                    /*let mut output_vec: Vec<f64> = Vec::with_capacity(output_len);
                    unsafe { output_vec.set_len(output_len); }*/
        ...
        let mut group = c.benchmark_group("typprice_c");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("C TYPPRICE", |b| {
            b.iter(|| {
                let mut output_vec = vec![0.0_f64; output_len];
                let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust implementation of TYPPRICE.
fn bench_rust_typprice(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("typprice");
        ...
        let mut group = c.benchmark_group("typprice_rust");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust from_state implementation of TYPPRICE.
fn bench_rust_typprice_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("typprice");
                ...
                timing.measure(
                    || {
                        let result = state
                            ...
                            .expect("Rust TYPPRICE from state indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        let mut group = c.benchmark_group("Rust TYPPRICE from state");
        group.sample_size(SAMPLE_SIZE);
        ...
        group.bench_function("benchmark", |b| {
            b.iter(|| {
                ...
                if !high_rem.is_empty() && !low_rem.is_empty() && !close_rem.is_empty() {
                    ...
                    let result =
                        state.batch_indicator(&[&high_rem_vec, &low_rem_vec, &close_rem_vec], None);
                    black_box(&result);
                }
            });
        });
        ...
        group.bench_function("benchmark", |b| {
            ...
            b.iter(|| {
                let result = state
                    .batch_indicator(
                        &[
                            final_high_vec.as_slice(),
                            final_low_vec.as_slice(),
                            final_close_vec.as_slice(),
                        ],
                        None,
                    )
                    .expect("Rust TYPPRICE from state indicator failed");
...
/// Benchmark the Rust SIMD by assets implementation of TYPPRICE.
fn bench_rust_typprice_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("typprice");
        ...
        let mut group = c.benchmark_group("typprice_rust_simd_by_assets");
        group.sample_size(SAMPLE_SIZE);
