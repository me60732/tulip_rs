...
use tulip_rs::indicators::mass::{
    Indicator, IndicatorByOptions, IndicatorState, Mass, TIndicatorState,
};
...
// Options for MASS - copied from test file
...
fn bench_c_mass(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("mass");
...
fn bench_rust_mass(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("mass");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result = Mass::indicator(&inputs, &options, None)
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        if !high_rem.is_empty() {
                            ...
                            let result =
                                state.batch_indicator(&[&high_rem_vec, &low_rem_vec], None);
                            black_box(&result);
                        }
                    },
                    SAMPLE_SIZE,
                ...
                if inputs[0].len() > 1 {
                    ...
                    let (_, mut state) = Mass::indicator(&new_inputs, &options, None)
                        .expect("Rust MASS Mass::indicator failed");
                    ...
                    let (_, state) = Mass::indicator(&new_inputs, &options, None)
                        .expect("Rust MASS Mass::indicator failed");
...
/// Benchmark the Rust SIMD by assets implementation of MASS.
fn bench_rust_mass_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("mass");
        ...
        for options in OPTIONS_LIST {
            ...
            timing.measure(
                || {
                    let result = Mass::indicator_by_assets::<4>(&inputs, &options, None)
...
/// Benchmark the Rust SIMD by options implementation of MASS.
fn bench_rust_mass_simd_by_options(c: &mut Criterion) {
    let options_4 = [
        &OPTIONS_LIST[0],
        &OPTIONS_LIST[1],
        &OPTIONS_LIST[2],
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let result_4 = Mass::indicator_by_options::<4>(&inputs, &options_4, None)
        ...
        let mut group = c.benchmark_group("mass_rust_simd");
        group.sample_size(SAMPLE_SIZE);
...
criterion_main!(mass_benchmarks);
