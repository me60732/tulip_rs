...
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_atr, ta_atr_start};

// Sample input data from atr_test.rs
const HIGH: [f64; 15] = [
...
// Options for ATR (period)
const OPTIONS_LIST: [[f64; 1]; 4] = [[5.0], [14.0], [25.0], [30.0]];
...
/// Benchmark the C implementation of ATR.
fn bench_c_atr(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("atr");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                log_timing_result("atr", "C_tulip", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        ...
        for options in OPTIONS_LIST {
            ...
            let mut group = c.benchmark_group("atr_c");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("C ATR {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut output_vec = vec![0.0_f64; output_len];
                    let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust implementation of ATR.
fn bench_rust_atr(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("atr");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result = Atr::indicator(&inputs, &options, None); //.expect("Rust ATR indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );

                log_timing_result("atr", "Rust", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        // Run Criterion benchmark with synthetic data
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("atr_rust");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Rust ATR {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let result =
                        Atr::indicator(&inputs, &options, None).expect("Rust ATR indicator failed");
                    black_box(&result);
                });
            });
            group.finish();
...
/// Benchmark the Rust from_state implementation of ATR.
fn bench_rust_atr_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("atr");
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        let (_, mut state) = Atr::indicator(&chunk_inputs, &options, None)
                ...
                if inputs[0].len() > 1 {
                    ...
                    let (_, mut state) = Atr::indicator(&new_inputs, &options, None)
                        .expect("Rust ATR indicator failed");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust ATR from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
                    ...
                    let (_, state) = Atr::indicator(&new_inputs, &options, None)
                        .expect("Rust ATR indicator failed");
        ...
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();
        //let inputs = [high_vec.as_slice(), low_vec.as_slice(), close_vec.as_slice()];

        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("atr_rust_from_state");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Rust ATR from state {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let min_data_val = Atr::min_data(&options).max(CHUNK_SIZE);
                    // First chunk
                    let chunk_inputs = [
...
/// Benchmark the Rust implementation of ATR with optional outputs.
fn bench_rust_atr_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("atr");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result = Atr::indicator(&inputs, &options, Some(&[true]))
                            .expect("Rust ATR indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
...
/// Benchmark the TA-Lib implementation of ATR.
#[cfg(feature = "talib")]
fn bench_talib_atr(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("atr");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let start_index = ta_atr_start(options[0]);
                        assert!(start_index >= 0, "ta_atr_start returned a negative index");
                        let output_len = high.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_atr(
                        ...
                        assert_eq!(ret, 0, "ta_atr returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );

                log_timing_result("atr", "talib", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        // Run Criterion benchmark with synthetic data
        ...
        for options in OPTIONS_LIST {
            ...
            let mut group = c.benchmark_group("atr_talib");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("TA-Lib ATR {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut output_vec = vec![0.0_f64; output_len];
                    let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust SIMD by assets implementation of ATR.
fn bench_rust_atr_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("atr");
        ...
        for options in &OPTIONS_LIST {
            ...
            let mut group = c.benchmark_group("atr_rust_simd_by_assets");
            group.sample_size(SAMPLE_SIZE);
...
fn bench_rust_atr_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("atr");
        ...
        let mut group = c.benchmark_group("atr_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options ATR (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
...
fn bench_rust_ta_atr(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let mut atr = AverageTrueRange::new(period).expect("ta ATR new failed");
                ...
                log_timing_result("atr", "RustTa", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        let (high_vec, low_vec, close_vec) = expand_inputs();
        ...
        for options in OPTIONS_LIST {
            let period = options[0] as usize;
            let mut group = c.benchmark_group("atr_rust_ta");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("RustTa ATR {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut atr = AverageTrueRange::new(period).expect("ta ATR new failed");
                    let mut last = 0.0_f64;
