...
use tulip_rs::indicators::wcprice::{indicator, min_data, IndicatorState, TIndicatorState};
...
// Options for WCPRICE (no options)
const OPTIONS: [f64; 0] = [];
...
/// Benchmark the C implementation of WCPRICE.
fn bench_c_wcprice(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let start_index = unsafe { ti_wcprice_start(OPTIONS.as_ptr()) };
                    assert!(
                        start_index >= 0,
        ...
        let mut group = c.benchmark_group("wcprice_c");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("C WCPRICE", |b| {
            b.iter(|| {
                let mut output_vec = vec![0.0_f64; output_len];
                let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
...
/// Benchmark the Rust implementation of WCPRICE.
fn bench_rust_wcprice(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wcprice");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    let result =
                        indicator(&inputs, &OPTIONS, None).expect("Rust WCPRICE indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );
        ...
        let mut group = c.benchmark_group("wcprice_rust");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust from_state implementation of WCPRICE.
fn bench_rust_wcprice_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wcprice");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    let (_, mut state) =
                        indicator(&chunk_inputs, &OPTIONS, None).expect("WCPRICE indicator failed");
            ...
            if high.len() > 1 {
                ...
                let (_, mut state) =
                    indicator(&new_inputs, &OPTIONS, None).expect("Rust WCPRICE indicator failed");
                ...
                timing.measure(
                    || {
                        let result = state
                            ...
                            .expect("Rust WCPRICE from state indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        let mut group = c.benchmark_group("Rust WCPRICE from state");
        group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust SIMD by assets implementation of WCPRICE.
fn bench_rust_wcprice_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wcprice");
        ...
        let mut group = c.benchmark_group("wcprice_rust_simd_by_assets");
        group.sample_size(SAMPLE_SIZE);
