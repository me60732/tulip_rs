...
// Options for VHF - copied from test file
...
fn bench_c_vhf(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("vhf");
...
fn bench_rust_vhf(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("vhf");
...
fn bench_rust_vhf_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("vhf");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust VHF from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
fn bench_rust_vhf_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("vhf");
...
fn bench_rust_vhf_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("vhf");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            // Create options array with 4 option sets for SIMD processing (2x4)
            let options_4_1 = [
                &OPTIONS_LIST[0], // [5.0]
                &OPTIONS_LIST[1], // [8.0]
                &OPTIONS_LIST[2], // [10.0]
            ...
            // Benchmark first batch of 4 options
            let mut timing = TimingMeasurements::new();
            timing.measure(
                || {
        ...
        let mut group = c.benchmark_group("vhf_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("SIMD by options VHF 2x4", |b| {
            b.iter(|| {
                let result =
                    tulip_rs::indicators::vhf::indicator_by_options::<8>(&inputs, &options_8, None)
                        .expect("Rust SIMD by options VHF indicator failed");
                black_box(&result);
                // Benchmark both batches of 4 options together (2x4)
                /*let result1 =
                    tulip_rs::indicators::nightly::by_option::vhf::indicator_by_options::<4>(
                        &inputs,
                        &options_4_1,
                        None,
                    )
...
criterion_group!(
    vhf_benchmarks,
    bench_rust_vhf_simd_by_options,
    bench_rust_vhf_simd_by_assets,
    bench_rust_vhf,
    bench_c_vhf,
    bench_rust_vhf_from_state,
);
criterion_main!(vhf_benchmarks);
