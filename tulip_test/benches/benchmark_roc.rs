    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result =
                            Roc::indicator(&inputs, &options, None).expect("ROC indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
        ...
        for options in OPTIONS_LIST {
            ...
            group.bench_function(format!("Rust ROC {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let result =
                        Roc::indicator(&inputs, &options, None).expect("Rust ROC indicator failed");
                    black_box(&result);
                });
            });
            group.finish();
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let min_data = Roc::min_data(&options).max(CHUNK_SIZE);
                        // First chunk
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result = Roc::indicator(&inputs, &options, Some(&[true, true]))
                            .expect("Rust ROC indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
...
/// Benchmark the `ta` crate (RustTa) implementation of ROC.
fn bench_rust_ta_roc(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let mut roc = RateOfChange::new(period).expect("ta ROC new failed");
                ...
                log_timing_result("roc", "RustTa", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        // Run Criterion benchmark with synthetic data
        ...
        for options in OPTIONS_LIST {
            let period = options[0] as usize;
            let mut group = c.benchmark_group("roc_rust_ta");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("RustTa ROC {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut roc = RateOfChange::new(period).expect("ta ROC new failed");
                    let mut last = 0.0_f64;
