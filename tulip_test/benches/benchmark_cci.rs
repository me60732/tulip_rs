use criterion::{black_box, criterion_group, criterion_main, Criterion};
use tulip_rs::indicators::cci::{
    Cci, Indicator, IndicatorByOptions, IndicatorState, TIndicatorState,
};
use tulip_test::benchmark_logger::{init_logging, log_timing_result, should_log_to_db};
//use tulip_test::benchmark_utils::SAMPLE_SIZE;
const SAMPLE_SIZE: usize = 60000;
use tulip_test::c_bindings::{ti_cci, ti_cci_start};
use tulip_test::criterion_logger::TimingMeasurements;
use tulip_test::database::{get_all_stock_data, init_database_data};
#[cfg(feature = "talib")]
use tulip_test::talib_bindings::{ta_cci, ta_cci_start};

// Sample input data (high, low, and close prices)
const HIGH: [f64; 15] = [
    82.15, 81.89, 83.03, 83.30, 83.85, 83.90, 83.33, 84.30, 84.84, 85.00, 85.90, 86.58, 86.98,
    88.00, 87.87,
];
const LOW: [f64; 15] = [
    81.29, 80.64, 81.31, 82.65, 83.07, 83.11, 82.49, 82.30, 84.15, 84.11, 84.03, 85.39, 85.76,
    87.17, 87.01,
];
const CLOSE: [f64; 15] = [
    81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36, 85.53, 86.54, 86.89,
    87.77, 87.29,
];

// Options for CCI
//const OPTIONS_LIST: [[f64; 1]; 10] = [[5.0], [14.0], [20.0], [25.0], [30.0], [40.0], [50.0], [80.0], [100.0], [200.0]];
const OPTIONS_LIST: [[f64; 1]; 4] = [[20.0], [25.0], [30.0], [50.0]];
// Chunk size for batched processing
const CHUNK_SIZE: usize = 100;

fn expand_inputs() -> (Vec<f64>, Vec<f64>, Vec<f64>) {
    let mut high_vec = HIGH.to_vec();
    let mut low_vec = LOW.to_vec();
    let mut close_vec = CLOSE.to_vec();
    for _ in 0..500 {
        high_vec.extend_from_slice(&HIGH);
        low_vec.extend_from_slice(&LOW);
        close_vec.extend_from_slice(&CLOSE);
    }
    (high_vec, low_vec, close_vec)
}

/// Benchmark the C implementation of CCI.
fn bench_c_cci(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let high_vec: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low_vec: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close_vec: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let inputs: Vec<*const f64> =
                vec![high_vec.as_ptr(), low_vec.as_ptr(), close_vec.as_ptr()];

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let start_index = unsafe { ti_cci_start(options.as_ptr()) };
                        assert!(start_index >= 0, "ti_cci_start returned a negative index");
                        let output_len = high_vec.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];

                        let ret = unsafe {
                            ti_cci(
                                high_vec.len() as i32,
                                inputs.as_ptr(),
                                options.as_ptr(),
                                outputs.as_mut_ptr(),
                            )
                        };
                        assert_eq!(ret, 0, "ti_cci returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );

                log_timing_result(
                    "cci",
                    "C_tulip",
                    &options,
                    high_vec.len(),
                    &timing,
                    Some(stock_symbol),
                );
            }
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();
        let inputs: Vec<*const f64> = vec![high_vec.as_ptr(), low_vec.as_ptr(), close_vec.as_ptr()];

        for options in OPTIONS_LIST {
            let start_index = unsafe { ti_cci_start(options.as_ptr()) };
            assert!(start_index >= 0, "ti_cci_start returned a negative index");
            let output_len = high_vec.len() - (start_index as usize);

            let mut group = c.benchmark_group("cci_c");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("C CCI {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut output_vec = vec![0.0_f64; output_len];
                    let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];

                    let ret = unsafe {
                        ti_cci(
                            high_vec.len() as i32,
                            inputs.as_ptr(),
                            options.as_ptr(),
                            outputs.as_mut_ptr(),
                        )
                    };
                    assert_eq!(ret, 0, "ti_cci returned error code {}", ret);
                    black_box(&output_vec);
                })
            });
            group.finish();
        }
    }
}

/// Benchmark the Rust implementation of CCI.
fn bench_rust_cci(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let high_vec: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low_vec: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close_vec: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let inputs = [
                high_vec.as_slice(),
                low_vec.as_slice(),
                close_vec.as_slice(),
            ];

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let _result = Cci::indicator(&inputs, &options, None)
                            .expect("Rust CCI indicator failed");
                    },
                    SAMPLE_SIZE,
                );

                log_timing_result(
                    "cci",
                    "Rust",
                    &options,
                    inputs[0].len(),
                    &timing,
                    Some(stock_symbol),
                );
            }
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();
        let inputs = [
            high_vec.as_slice(),
            low_vec.as_slice(),
            close_vec.as_slice(),
        ];

        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("cci_rust");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Rust CCI {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let result =
                        Cci::indicator(&inputs, &options, None).expect("Rust CCI indicator failed");
                    black_box(&result);
                })
            });
            group.finish();
        }
    }
}

/// Benchmark the Rust from_state implementation of CCI.
fn bench_rust_cci_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        // Database logging mode - benchmark real market data
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let high: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let n = high.len();
            let inputs = [high.as_slice(), low.as_slice(), close.as_slice()];

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let min_data = Cci::min_data(&options);
                        // First chunk
                        let chunk_inputs = [&high[..min_data], &low[..min_data], &low[..min_data]];

                        let (_, mut state) = Cci::indicator(&chunk_inputs, &options, None)
                            .expect("CCI indicator failed");

                        // Chunks
                        let mut high_chunks = high[min_data..].chunks_exact(CHUNK_SIZE);
                        let mut low_chunks = low[min_data..].chunks_exact(CHUNK_SIZE);
                        let mut close_chunks = close[min_data..].chunks_exact(CHUNK_SIZE);

                        for ((high_chunk, low_chunk), close_chunk) in high_chunks
                            .by_ref()
                            .zip(low_chunks.by_ref())
                            .zip(close_chunks.by_ref())
                        {
                            let chunk_inputs = [high_chunk, low_chunk, close_chunk];
                            let result = state.batch_indicator(&chunk_inputs, None);
                            black_box(&result);
                        }

                        // Remainder
                        let high_rem = high_chunks.remainder();
                        let low_rem = low_chunks.remainder();
                        let close_rem = close_chunks.remainder();

                        if !high_rem.is_empty() {
                            let chunk_inputs = [high_rem, low_rem, close_rem];
                            let result = state.batch_indicator(&chunk_inputs, None);
                            black_box(&result);
                        }
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result(
                    "cci",
                    "Rust_FromState",
                    &options,
                    n,
                    &timing,
                    Some(stock_symbol),
                );

                // --- Rust_FromState_1_Bar benchmark ---
                if inputs[0].len() > 1 {
                    let new_inputs = [
                        &high[..high.len() - 1],
                        &low[..low.len() - 1],
                        &close[..close.len() - 1],
                    ];
                    let final_inputs = [
                        &high[high.len() - 1..],
                        &low[low.len() - 1..],
                        &close[close.len() - 1..],
                    ];
                    let (_, mut state) = Cci::indicator(&new_inputs, &options, None)
                        .expect("Rust CCI indicator failed");

                    let mut timing = TimingMeasurements::new();
                    timing.measure(
                        || {
                            let result = state
                                .batch_indicator(&final_inputs, None)
                                .expect("Rust CCI from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );

                    log_timing_result(
                        "cci",
                        "Rust_FromState_1_Bar",
                        &options,
                        n,
                        &timing,
                        Some(stock_symbol),
                    );

                    // --- Rust_FromState_1_Bar_json benchmark ---
                    let (_, state) = Cci::indicator(&new_inputs, &options, None)
                        .expect("Rust CCI indicator failed");
                    let json = serde_json::to_string(&state).expect("json failed");

                    let mut timing = TimingMeasurements::new();
                    timing.measure(
                        || {
                            let mut state: IndicatorState =
                                serde_json::from_str(&json).expect("JSON failed");
                            let result = state
                                .batch_indicator(&final_inputs, None)
                                .expect("Rust CCI from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );

                    log_timing_result(
                        "cci",
                        "Rust_FromState_1_Bar_json",
                        &options,
                        n,
                        &timing,
                        Some(stock_symbol),
                    );
                }
            }
        }
    } else {
        // Criterion profiling mode - benchmark synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();
        let _inputs = [&high_vec, &low_vec, &close_vec];

        for options in OPTIONS_LIST {
            let min_data = Cci::min_data(&options);
            // First chunk
            let chunk_inputs = [
                &high_vec[..min_data],
                &low_vec[..min_data],
                &close_vec[..min_data],
            ];

            let (_, mut state) =
                Cci::indicator(&chunk_inputs, &options, None).expect("CCI indicator failed");

            let mut group = c.benchmark_group("cci_rust_from_state");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Rust CCI from state {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut high_chunks = high_vec[min_data..].chunks_exact(CHUNK_SIZE);
                    let mut low_chunks = low_vec[min_data..].chunks_exact(CHUNK_SIZE);
                    let mut close_chunks = close_vec[min_data..].chunks_exact(CHUNK_SIZE);

                    for ((high_chunk, low_chunk), close_chunk) in high_chunks
                        .by_ref()
                        .zip(low_chunks.by_ref())
                        .zip(close_chunks.by_ref())
                    {
                        let chunk_inputs = [high_chunk, low_chunk, close_chunk];
                        let result = state.batch_indicator(&chunk_inputs, None);
                        black_box(&result);
                    }

                    // Remainder
                    let high_rem = high_chunks.remainder();
                    let low_rem = low_chunks.remainder();
                    let close_rem = close_chunks.remainder();

                    if !high_rem.is_empty() {
                        let chunk_inputs = [high_rem, low_rem, close_rem];
                        let result = state.batch_indicator(&chunk_inputs, None);
                        black_box(&result);
                    }
                });
            });
            group.finish();

            // Benchmark with 1 bar from state
            if high_vec.len() > 1 {
                let new_inputs = [
                    &high_vec[..high_vec.len() - 1],
                    &low_vec[..low_vec.len() - 1],
                    &close_vec[..close_vec.len() - 1],
                ];
                let final_inputs = [
                    &high_vec[high_vec.len() - 1..],
                    &low_vec[low_vec.len() - 1..],
                    &close_vec[close_vec.len() - 1..],
                ];
                let (_, mut state) =
                    Cci::indicator(&new_inputs, &options, None).expect("Rust CCI indicator failed");

                let mut group = c.benchmark_group("cci_rust_from_state_1_bar");
                group.sample_size(SAMPLE_SIZE);
                group.bench_function(
                    format!("Rust CCI from state 1 bar {{ {} }}", options[0]),
                    |b| {
                        b.iter(|| {
                            let result = state
                                .batch_indicator(&final_inputs, None)
                                .expect("Rust CCI from state indicator failed");
                            black_box(&result);
                        });
                    },
                );
                group.finish();
            }
        }
    }
}

/// Benchmark the Rust implementation of CCI with optional outputs.
fn bench_rust_cci_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let high_vec: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low_vec: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close_vec: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let inputs = [
                high_vec.as_slice(),
                low_vec.as_slice(),
                close_vec.as_slice(),
            ];

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let result = Cci::indicator(&inputs, &options, Some(&[true, true, true]))
                            .expect("Rust CCI indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );

                log_timing_result(
                    "cci",
                    "Rust_optional",
                    &options,
                    inputs[0].len(),
                    &timing,
                    Some(stock_symbol),
                );
            }
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();
        let inputs = [
            high_vec.as_slice(),
            low_vec.as_slice(),
            close_vec.as_slice(),
        ];

        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("cci_rust");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Rust CCI {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let result = Cci::indicator(&inputs, &options, Some(&[true, true, true]))
                        .expect("Rust CCI indicator failed");
                    black_box(&result);
                });
            });
            group.finish();
        }
    }
}

/// Benchmark the TA-Lib implementation of CCI.
#[cfg(feature = "talib")]
fn bench_talib_cci(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let high: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let n = high.len();
            let inputs: Vec<*const f64> = vec![high.as_ptr(), low.as_ptr(), close.as_ptr()];

            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();

                timing.measure(
                    || {
                        let start_index = ta_cci_start(options[0]);
                        assert!(start_index >= 0, "ta_cci_start returned a negative index");
                        let output_len = high.len() - (start_index as usize);
                        let mut output_vec = vec![0.0_f64; output_len];
                        let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];
                        let ret = ta_cci(
                            high.len() as i32,
                            inputs.as_ptr(),
                            options.as_ptr(),
                            outputs.as_mut_ptr(),
                        );
                        assert_eq!(ret, 0, "ta_cci returned error code {}", ret);
                        black_box(&output_vec);
                    },
                    SAMPLE_SIZE,
                );

                log_timing_result("cci", "talib", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();
        let inputs: Vec<*const f64> = vec![high_vec.as_ptr(), low_vec.as_ptr(), close_vec.as_ptr()];

        for options in OPTIONS_LIST {
            let start_index = ta_cci_start(options[0]);
            assert!(start_index >= 0, "ta_cci_start returned a negative index");
            let output_len = high_vec.len() - (start_index as usize);

            let mut group = c.benchmark_group("cci_talib");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("TA-Lib CCI {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut output_vec = vec![0.0_f64; output_len];
                    let mut outputs: Vec<*mut f64> = vec![output_vec.as_mut_ptr()];

                    let ret = ta_cci(
                        high_vec.len() as i32,
                        inputs.as_ptr(),
                        options.as_ptr(),
                        outputs.as_mut_ptr(),
                    );
                    assert_eq!(ret, 0, "ta_cci returned error code {}", ret);
                    black_box(&output_vec);
                });
            });
            group.finish();
        }
    }
}

/// Benchmark the Rust SIMD by assets implementation of CCI.
fn bench_rust_cci_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        // Get first 4 stocks' data
        let stock_data: Vec<(String, Vec<f64>, Vec<f64>, Vec<f64>)> = data
            .iter()
            .take(4)
            .map(|(symbol, data)| {
                (
                    symbol.clone(),
                    data.iter().map(|d| d.high).collect(),
                    data.iter().map(|d| d.low).collect(),
                    data.iter().map(|d| d.close).collect(),
                )
            })
            .collect();

        // Prepare inputs in the format expected by indicator_by_assets
        let inputs: [&[&[f64]; 3]; 4] = [
            &[
                &stock_data[0].1, // high
                &stock_data[0].2, // low
                &stock_data[0].3, // close
            ],
            &[
                &stock_data[1].1, // high
                &stock_data[1].2, // low
                &stock_data[1].3, // close
            ],
            &[
                &stock_data[2].1, // high
                &stock_data[2].2, // low
                &stock_data[2].3, // close
            ],
            &[
                &stock_data[3].1, // high
                &stock_data[3].2, // low
                &stock_data[3].3, // close
            ],
        ];

        for options in OPTIONS_LIST {
            let mut timing = TimingMeasurements::new();
            timing.measure(
                || {
                    let result = Cci::indicator_by_assets::<4>(&inputs, &options, None)
                        .expect("Rust SIMD by assets CCI indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );

            log_timing_result(
                "cci",
                "Rust_SIMD_by_assets",
                &options,
                stock_data[0].1.len(),
                &timing,
                Some("4_Assets"),
            );
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();

        // Create 4 identical datasets for SIMD processing
        let inputs: [&[&[f64]; 3]; 4] = [
            &[&high_vec, &low_vec, &close_vec],
            &[&high_vec, &low_vec, &close_vec],
            &[&high_vec, &low_vec, &close_vec],
            &[&high_vec, &low_vec, &close_vec],
        ];

        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("cci_rust_simd_by_assets");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(
                format!("Rust SIMD by assets CCI {{ {} }}", options[0]),
                |b| {
                    b.iter(|| {
                        let result = Cci::indicator_by_assets::<4>(&inputs, &options, None)
                            .expect("Rust SIMD by assets CCI indicator failed");
                        black_box(&result);
                    });
                },
            );
            group.finish();
        }
    }
}

fn bench_rust_cci_simd_by_options(c: &mut Criterion) {
    let options_4 = [
        &OPTIONS_LIST[0],
        &OPTIONS_LIST[1],
        &OPTIONS_LIST[2],
        &OPTIONS_LIST[3],
    ];
    if should_log_to_db() {
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let high_vec: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low_vec: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close_vec: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let inputs = [
                high_vec.as_slice(),
                low_vec.as_slice(),
                close_vec.as_slice(),
            ];

            let mut timing = TimingMeasurements::new();
            timing.measure(
                || {
                    // Process first 4 options with 4-wide SIMD
                    let result = Cci::indicator_by_options::<4>(&inputs, &options_4, None)
                        .expect("Rust SIMD CCI indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );

            log_timing_result(
                "cci",
                "Rust_SIMD",
                &[0.0],
                high_vec.len(),
                &timing,
                Some(stock_symbol),
            );
        }
    } else {
        // Run Criterion benchmark with synthetic data
        let (high_vec, low_vec, close_vec) = expand_inputs();
        let inputs = [
            high_vec.as_slice(),
            low_vec.as_slice(),
            close_vec.as_slice(),
        ];

        let mut group = c.benchmark_group("cci_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options CCI (4 lanes)", |b| {
            b.iter(|| {
                // Process first 4 options with 4-wide SIMD
                let result = Cci::indicator_by_options::<4>(&inputs, &options_4, None)
                    .expect("Rust SIMD CCI indicator failed");
                black_box(&result);
            });
        });

        group.finish();
    }
}

fn bench_rust_ta_cci(c: &mut Criterion) {
    use ta::indicators::CommodityChannelIndex;
    use ta::{DataItem, Next};

    if should_log_to_db() {
        init_database_data();
        init_logging("cci");

        let data = get_all_stock_data().unwrap();

        for (stock_symbol, stock_data) in data {
            let high: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let n = close.len();

            for options in OPTIONS_LIST {
                let period = options[0] as usize;
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let mut cci =
                            CommodityChannelIndex::new(period).expect("ta CCI new failed");
                        let mut last = 0.0_f64;
                        for i in 0..high.len() {
                            let h = high[i].max(close[i]);
                            let l = low[i].min(close[i]);
                            let item = DataItem::builder()
                                .high(h)
                                .low(l)
                                .close(close[i])
                                .open(close[i])
                                .volume(1000.0)
                                .build()
                                .expect("DataItem build failed");
                            last = cci.next(&item);
                        }
                        black_box(last);
                    },
                    SAMPLE_SIZE,
                );

                log_timing_result("cci", "RustTa", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        let (high_vec, low_vec, close_vec) = expand_inputs();

        for options in OPTIONS_LIST {
            let period = options[0] as usize;
            let mut group = c.benchmark_group("cci_rust_ta");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("RustTa CCI {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut cci = CommodityChannelIndex::new(period).expect("ta CCI new failed");
                    let mut last = 0.0_f64;
                    for i in 0..high_vec.len() {
                        let h = high_vec[i].max(close_vec[i]);
                        let l = low_vec[i].min(close_vec[i]);
                        let item = DataItem::builder()
                            .high(h)
                            .low(l)
                            .close(close_vec[i])
                            .open(close_vec[i])
                            .volume(1000.0)
                            .build()
                            .expect("DataItem build failed");
                        last = cci.next(&item);
                    }
                    black_box(last);
                });
            });
            group.finish();
        }
    }
}

fn bench_kand_cci(c: &mut Criterion) {
    use kand::ohlcv::cci;

    if should_log_to_db() {
        init_database_data();
        init_logging("cci");
        let data = get_all_stock_data().unwrap();
        for (stock_symbol, stock_data) in data {
            let high: Vec<f64> = stock_data.iter().map(|d| d.high).collect();
            let low: Vec<f64> = stock_data.iter().map(|d| d.low).collect();
            let close: Vec<f64> = stock_data.iter().map(|d| d.close).collect();
            let n = close.len();
            for options in OPTIONS_LIST {
                let period = options[0] as usize;
                let mut timing = TimingMeasurements::new();
                timing.measure(
                    || {
                        let mut out_cci = vec![0.0_f64; n];
                        let mut out_tp = vec![0.0_f64; n];
                        let mut out_sma = vec![0.0_f64; n];
                        let mut out_md = vec![0.0_f64; n];
                        cci::cci(
                            &high,
                            &low,
                            &close,
                            period,
                            &mut out_cci,
                            &mut out_tp,
                            &mut out_sma,
                            &mut out_md,
                        )
                        .unwrap();
                        black_box(&out_cci);
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result("cci", "RustKanda", &options, n, &timing, Some(stock_symbol));
            }
        }
    } else {
        let (high_vec, low_vec, close_vec) = expand_inputs();
        let n = close_vec.len();
        for options in OPTIONS_LIST {
            let period = options[0] as usize;
            let mut group = c.benchmark_group("cci_kand");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(format!("Kand CCI {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let mut out_cci = vec![0.0_f64; n];
                    let mut out_tp = vec![0.0_f64; n];
                    let mut out_tp_sma = vec![0.0_f64; n];
                    let mut out_mean_dev = vec![0.0_f64; n];
                    cci::cci(
                        &high_vec,
                        &low_vec,
                        &close_vec,
                        period,
                        &mut out_cci,
                        &mut out_tp,
                        &mut out_tp_sma,
                        &mut out_mean_dev,
                    )
                    .unwrap();
                    black_box(out_cci);
                });
            });
            group.finish();
        }
    }
}

#[cfg(feature = "talib")]
criterion_group!(
    benches,
    bench_rust_cci_simd_by_assets,
    bench_rust_cci_simd_by_options,
    bench_rust_cci,
    bench_rust_ta_cci,
    bench_c_cci,
    bench_talib_cci,
    bench_rust_cci_from_state,
    bench_rust_cci_optional,
    bench_kand_cci,
);

#[cfg(not(feature = "talib"))]
criterion_group!(
    benches,
    bench_rust_cci_simd_by_assets,
    bench_rust_cci_simd_by_options,
    bench_rust_cci,
    bench_rust_ta_cci,
    bench_c_cci,
    bench_rust_cci_from_state,
    bench_rust_cci_optional,
    bench_kand_cci,
);
criterion_main!(benches);
