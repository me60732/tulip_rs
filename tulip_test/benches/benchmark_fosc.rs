...
use tulip_test::c_bindings::{ti_fosc, ti_fosc_start};
use tulip_test::criterion_logger::TimingMeasurements;
use tulip_test::database::{get_all_stock_data, init_database_data};

// Test data from fosc_test.rs
const CLOSE: [f64; 15] = [
    81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36, 85.53, 86.54, 86.89,
    87.77, 87.29,
];

// Options from fosc_test.rs
...
fn bench_rust_fosc(c: &mut Criterion) {
    if should_log_to_db() {
        // Database logging mode - benchmark real market data
        init_database_data();
        init_logging("fosc");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result = Fosc::indicator(&[close.as_slice()], &options, None); //.expect("FOSC Fosc::indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
...
fn bench_rust_fosc_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("fosc");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                let mut timing = TimingMeasurements::new();
                let min_data = Fosc::min_data(&options);
                timing.measure(
                    || {
                        // First chunk
                        ...
                        let (_, mut state) = Fosc::indicator(&chunk_inputs, &options, None)
                ...
                if inputs[0].len() > 1 {
                    ...
                    timing.measure(
                        || {
                            let result = state
                                .batch_indicator(&[&close[close.len() - 1..]], None)
                                .expect("Rust FOSC from state Fosc::indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
        ...
        for options in OPTIONS_LIST {
            ...
            if close.len() > 1 {
                //let new_inputs = [&close[..close.len() - 1]];
                //let _final_inputs = [&close[close.len() - 1..]];
                let (_, mut state) = Fosc::indicator(&[&close[..close.len() - 1]], &options, None)
...
fn bench_rust_fosc_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("fosc");
...
/// Benchmark the Rust implementation of FOSC with optional outputs.
fn bench_rust_fosc_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("fosc");
...
fn bench_rust_fosc_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("fosc");
        ...
        let mut group = c.benchmark_group("fosc_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options FOSC (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
...
criterion_group!(
    fosc_benchmarks,
    bench_rust_fosc_simd_by_assets,
...
criterion_main!(fosc_benchmarks);
