...
use tulip_test::c_bindings::{ti_linreg, ti_linreg_start};
use tulip_test::criterion_logger::TimingMeasurements;
use tulip_test::database::{get_all_stock_data, init_database_data};

// Test data from linreg_test.rs
const CLOSE: [f64; 15] = [
    81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36, 85.53, 86.54, 86.89,
    87.77, 87.29,
];

// Options from linreg_test.rs
...
fn bench_c_linreg(c: &mut Criterion) {
    if should_log_to_db() {
...
fn bench_rust_linreg(c: &mut Criterion) {
    if should_log_to_db() {
        // Database logging mode - benchmark real market data
        init_database_data();
        init_logging("linreg");
...
fn bench_rust_linreg_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        // Database logging mode - benchmark real market data
        init_database_data();
        init_logging("linreg");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust LINREG from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the Rust SIMD by assets implementation of LINREG.
fn bench_rust_linreg_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("linreg");
...
/// Benchmark the Rust implementation of LINREG with optional outputs.
fn bench_rust_linreg_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("linreg");
...
/// Benchmark the Rust LINREG SIMD by options implementation.
fn bench_rust_linreg_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("linreg");
        ...
        let mut group = c.benchmark_group("linreg_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options LINREG (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
...
criterion_main!(linreg_benchmarks);
