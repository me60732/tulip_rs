...
// Options for WILDERS (period)
const OPTIONS_LIST: [[f64; 1]; 8] = [[5.0], [7.0], [10.0], [14.0], [20.0], [25.0], [30.0], [50.0]];
...
/// Benchmark the C implementation of WILDERS.
fn bench_c_wilders(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
...
/// Benchmark the Rust implementation of WILDERS.
fn bench_rust_wilders(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wilders");
...
/// Benchmark the Rust from_state implementation of WILDERS.
fn bench_rust_wilders_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wilders");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust WILDERS from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
...
/// Benchmark the Rust SIMD by assets implementation of WILDERS.
fn bench_rust_wilders_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("wilders");
...
fn bench_rust_wilders_simd_by_options(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    let result_4 = indicator_by_options::<8>(&inputs, &options_8, None)
                        .expect("Rust SIMD WILDERS indicator failed");
                    black_box(&result_4);

                    // Process remaining 2 options with 2-wide SIMD
                    /*let result_2 = indicator_by_options::<4>(&inputs, &options_2, None)
                        .expect("Rust SIMD WILDERS indicator failed");
                    black_box(&result_2);*/
                },
                SAMPLE_SIZE,
        ...
        let mut group = c.benchmark_group("wilders_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD WILDERS (4+2 lanes)", |b| {
            b.iter(|| {
                // Process first 4 options with 4-wide SIMD
