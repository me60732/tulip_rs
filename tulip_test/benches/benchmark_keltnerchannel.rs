...
// Helper function to get OHLCV arrays from stock data
fn get_arrays(
...
/// Benchmark the Rust implementation of Keltner Channel.
fn bench_rust_keltnerchannel(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("keltnerchannel_rust");
...
/// Benchmark the Rust from_state implementation of Keltner Channel.
fn bench_rust_keltnerchannel_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("keltnerchannel");
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust KeltnerChannel from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
        ...
        for options in OPTIONS_LIST {
            ...
            group.bench_function("benchmark", |b| {
                b.iter(|| {
                    ...
                    let close_rem = close_chunks.remainder();
                    if !high_rem.is_empty() {
                        let result = state.batch_indicator(&[high_rem, low_rem, close_rem], None);
                        black_box(&result);
                    }
                });
            });
            ...
            if high_vec.len() > 1 {
                ...
                let mut group = c.benchmark_group(format!(
                    "Rust KeltnerChannel from state 1 bar {{ {:.1}, {:.1} }}",
                    options[0], options[1]
                ));
                group.sample_size(SAMPLE_SIZE);
                group.bench_function("benchmark", |b| {
...
/// Benchmark the Rust implementation of Keltner Channel with optional outputs (atr, tr).
fn bench_rust_keltnerchannel_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("keltnerchannel");
        ...
        for options in OPTIONS_LIST {
            // atr only
            let mut group = c.benchmark_group("keltnerchannel_rust_optional_atr");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(
                format!(
                    "Rust KeltnerChannel optional atr {{ {}, {} }}",
                    options[0], options[1]
                ),
                |b| {
                    b.iter(|| {
...
/// Benchmark the `ta` crate implementation of Keltner Channel.
fn bench_rust_ta_keltnerchannel(c: &mut Criterion) {
    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let mut kc = KeltnerChannel::new(period, multiplier)
                            .expect("ta KeltnerChannel new failed");
                        let mut last_lower = 0.0_f64;
                        let mut last_middle = 0.0_f64;
                        let mut last_upper = 0.0_f64;
                        for i in 0..high.len() {
                            let item = unsafe {
                                DataItem::builder()
                            ...
                            last_lower = out.lower;
                            last_middle = out.average;
                            last_upper = out.upper;
                        }
                        black_box(last_lower);
                        black_box(last_middle);
                        black_box(last_upper);
                    },
                    SAMPLE_SIZE,
                );
        ...
        for options in OPTIONS_LIST {
            let period = options[0] as usize;
            let multiplier = options[1];
            let mut group = c.benchmark_group("keltnerchannel_rust_ta");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(
                format!("RustTa KeltnerChannel {{ {}, {} }}", options[0], options[1]),
                |b| {
                    b.iter(|| {
                        let mut kc = KeltnerChannel::new(period, multiplier)
                            .expect("ta KeltnerChannel new failed");
                        ...
                        for i in 0..high_vec.len() {
                            // Clamp to satisfy DataItem's high >= close >= low invariant on repeated data.
...
/// Benchmark the Rust SIMD by-assets implementation of Keltner Channel.
fn bench_rust_keltnerchannel_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("keltnerchannel");
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("keltnerchannel_rust_simd_by_assets");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(
                format!(
                    "Rust SIMD by assets KC {{ {}, {} }}",
                    options[0], options[1]
                ),
                |b| {
                    b.iter(|| {
...
/// Benchmark the Rust SIMD by-options implementation of Keltner Channel.
fn bench_rust_keltnerchannel_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("keltnerchannel");
        ...
        let mut group = c.benchmark_group("keltnerchannel_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options KC (4 lanes)", |b| {
            b.iter(|| {
                let options_4 = [
                    &OPTIONS_LIST[0],
                    &OPTIONS_LIST[1],
...
criterion_group!(
    ...
    bench_rust_keltnerchannel_simd_by_options,
    bench_rust_keltnerchannel,
