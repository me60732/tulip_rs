    ...
    if should_log_to_db() {
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                log_timing_result("dx", "C_tulip", &options, n, &timing, Some(&stock_symbol));
            }
        }
    } else {
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group(&format!("C DX {{ {:.1} }}", options[0]));
...
fn bench_rust_dx(c: &mut Criterion) {
    if should_log_to_db() {
        // Database logging mode - benchmark real market data
        init_database_data();
        init_logging("dx");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        let result =
                            indicator(&inputs, &options, None).expect("DX indicator failed");
                        black_box(&result);
                    },
                    SAMPLE_SIZE,
                );
                log_timing_result("dx", "Rust", &options, n, &timing, Some(&stock_symbol));
            }
        }
    } else {
        // Criterion profiling mode - benchmark synthetic data
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group(&format!("Rust DX {{ {:.1} }}", options[0]));
            group.sample_size(SAMPLE_SIZE);

            group.bench_function("benchmark", |b| {
                b.iter(|| {
                    let inputs = [high.as_slice(), low.as_slice(), close.as_slice()];
                    let result = indicator(&inputs, &options, None).expect("DX indicator failed");
                    black_box(&result);
                });
            });
...
fn bench_rust_dx_from_state(c: &mut Criterion) {
    if should_log_to_db() {
        // Database logging mode - benchmark real market data
        init_database_data();
        init_logging("dx");
        ...
        for options in OPTIONS_LIST {
            let mut group =
                c.benchmark_group(&format!("Rust DX from state {{ {:.1} }}", options[0]));
            group.sample_size(SAMPLE_SIZE);
...
/// Benchmark the Rust implementation of DX with optional outputs.
fn bench_rust_dx_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("dx");
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group("dx_rust");
            group.sample_size(SAMPLE_SIZE);
            group.bench_function(&format!("Rust DX {{ {} }}", options[0]), |b| {
                b.iter(|| {
                    let result = indicator(&inputs, &options, Some(&[true, true]))
                        .expect("Rust DX indicator failed");
                    black_box(&result);
...
fn bench_rust_dx_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("dx");
        ...
        for options in &OPTIONS_LIST {
            ...
            let mut group = c.benchmark_group("dx_rust_simd_by_assets");
            group.sample_size(SAMPLE_SIZE);
...
fn bench_rust_dx_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("dx");
        ...
        let mut group = c.benchmark_group("dx_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options DX (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
