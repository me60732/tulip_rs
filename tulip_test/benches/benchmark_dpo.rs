...
use tulip_rs::indicators::dpo::{
    Dpo, Indicator, IndicatorByOptions, IndicatorState, TIndicatorState,
};
use tulip_test::benchmark_logger::{init_logging, log_timing_result, should_log_to_db};
use tulip_test::benchmark_utils::SAMPLE_SIZE;
use tulip_test::c_bindings::{ti_dpo, ti_dpo_start};
use tulip_test::criterion_logger::TimingMeasurements;
use tulip_test::database::{get_all_stock_data, init_database_data};

// Test data from dpo_test.rs
const CLOSE: [f64; 15] = [
    81.59, 81.06, 82.87, 83.00, 83.61, 83.15, 82.84, 83.99, 84.55, 84.36, 85.53, 86.54, 86.89,
    87.77, 87.29,
];

// Options from dpo_test.rs
...
fn bench_c_dpo(c: &mut Criterion) {
    if should_log_to_db() {
...
fn bench_rust_dpo(c: &mut Criterion) {
    if should_log_to_db() {
        // Database logging mode - benchmark real market data
        init_database_data();
        init_logging("dpo");
    ...
    if should_log_to_db() {
            ...
            for options in OPTIONS_LIST {
                ...
                timing.measure(
                    || {
                        ...
                        let (_, mut state) = Dpo::indicator(&chunk_inputs, &options, None)
                            .expect("DPO indicator failed");
                ...
                if inputs[0].len() > 1 {
                    ...
                    let (_, mut state) = Dpo::indicator(&new_inputs, &options, None)
                    ...
                    timing.measure(
                        || {
                            let result = state
                                ...
                                .expect("Rust DPO from state indicator failed");
                            black_box(&result);
                        },
                        SAMPLE_SIZE,
                    );
                    ...
                    let (_, state) = Dpo::indicator(&new_inputs, &options, None)
                        .expect("Rust DPO indicator failed");
        ...
        for options in OPTIONS_LIST {
            ...
            if close.len() > 1 {
                ...
                let mut group =
                    c.benchmark_group(format!("Rust DPO from state 1 bar {{ {:.1} }}", options[0]));
                group.sample_size(SAMPLE_SIZE);
                group.bench_function("benchmark", |b| {
                    b.iter(|| {
                        let result = state
...
/// Benchmark the Rust implementation of DPO with optional outputs.
fn bench_rust_dpo_optional(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("dpo");
        ...
        for options in OPTIONS_LIST {
            let mut group = c.benchmark_group(format!("Rust DPO optional {{ {:.1} }}", options[0]));
            group.sample_size(SAMPLE_SIZE);
            group.bench_function("benchmark", |b| {
                b.iter(|| {
                    let result = Dpo::indicator(&inputs, &options, Some(&[true]))
...
/// Benchmark the Rust SIMD by assets implementation of DPO.
fn bench_rust_dpo_simd_by_assets(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("dpo");
        ...
        for options in OPTIONS_LIST {
            ...
            timing.measure(
                || {
                    let result = Dpo::indicator_by_assets::<4>(&inputs, &options, None)
                        .expect("Rust SIMD by assets DPO indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );
...
fn bench_rust_dpo_simd_by_options(c: &mut Criterion) {
    if should_log_to_db() {
        init_database_data();
        init_logging("dpo");
        ...
        for (stock_symbol, stock_data) in data {
            ...
            timing.measure(
                || {
                    ...
                    let result = Dpo::indicator_by_options::<4>(&inputs, &options_4, None)
                        .expect("Rust SIMD DPO indicator failed");
                    black_box(&result);
                },
                SAMPLE_SIZE,
            );
        ...
        let mut group = c.benchmark_group("dpo_rust_simd_by_options");
        group.sample_size(SAMPLE_SIZE);
        group.bench_function("Rust SIMD by options DPO (4 lanes)", |b| {
            b.iter(|| {
                // Process all 4 options with 4-wide SIMD
                let options_4 = [
                    &OPTIONS_LIST[0],
