...
int ti_cos_start(TI_REAL const *options) { (void)options; return 0; } int 
ti_cos(int size, TI_REAL const *const *inputs, TI_REAL const *options, TI_REAL *const *outputs) { 
...
int ti_dx(int size, TI_REAL const *const *inputs, TI_REAL const *options, TI_REAL *const *outputs) {
    ...
    for (i = 1; i < period; ++i) {
        ...
        do {
            dp = high[i] - high[i-1];
            dm = low[i-1] - low[i];
            if (dp < 0) dp = 0; else if (dp > dm) dm = 0;
            if (dm < 0) dm = 0; else if (dm > dp) dp = 0;
        } while (0);
        dmup += dp;
        dmdown += dm;
    }
...
int ti_linreg(int size, TI_REAL const *const *inputs, TI_REAL const *options, TI_REAL *const *outputs) {
    ...
    do {
        ...
        const TI_REAL bd = 1.0 / ((period) * x2 - x * x); //multiplier

        for (i = (period)-1; i < (size); ++i) {
            xy += (input)[i] * (period);
            y += (input)[i];
            const TI_REAL b = ((period) * xy - x * y) * bd; //slope
            do {
                const TI_REAL a = (y - b * x) * p; //intercept
                *output++ = (a + b * ((period))); //linreg
            } while (0);
            xy -= y;
            y -= (input)[i-(period)+1];
        }
...
int ti_linregintercept(int size, TI_REAL const *const *inputs, TI_REAL const *options, TI_REAL *const *outputs) {
    ...
    do {
        ...
        for (i = (period)-1; i < (size); ++i) {
            ...
            const TI_REAL b = ((period) * xy - x * y) * bd; //slope
            do {
                const TI_REAL a = (y - b * x) * p; //intercept diffrent per??? 1/period
                *output++ = (a + b * ((1)));
            } while (0);
            xy -= y;
            y -= (input)[i-(period)+1];
...
int ti_macd(int size, TI_REAL const *const *inputs, TI_REAL const *options, TI_REAL *const *outputs) {
    ...
    /*if (short_period == 12 && long_period == 26) { // why is this in here??????????? what a hack!
        short_per = 0.15;
        long_per = 0.075;
    }*/
    TI_REAL short_ema = input[0];
...
int ti_min(int size, TI_REAL const *const *inputs, TI_REAL const *options, TI_REAL *const *outputs) {
    ...
    for (i = period-1; i < size; ++i, ++trail) {
        TI_REAL bar = input[i];
        if (mini < trail) {
            // Increment counter when full window search happens
            search_counter++;
    ...
    // Print the counter
    printf("C: Period: %d, Counter: %d\n", period, search_counter);
